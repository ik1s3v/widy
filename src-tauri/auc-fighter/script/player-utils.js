...
var Animation = function (
	...
	this.Vx = 0;
	this.Vy = 0;
	this.chainVxFn = function (x) {
		return x;
	};
	this.chainVyFn = function (y) {
		return y;
	};
	this.vyFn = function (a) {
		return function (b) {
			return b;
		};
	};
	this.vxFn = function (a) {
		return function (b) {
			return b;
		};
	};
	this.vyAirFn = function (a) {
		return function (b) {
			return b;
		};
	};
	this.vxAirFn = function (a) {
		return function (b) {
			return b;
		};
	};
...
Animation.prototype.setDefaultHitStop = function (local, foe) {
	this.DefaultLocalHitStop = local;
	this.DefaultEnemyHitStop = foe;
};
Animation.prototype.getXModifier = function () {
	return this.xModifier(this.VxFnArgs);
};
Animation.prototype.getYModifier = function () {
	return this.yModifier(this.VyFnArgs);
};
Animation.prototype.getAirYModifier = function () {
	return this.airYModifier(this.VyAirFnArgs);
};
Animation.prototype.getAirXModifier = function () {
	return this.airXModifier(this.VxAirFnArgs);
};
Animation.prototype.xModifier = function (args) {
	return this.vxFn(args);
};
Animation.prototype.yModifier = function (args) {
	return this.vyFn(args);
};
Animation.prototype.airYModifier = function (args) {
	return this.vyAirFn(args);
};
Animation.prototype.airXModifier = function (args) {
	return this.vxAirFn(args);
};
Animation.prototype.setOtherPlayerAirborneFlags = function (flags) {
	this.OtherPlayerAirborneFlags = flags;
};
Animation.prototype.getOtherPlayerAirborneFlags = function () {
	return this.OtherPlayerAirborneFlags;
};
Animation.prototype.isAttack = function () {
	return this.BaseAnimation.IsAttack;
};
...
Animation.prototype.endBlock = function () {
	for (var i = 0; i < this.BaseAnimation.Frames.length; ++i) {
		if (
			!!hasFlag(
				this.BaseAnimation.Frames[i].FlagsToSet.Combat,
				COMBAT_FLAGS.ATTACK,
			)
		)
			break;
...
Animation.prototype.chainOnButtonPress = function (
	move,
	frameOffset,
	key,
	offset,
...
Animation.prototype.setDefaultsForLastFrames = function (nbFrames) {
	...
	while (!!i) {
		if (!!this.DefaultLocalHitStop)
			this.BaseAnimation.Frames[this.BaseAnimation.Frames.length - i].HitStop =
				this.DefaultLocalHitStop;
		if (!!this.DefaultEnemyHitStop)
			this.BaseAnimation.Frames[
				this.BaseAnimation.Frames.length - i
			].EnemyHitStop = this.DefaultEnemyHitStop;
		--i;
	}
};

Animation.prototype.addFrameWithSound = function (
	player,
	volume,
	soundFilename,
	shadowOffset,
	...
	this.IgnoresCollisions =
		!!flagsToSet && hasFlag(flagsToSet.Player, PLAYER_FLAGS.IGNORE_COLLISIONS);
...
Animation.prototype.addOffsetFrame = function (
	player,
	shadowOffset,
	shadowImage,
	image,
...
Animation.prototype.addRepeatingFrameWithSound = function (
	player,
	volume,
	soundFilename,
	shadowOffset,
	...
	this.IgnoresCollisions =
		!!flagsToSet && hasFlag(flagsToSet.Player, PLAYER_FLAGS.IGNORE_COLLISIONS);
	this.BaseAnimation.addRepeatingFrameWithSound.apply(
		this.BaseAnimation,
		arguments,
	);
...
Animation.prototype.addRepeatingFrame = function (
	player,
	shadowOffset,
	shadowImage,
	image,
...
Animation.prototype.getNextFrameOffset = function (id) {
	return this.BaseAnimation.getNextFrameOffset.apply(
		this.BaseAnimation,
		arguments,
	);
};
Animation.prototype.getFrame = function (frameDelta) {
	if (!!this.ControllerAnimation && !!this.ControllerAnimation.Animation) {
		...
		if (index > -1) return this.BaseAnimation.Frames[index];
	} else {
...
Animation.prototype.getFrameIndex = function (id) {
	return this.BaseAnimation.getFrameIndex(id);
};
...
Animation.prototype.renderChildren = function (
	frame,
	startFrame,
	direction,
	zIndex,
	...
	for (var i = 0, length = this.Animations.length; i < length; ++i)
		this.Animations[i].tryRender(
			frame,
			{ StartFrame: startFrame, ZIndex: zIndex, X: x, Y: y },
			direction,
		);
};
...
Animation.prototype.release = function () {
	...
	if (!!this.Trail) this.Trail.release();
...
Animation.prototype.allFramesSet = function (object) {
	for (var i = 0; i < this.BaseAnimation.Frames.length; ++i)
		for (var x in object) this.BaseAnimation.Frames[i][x] = object[x];
};
...
GenericAnimation.prototype.addUserDataToFrame = function (index, data) {
	var frame = this.BaseAnimation.Frames[index];
	if (!!frame) frame.UserData[frame.UserData.length] = data;
};
...
GenericAnimation.prototype.addFrame = function (
	player,
	shadowImage,
	image,
	nbFrames,
...
GenericAnimation.prototype.addRepeatingFrame = function (
	player,
	shadowImage,
	image,
	nbFrames,
	...
	return this.BaseAnimation.addRepeatingFrame.apply(
		this.BaseAnimation,
		arguments,
	);
};
...
GenericAnimation.prototype.addRepeatingTrailFrame = function (
	player,
	image,
	nbFrames,
) {
...
GenericAnimation.prototype.getFrame = function (frameDelta) {
	return this.BaseAnimation.getFrame.apply(this.BaseAnimation, arguments);
};
GenericAnimation.prototype.getNextFrameOffset = function (id) {
	return this.BaseAnimation.getNextFrameOffset.apply(
		this.BaseAnimation,
		arguments,
	);
...
GenericAnimation.prototype.renderWithPlayer = function (
	frame,
	startFrame,
	element,
	direction,
...
GenericAnimation.prototype.tryRender = function (
	frame,
	startFrame,
	element,
	stageX,
	...
	if (!newFrame) {
		...
		if (!!this.CenteredOffset) offsetX = playerWidth * this.CenteredOffset;
		if (!!this.TopOffset) offsetY = this.TopOffset;
	...
	if (this.Direction > 0) {
		...
		if (hasFlag(this.MoveFlags, MOVE_FLAGS.MOVE_TO_PLAYER))
			...
			element.style.right =
				offsetX + this.InitialX + (this.InitialStageX - stageX) + "px";
	} else {
		element.style.right = "";
		if (hasFlag(this.MoveFlags, MOVE_FLAGS.MOVE_TO_PLAYER))
	...
	element.style.bottom =
		offsetY + this.InitialY + (stageY - this.InitialStageY) + "px";
	if (element.style.display != "") element.style.display = "";
	return true;
};
...
var BasicAnimation = function (name, frames, isLooping, direction, bgImg) {
	...
	this.InitialStageY = 0;
	if (!!bgImg) this.createElement(bgImg);
};
...
BasicAnimation.prototype.release = function () {
	if (!!this.Element) utils_.removeFromDOM(this.Element);
};
...
BasicAnimation.prototype.tryRender = function (frame, object, direction) {
	...
	if (!!object.ZIndex) element.style.zIndex = object.ZIndex;
	var delta = 0;
	if (!!this.IsLooping) {
		if (this.InternalFrame > this.BaseAnimation.NbFrames)
			this.InternalFrame = 0;
	...
	if (offsetY != undefined) element.style.bottom = offsetY + "px";
	if (element.style.display != "") element.style.display = "";
	return true;
};
...
var CreateFrameImageLookup = function () {
	...
	FrameImageLookup.prototype.getNbImages = function () {
		return nbImages_;
	};
	FrameImageLookup.prototype.getNbImagesLoading = function () {
		return nbImagesLoading_;
	};
	FrameImageLookup.prototype.getElement = function () {
		return element_;
	};
			...
			images_[src].onload = (function (thisValue) {
				return function () {
					if (!!--thisValue.NbImagesLoading) {
						thisValue.getElement().innerHTML = (
							(100 *
								(thisValue.getNbImages() - thisValue.getNbImagesLoading())) /
							thisValue.getNbImages()
						).toFixed(1);
					} else {
	...
	FrameImageLookup.prototype.getB64 = function (src, isBg) {
		if (!!isBg) return "url('" + this.Data[src] + "')";
		else return this.Data[src];
	};
...
var CreateSpriteLookup = function () {
	...
	/*Image only loaded once*/
	SpriteLookup.prototype.load = function (
		key,
		src,
		left,
		bottom,
	...
	SpriteLookup.prototype.set = function (
		element,
		key,
		setBackgroundImage,
		displayNone,
				...
				if (!isBase64) {
					...
					if (!element.style.backgroundImage)
						element.style.backgroundImage =
							"url(" + imageLookup_.Data[data.Sprite] + ")";
				}
			}
...
var Frame = function (
	...
	this.ShadowImageSrc = !!shadowImage
		? "images/misc/misc/shadow-" + shadowImage + ".png"
		: null;
	...
	this.FlagsToClear.SwingSound = !!flagsToClear
		? flagsToClear.SwingSound || 0
		: 0;
	this.FlagsToClear.HitSound = !!flagsToClear ? flagsToClear.HitSound || 0 : 0;
	this.FlagsToClear.BlockSound = !!flagsToClear
		? flagsToClear.BlockSound || 0
		: 0;
...
Frame.prototype.getEndFrameOffset = function () {
	return this.Frames + this.FrameOffset;
};
Frame.prototype.getImageSrc = function (direction) {
	return this.RightSrc;
};
Frame.prototype.isSettingAirborneFlag = function () {
	return (
		!!this.Jump ||
		hasFlag(this.FlagsToSet.Pose, POSE_FLAGS.AIR_COMBO_1) ||
		hasFlag(this.FlagsToSet.Pose, POSE_FLAGS.AIR_COMBO_2) ||
		hasFlag(this.FlagsToSet.Pose, POSE_FLAGS.AIRBORNE) ||
		hasFlag(this.FlagsToSet.Pose, POSE_FLAGS.AIRBORNE_FB)
	);
};
Frame.prototype.isClearingAirborneFlag = function () {
	return (
		hasFlag(this.FlagsToClear.Pose, POSE_FLAGS.AIR_COMBO_1) ||
		hasFlag(this.FlagsToClear.Pose, POSE_FLAGS.AIR_COMBO_2) ||
		hasFlag(this.FlagsToClear.Pose, POSE_FLAGS.AIRBORNE) ||
		hasFlag(this.FlagsToClear.Pose, POSE_FLAGS.AIRBORNE_FB)
	);
};
Frame.prototype.isClearingAirborne = function () {
	return (
		hasFlag(this.FlagsToClear.Pose, POSE_FLAGS.AIRBORNE) ||
		hasFlag(this.FlagsToClear.Pose, POSE_FLAGS.AIRBORNE_FB)
	);
...
var CreateFrameAdapter = function (frameArray, nbFrames) {
	var set = function () {
		for (
			var i = frameArray.length - (nbFrames + 1);
			i < frameArray.length;
			++i
		) {
...
Projectile.prototype.getHitId = function (frame) {
	return this.Id + "_" + frame;
};
Projectile.prototype.setEnergyToAdd = function (value) {
	this.EnergyToAdd = value;
};
Projectile.prototype.getVxFn = function () {
	return this.vxFn;
};
Projectile.prototype.setVxFn = function (value) {
	this.vxFn = value;
};
Projectile.prototype.getVyFn = function () {
	return this.vyFn;
};
Projectile.prototype.setVyFn = function (value) {
	this.vyFn = value;
};
Projectile.prototype.isFirstFrame = function () {
	return this.T <= 1;
};
...
Projectile.prototype.cancel = function (ignoreOnGoneEvent) {
	...
	this.IsActive = false;
	this.IsDisintegrating = false;
	if (!ignoreOnGoneEvent) this.Owner.onProjectileGoneFn(0, this.Id);
};
...
Projectile.prototype.execute = function (frame, stageX, stageY) {
	if (!!this.IsDisintegrating) this.cancel();
	this.SpeedRate = 1;
	this.StartFrame = frame;
	this.T = 0;
	this.Element.style.display = "none";
	this.Direction = this.Owner.Direction;
	if (this.Direction == -1) this.Element.className = "projectile flipped";
	else this.Element.className = "projectile";
...
Projectile.prototype.getBackX = function () {
	if (this.Direction < 0)
		...
		return (
			STAGE.MAX_STAGEX - (parseInt(this.Element.style.right) + this.ClipBack)
		);
};
...
Projectile.prototype.getFrontX = function () {
	if (this.Direction < 0)
		...
		return (
			STAGE.MAX_STAGEX -
			(parseInt(this.Element.style.right) +
				parseInt(this.Element.style.width) -
				this.ClipFront)
		);
};
Projectile.prototype.getLeftX = function () {
	if (this.Direction > 0) {
		return STAGE.MAX_STAGEX - (this.X + parseInt(this.Element.style.width));
	} else {
		return this.X;
	}
};
Projectile.prototype.getRightX = function () {
	if (this.Direction > 0) {
		return STAGE.MAX_STAGEX - this.X;
	} else {
		return this.X + parseInt(this.Element.style.width);
	}
};
...
Projectile.prototype.getIsActive = function () {
	if (this.getIsDisintegrating()) return false;
...
Projectile.prototype.isVisible = function (stageX, stageY) {
	return (
		this.X < STAGE.MAX_STAGEX && this.X > -100 && this.Y > 0 && this.Y < 5000
	);
};
...
Projectile.prototype.didHitFloor = function (stageY) {
	return (
		!this.IsDisintegrating &&
		this.getBottom() < game_.getMatch().getStage().getGroundY()
	);
};
...
Projectile.prototype.advance = function (frame, stageX, stageY) {
	...
	if (!this.isInHitStop(frame)) {
		...
		if (!this.IsDisintegrating) {
			...
			var dx =
				xSpeed +
				(this.Direction > 0 ? this.StageX - stageX : stageX - this.StageX);
...
Projectile.prototype.setSprite = function (
	newFrame,
	offsetX,
	offsetY,
	stageX,
	...
	if (this.IsDisintegrating) {
		if (this.Direction > 0) {
			this.Element.style.left =
				offsetX + FlipCoord(this.X, parseInt(this.Element.style.width)) + "px";
	...
	if (this.Element.style.display != "") this.Element.style.display = "";
};
...
Projectile.prototype.hitPlayer = function (frame) {
	...
	if (++this.NbHits >= this.MaxHits) this.disintegrate(frame);
};
...
Projectile.prototype.hitProjectile = function (frame, otherProjectile) {
	...
	if (!isSuper || areBothSupers) this.disintegrate(frame);
	if (!isOtherSuper || areBothSupers) otherProjectile.disintegrate(frame);
};
