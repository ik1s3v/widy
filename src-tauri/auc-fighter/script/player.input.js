...
Player.prototype.isKeyDown = function (key) {
	return (
		...
		this.ButtonStates[this.ButtonStates.length - 1].State[key].Value ==
...
Player.prototype.cleanUpButtonStateChanges = function (frame) {
	if (this.ButtonStates.length > CONSTANTS.MAX_KEYSTATES)
		this.ButtonStates = this.ButtonStates.slice(
			this.ButtonStates.length - CONSTANTS.MAX_KEYSTATES,
		);

	var i = 0;
	while (i < this.ButtonStates.length) {
		if (
			frame - this.ButtonStates[i].Frame > CONSTANTS.NBINTERIM_FRAMES &&
...
Player.prototype.injectInput = function (isDown, bit, frame, funcName) {
	if (bit === null && !!funcName) {
		...
		if (!!key) this.onKeyStateChanged(isDown, key, frame);
	}
};

//Simuates pressing keys
Player.prototype.sendInput = function (input) {
	if (!input || !this.getMatch().getAllowInput()) return;
...
Player.prototype.addCurrentButtonState = function (frame) {
	var state = {};
	for (var i in this.ButtonState)
		state[i] = makeButtonState(
			this.ButtonState[i].Value,
			this.ButtonState[i].Frame,
		);
...
Player.prototype.compareButtonSequence = function (frame, move, input) {
	...
	requiredKeys: while (iIndex > -1 && mIndex > -1) {
		moveButtons: for (var b = 0; b < move.ButtonSequence[mIndex].length; ++b) {
			...
			var btn = [];
			if (!!moveItem.Button.length) btn = moveItem.Button;
			else btn.push(moveItem.Button);
			...
			for (var i = 0; i < btn.length; ++i) {
				...
				if (moveItem.State == inputItem.Value) {
					...
					if (
						!!moveItem.MaxNbFrames &&
						frame - inputItem.Frame > moveItem.MaxNbFrames
					) {
						//not ok
						return false;
					}
					...
					if (
						!!moveItem.MinNbFrames &&
						frame - inputItem.Frame < moveItem.MinNbFrames
					) {
						//not ok
						return false;
					}
...
Player.prototype.allowInterupt = function () {
	return (
		this.Flags.Pose.has(POSE_FLAGS.ALLOW_INTERUPT_1) ||
...
Player.prototype.checkForAnimation = function (frame) {
	if (!this.getMatch().getAllowInput()) return;
	if (!!this.CheckedForAnimationFrame == frame) return;
	this.CheckedForAnimationFrame = frame;

	if (
		this.Flags.Player.has(PLAYER_FLAGS.MOBILE) ||
		(this.allowInterupt() && !this.InteruptAnimation)
	) {
		for (var i = 0; i < 2; ++i) {
			...
			while (tmpButtons.length > 0) {
				var move =
					i == 0
						? this.findThrow(tmpButtons, frame)
						: this.findAnimation(tmpButtons, frame);
				if (!!move) {
					...
					//is there no current move, or is the user executing a new move
						...
						if (!this.setInteruptAnimation(move)) {
							if (
								move.BaseAnimation.Name == "turn" ||
								move.BaseAnimation.Name == "crouch turn"
							)
