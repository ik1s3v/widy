...
Player.prototype.isWithinDistanceX = function (x, distance) {
	return (
		(this.Direction == 1
			...
			: Math.abs(this.getRect().Right - x)) < distance
	);
};
Player.prototype.isImgWithinDistanceX = function (x, distance, direction) {
	return (
		Math.abs(x - (direction < 1 ? this.LeftOffset : this.RightOffset)) <
			distance ||
		(x > this.LeftOffset && x < this.RightOffset)
	);
};
...
Player.prototype.getXDir = function () {
	return (this.X - this.LastX) / Math.abs(this.X - this.LastX) || 0;
};
//0 would mean stationary on the Y
Player.prototype.getYDir = function () {
	return (this.Y - this.LastY) / Math.abs(this.Y - this.LastY) || 0;
};

Player.prototype.getLeftX = function (useImageWidth) {
	if (this.Direction > 0) {
		return (
			STAGE.MAX_STAGEX -
			(this.getX() +
				(!!useImageWidth ? this.getBoxWidth() : this.getConstWidth()))
		);
	} else {
		return this.getX();
	}
};
Player.prototype.getRightX = function (useImageWidth) {
	if (this.Direction > 0) {
		return STAGE.MAX_STAGEX - this.getX();
	} else {
		return (
			this.getX() +
			(!!useImageWidth ? this.getBoxWidth() : this.getConstWidth())
...
Player.prototype.getImgFrontX = function (useImageWidth) {
	if (this.Direction > 0) {
		return this.getLeftX(useImageWidth);
	} else {
		return this.getRightX(useImageWidth);
	}
};
Player.prototype.getAbsBackX = function (useImageWidth) {
	if (this.Direction > 0) {
		return this.getRightX(useImageWidth);
	} else {
		return this.getLeftX(useImageWidth);
...
Player.prototype.getBoxTop = function () {
	return this.Y + this.getBoxHeight();
};
Player.prototype.getConstOffsetTop = function () {
	return this.Height + game_.getMatch().getStage().getGroundY();
};
Player.prototype.getOffsetBoxTop = function () {
	return this.Y + this.getBoxHeight() - this.ClipMoveTop;
};
Player.prototype.getBoxBottom = function () {
	return this.Y;
};
Player.prototype.getOffsetBoxBottom = function () {
	return this.Y + this.ClipMoveBottom;
};
...
Player.prototype.getConstHeight = function () {
	return this.Height;
};
Player.prototype.getConstWidth = function () {
	return this.Width;
};
Player.prototype.getConstFrontX = function () {
	return this.getX() + this.getConstWidth();
};
Player.prototype.getFrontX = function () {
	if (this.Direction > 0) {
...
Player.prototype.getBackX = function () {
	if (this.Direction > 1) {
		return this.LeftOffset;
	} else {
		return this.RightOffset;
...
Player.prototype.getBoxWidth = function () {
	return this.SpriteWidth;
};
Player.prototype.getBoxHeight = function () {
	return this.SpriteHeight;
};
Player.prototype.getRect = function (useImageWidth) {
	return {
		Left:
			this.getLeftX(useImageWidth) +
			(this.Direction > 0 ? this.ClipMoveFront : this.ClipMoveBack),
		Right:
			this.getRightX(useImageWidth) -
			(this.Direction > 0 ? this.ClipMoveBack : this.ClipMoveFront),
...
Player.prototype.isFacingProjectile = function (projectile) {
	if (
		...
		this.LeftOffset < projectile.getRightX()
	)
		return true;
	if (
		this.Direction == 1 &&
		projectile.Direction == -1 &&
		this.RightOffset > projectile.getLeftX()
	)
		return true;
...
Player.prototype.isFacingPlayer = function (otherPlayer, ignoreOtherDirection) {
	if (!!ignoreOtherDirection) {
		if (
			...
			this.RightOffset > otherPlayer.LeftOffset
		)
			return true;
		if (
			this.Direction == 1 &&
			otherPlayer.Direction == 1 &&
			this.RightOffset > otherPlayer.RightOffset
		)
			return true;
		...
		if (
			this.Direction == -1 &&
			otherPlayer.Direction == 1 &&
			this.LeftOffset < otherPlayer.RightOffset
		)
			return true;
		if (
			this.Direction == -1 &&
			otherPlayer.Direction == -1 &&
			this.LeftOffset < otherPlayer.LeftOffset
		)
			return true;
	} else {
		if (
...
Player.prototype.alignX = function (deltaX) {
	this.X += deltaX * -this.Direction;
};
Player.prototype.checkGroundY = function () {
	this.setGroundY(this.getStage().getGroundY());
};
Player.prototype.setImageX = function (value) {
	if (this.Direction > 0) {
		this.SpriteElement.style.right = value + "px";
	} else {
		this.SpriteElement.style.left = value + "px";
	}
};
Player.prototype.setImageY = function (value) {
	this.SpriteElement.style.bottom = value + "px";
};
Player.prototype.isCrouching = function () {
	return this.Flags.Pose.has(POSE_FLAGS.CROUCHING);
};
Player.prototype.isOnGround = function () {
	return this.Y == game_.getMatch().getStage().getGroundY();
};
Player.prototype.isMobile = function () {
	return this.Flags.Player.has(PLAYER_FLAGS.MOBILE);
};
Player.prototype.hasAirborneFlag = function () {
	return (
		this.Flags.Pose.has(POSE_FLAGS.AIRBORNE) ||
		this.Flags.Pose.has(POSE_FLAGS.AIRBORNE_FB)
	);
};
Player.prototype.hasAirborneComboFlag = function () {
	return (
		this.Flags.Pose.has(POSE_FLAGS.AIR_COMBO_1) ||
		this.Flags.Pose.has(POSE_FLAGS.AIR_COMBO_2)
	);
};
Player.prototype.isAirborne = function () {
	return (
		this.hasAirborneComboFlag() ||
		this.hasAirborneFlag() ||
		this.Y > game_.getMatch().getStage().getGroundY()
	);
};
Player.prototype.setGroundY = function (groundY) {
	if (!!this.isBeingGrappled()) return;
...
Player.prototype.isDescending = function () {
	return this.LastFrameY > this.ConstY;
};
Player.prototype.isVisible = function () {
	return !this.Flags.Player.has(PLAYER_FLAGS.INVISIBLE);
};
Player.prototype.isTeleporting = function () {
	return (
		!!this.isTeleportingStarting() ||
		!!this.isTeleportingEnding() ||
		this.Flags.Combat.has(COMBAT_FLAGS.TELEPORT)
	);
};
Player.prototype.isTeleportingStarting = function () {
	return (
		!!this.CurrentAnimation &&
		!!this.CurrentAnimation.Animation &&
		hasFlag(
			this.CurrentAnimation.Animation.Flags.Combat,
			COMBAT_FLAGS.TELEPORT_START,
		)
	);
};
Player.prototype.isTeleportingEnding = function () {
	return (
		!!this.CurrentAnimation &&
		!!this.CurrentAnimation.Animation &&
		hasFlag(
			this.CurrentAnimation.Animation.Flags.Combat,
			COMBAT_FLAGS.TELEPORT_END,
		)
	);
};
Player.prototype.jumpedOverAPlayer = function () {
	return this.isAirborne() && this.isDescending() && !!this.MustChangeDirection;
};

Player.prototype.setDirection = function (value) {
	if (value != this.Direction) this.changeDirection();
};
Player.prototype.turnAround = function () {
	this.MustChangeDirection = 1;

	if (!!this.IsBeingThrown) this.MustChangeDirectionQuick = true;
};
...
Player.prototype.changeDirection = function (quick, ignoreSetAnimation) {
	...
	for (var i = 0; i < this.ButtonStates.length; ++i) {
		if (
			...
			this.ButtonStates[i].State[BUTTONS.FORWARD].Frame =
				this.ButtonStates[i].State[BUTTONS.BACK].Frame;
	...
	if (this.ButtonState[BUTTONS.BACK].Value == BUTTON_STATE.PRESSED) {
		...
		this.ButtonState[BUTTONS.FORWARD].Frame =
			this.ButtonState[BUTTONS.BACK].Frame;
...
Player.prototype.moveCircle = function () {
	var x = 0;
	if (this.Direction < 0) x = this.getX();
	else x = STAGE.MAX_STAGEX - this.getX();
	if (!!this.CurrentFrame) {
		var r = this.getRect();
		this.Circle.R = (r.Right - r.Left) / 2;
		this.Circle.RSq = this.Circle.R * this.Circle.R;
	}
	if (this.Direction == -1) this.Circle.RenderX = x + this.Circle.R;
	else this.Circle.RenderX = x - this.Circle.R;
...
Player.prototype.setTeleportTarget = function (flag, nbFrames) {
	...
	if (foe) {
		...
		this.TeleportX = 0;
		switch (flag) {
			case COMBAT_FLAGS.TELEPORT_BEHIND: {
				this.TeleportX = (STAGE.MAX_STAGEX - foe.X) / nbFrames;
				this.Teleport0GapX = "b";
				break;
			}
			case COMBAT_FLAGS.TELEPORT_INFRONT: {
				this.TeleportX =
					(STAGE.MAX_STAGEX - foe.X - foe.OffsetWidth - this.OffsetWidth) /
					nbFrames;
				this.Teleport0GapX = "f";
				break;
			}
			case COMBAT_FLAGS.TELEPORT_MIDDLE: {
				this.TeleportX =
					(STAGE.MAX_STAGEX - foe.X - foe.OffsetWidth - this.OffsetWidth) /
					nbFrames /
					2;
				this.Teleport0GapX = "m";
				break;
			}
			case COMBAT_FLAGS.TELEPORT_BACK: {
				this.TeleportX = (0 - this.X) / nbFrames;
				this.Teleport0GapX = "bw";
				break;
			}

			case COMBAT_FLAGS.TELEPORT: {
				this.TeleportX = this.CurrentFrame.TeleportSpeed;
				this.Teleport0GapX = "t";
				break;
			}
		}
		this.TeleportX /= 2;
		...
		if (this.TeleportFramesLeft <= 0) {
				...
				if (foe) {
					...
					switch (this.Teleport0GapX) {
						case "f": {
							...
							x =
								this.Direction == -1
									? (x = otherRect.Left - this.OffsetWidth * buffer)
									: STAGE.MAX_STAGEX -
										otherRect.Right -
										this.OffsetWidth * buffer;
							break;
						}
						case "b": {
							...
							x =
								this.Direction == -1
									? (x = otherRect.Right)
									: (x = STAGE.MAX_STAGEX - otherRect.Left);
							break;
						}
						case "bw": {
							//back wall
						...
						case "m": {
							...
							x =
								this.Direction == -1
									? otherRect.Left * midBuffer
									: (STAGE.MAX_STAGEX - otherRect.Right) * midBuffer;
							break;
						}
					}
					this.setX(x);
			...
			if (
				...
				!game_
					.getMatch()
					.getCDHelper()
					.isWithinDistanceX(this, foe, CONSTANTS.MIN_TELEPORT_DISTANCE_SQ) ||
				this.Teleport0GapX == "t"
			)
...
Player.prototype.isFacingTarget = function (ignorePendingDirectionChange) {
	...
	else otherRect = this.getMatch().getTeamA().getPlayer(this.Target).getRect();
	...
	if (
		this.Direction == -1 &&
		(!this.MustChangeDirection || !!ignorePendingDirectionChange) &&
...
Player.prototype.targetRearEnemy = function () {
	var target = this.getCDHelper().getRearEnemy(
		this.Team,
		this.Direction,
		this.getRect(),
		this.Target,
...
Player.prototype.warpX = function (amount, autoDir) {
	if (!!autoDir) {
		if (amount > 0) {
			//moving right
			if (this.Direction == -1) amount = Math.abs(amount);
...
Player.prototype.convertX = function (x) {
	if (this.Direction < 0) return STAGE.MAX_STAGEX - this.getBoxWidth() - x;
	else return STAGE.MAX_STAGEX - this.getBoxWidth() - x;
};
...
Player.prototype.slide = function (frame) {
	if (!!this.FrameFreeze) return;
	if (
		this.T >= CONSTANTS.HALF_PI ||
		!this.IsSliding ||
	...
	if (this.isOnGround()) this.moveX(deltaX);
	else this.SlideFactor *= 0.5;
};
...
Player.prototype.mustHoldAirborne = function () {
	return (
		this.Flags.Pose.has(POSE_FLAGS.FORCE_HOLD_AIRBORNE_XY) ||
		this.Flags.Pose.has(POSE_FLAGS.HOLD_AIRBORNE)
	);
};
...
Player.prototype.advanceJump = function (ignoreYCheck) {
	...
	var y =
		this.Y0 +
		(this.JumpVelocityY * this.JumpT -
			CONSTANTS.HALF_G * this.Mass * (this.JumpT * this.JumpT)) *
			CONSTANTS.Y_DAMPING;

	if (this.mustHoldAirborne()) {
		this.holdJump();
		if (this.Flags.Pose.has(POSE_FLAGS.FORCE_HOLD_AIRBORNE_XY)) return true;
	}
	...
	if (!!this.vxFn) dx = this.vxFn(dx, this.JumpT);
	if (!!this.vyFn) dy = this.vyFn(dy, this.JumpT);
	...
	this.peakY = this.peakY < this.Y ? this.Y : this.peakY;

	//did the player just hit the ground
	if (
		this.JumpT > this.JumpSpeed &&
		this.getY() <= game_.getMatch().getStage().getGroundY() &&
		!this.Flags.Player.has(PLAYER_FLAGS.HUMAN_PROJECTILE)
	) {
		this.IsLosing = false;
...
Player.prototype.performJump = function (
	vx,
	vy,
	vxFn,
	vyFn,
...
Player.prototype.holdJump = function () {
	if (
		this.Flags.Pose.has(POSE_FLAGS.FORCE_HOLD_AIRBORNE_XY) ||
		(!!this.JumpT && !!this.CanHoldAirborne)
	) {
		this.JumpT -= this.JumpSpeed;
	}
};

Player.prototype.setVyFn = function (fn) {
	this.vyFn = this.vyFn || fn;
};
Player.prototype.setVxFn = function (fn) {
	this.vxFn = this.vxFn || fn;
};

Player.prototype.resetVyFn = function (fn) {
	this.vyFn = function (b) {
		return b;
	};
};
Player.prototype.resetVxFn = function (fn) {
	this.vxFn = function (b) {
		return b;
	};
};

Player.prototype.clearVyFn = function () {
	this.vyFn = null;
};
Player.prototype.clearVxFn = function () {
	this.vxFn = null;
};
