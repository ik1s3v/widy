var CreateAnimationTrail = function (animations, zIndex, delay) {
	...
	AnimationTrail.prototype.getNextId = function () {
		return AnimationTrail.prototype.id++;
	};
	...
	AnimationTrail.prototype.add = function (
		animation,
		followElement,
		folder,
		player,
		...
		div.style.backgroundImage =
			"url(images/misc/" + folder + "/trail-sprites.png)";
		div.appendChild(img);

		this.Trail[this.Trail.length] = {
			StartFrame: 0,
			Element: div,
			Animation: animation,
			FrameIndex: 0,
		...
		if (!!this.FollowElement) {
			...
			if (container.children.length == 0) container.appendChild(div);
			else container.insertBefore(div, container.children[0]);
		}
	};
	...
	AnimationTrail.prototype.getCurrentFrame = function (index) {
		return this.Trail[index].Animation.BaseAnimation.Frames[
			this.Trail[index].FrameIndex
		];
	};
	/**/
	AnimationTrail.prototype.frameMove = function (
		frame,
		index,
		direction,
		stageX,
		...
		if (this.Enabled) {
			...
			if (!!this.Player) {
				...
				if (right == "") left = rect.Left;
				else right = STAGE.MAX_STAGEX - rect.Right;
			}

			if (right === "") left = left + "px";
			else right = right + "px";
	...
	AnimationTrail.prototype.applyStageOffset = function (
		stageDiffX,
		stageDiffY,
	) {
		if (!!stageDiffX) {
			for (
				var trailIndex = 0, nbTrails = this.Trail.length;
				trailIndex < nbTrails;
				trailIndex++
			) {
				for (
					var frameIndex = 0,
						nbFrames =
							this.Trail[trailIndex].Animation.BaseAnimation.Frames.length;
					frameIndex < nbFrames;
					++frameIndex
				) {
					var frame =
						this.Trail[trailIndex].Animation.BaseAnimation.Frames[frameIndex];
					var coords = frame.UserData;
					for (
						var coordIndex = 0, nbCoords = coords.length;
						coordIndex < nbCoords;
						coordIndex++
					) {
		...
		if (this.Enabled) {
			...
			for (var i = 0, length = this.Trail.length; i < length; i++) {
				if (frame > this.Trail[i].StartFrame) {
					...
					if (!!coords) {
						this.Trail[i].Element.style.left = !!coords.Left
							? coords.DeltaX + parseInt(coords.Left) + "px"
							: "";
						this.Trail[i].Element.style.right = !!coords.Right
							? coords.DeltaX + parseInt(coords.Right) + "px"
							: "";
						this.Trail[i].Element.style.bottom = !!coords.Bottom
							? coords.DeltaY + stageOffsetY + parseInt(coords.Bottom) + "px"
							: "";
						this.Trail[i].Element.style.top = !!coords.Top
							? coords.DeltaY + parseInt(coords.Top) + "px"
							: "";
						if (!!coords.Flip)
							AutoApplyFlip(
								this.Trail[i].Element,
								this.Direction == 1 ? coords.Flip : !coords.Flip,
							);
					}
				}
				if (frame > this.Trail[i].StartFrame) {
