var Player = function (
	...
	this.NameImageSrc =
		nameImageSrc ||
		"images/misc/misc/" + this.Name.toLowerCase() + "-name-1.png";
	this.PortriatImageSrc =
		portriatImageSrc ||
		"images/misc/misc/" + this.Folder.toLowerCase() + "-r-portriat-1.png";
...
Player.prototype.loadAssets = function (name, folder, loadProjectiles) {
	stuffLoader_.queue(
		(this.Name || name).toLowerCase() + ".js",
		RESOURCE_TYPES.BASE64AUDIO,
	);
	//stuffLoader_.queue("script/" + (this.Name || name).toLowerCase() + "-ai.js",RESOURCE_TYPES.SCRIPT);
	stuffLoader_.queue(
		"images/misc/" + (this.Folder || folder).toLowerCase() + "/sprites.png",
...
Player.prototype.enableAI = function (createAiFn) {
	...
	this.Ai.enableAI(
		...
		createAiFn ||
			window[
				"Create" + this.Name[0].toUpperCase() + this.Name.substring(1) + "AI"
			],
	);
	if (!!this.getMatch()) this.getMatch().checkAIMatch();
};
...
Player.prototype.getEnergyLevel = function () {
	...
	if (value >= ENERGYBAR.MAX_LEVEL2) return ENERGYBAR.LEVELMAXED;
	else if (value >= ENERGYBAR.MAX_LEVEL1) return ENERGYBAR.LEVEL2;
	else if (value >= ENERGYBAR.MAX_LEVEL0) return ENERGYBAR.LEVEL1;
	else return ENERGYBAR.LEVEL0;
};
...
Player.prototype.resetCombo = function () {
	if (!!this.onDecComboRefCountFn) this.onDecComboRefCountFn();
	this.ComboCount = 0;
...
Player.prototype.incCombo = function (attackId) {
	if (!this.getCurrentComboCountFn()) this.onIncComboRefCountFn();
	this.onIncComboFn();
	++this.ComboCount;
...
Player.prototype.reset = function (ignoreDirection) {
	...
	if (!ignoreDirection) this.Direction = 1;
	this.Health = 100;
...
Player.prototype.createElement = function (x, y, parentElement) {
	...
	var createElement = function (tagName, className, attrib, value, parent) {
		...
		if (!!attrib) i.style[attrib] = value;
	...
	for (var i = 0; i < CONSTANTS.MAX_EXTRA_IMAGES; ++i)
		this.FrontHitReportImages[this.FrontHitReportImages.length] =
			createElement.call(
				this,
				"div",
				"front-hit-report",
	...
	for (var i = 0; i < CONSTANTS.MAX_EXTRA_IMAGES; ++i)
		this.RearHitReportImages[this.RearHitReportImages.length] =
			createElement.call(
				this,
				"div",
				"rear-hit-report",
	...
	this.Shadow = createElement.call(
		this,
		"div",
		"shadow",
		"",
...
Player.prototype.moveToBack = function (dontMoveOtherPlayers) {
	this.setZOrder(1);
	if (!dontMoveOtherPlayers) this.moveOtherPlayersToFrontFn();
};

Player.prototype.moveToFront = function (dontMoveOtherPlayers) {
	if (this.getIsExecutingSuperMove()) this.setZOrder(16);
	else this.setZOrder(14);

	if (!dontMoveOtherPlayers) this.moveOtherPlayersToBackFn();
};
...
Player.prototype.isProjectileInUse = function (move) {
	if (
		hasFlag(move.Flags.Combat, COMBAT_FLAGS.MULTI_PROJECTILE) &&
		move.ProjectileId !== null
	) {
		return this.Projectiles[move.ProjectileId].IsActive;
	} else {
		return (
			!!this.HasActiveProjectiles &&
			hasFlag(move.Flags.Combat, COMBAT_FLAGS.PROJECTILE_ACTIVE)
		);
	}
};
...
Player.prototype.getAttackDirection = function (attackDirection) {
	...
	if (this.Direction > 0 && attackDirection > 0) direction = -1;
	else if (this.Direction < 0 && attackDirection < 0) direction = 1;
	else if (this.Direction > 0 && attackDirection < 0) direction = 1;
	else if (this.Direction < 0 && attackDirection > 0) direction = -1;
	return direction;
};
...
Player.prototype.otherAnimationFrameMove = function (frame, stageX, stageY) {
	...
	if (this.isDizzy()) {
		...
		if (
			item.Animation.renderWithPlayer(
				frame,
				item.StartFrame,
				item.Element,
				this.Direction,
...
Player.prototype.reverseFrame = function (frame) {
	if (!!this.CurrentFrame) this.CurrentAnimation.StartFrame += 2;
};
...
Player.prototype.holdFrame = function (frame) {
	...
	if (!!this.CurrentFrame) ++this.CurrentAnimation.StartFrame;
	if (this.isAirborne()) this.getStage().holdFrame();
};
//Prevents the animation from continuing for one frame
...
Player.prototype.forceNextFrame = function (frame) {
	if (!!this.CurrentFrame)
		this.CurrentAnimation.StartFrame =
			frame - this.CurrentFrame.getEndFrameOffset();
};
...
Player.prototype.handleAI = function (frame) {
	if (this.Ai.isRunning() && !this.isDead()) this.Ai.frameMove(frame);
};
...
Player.prototype.onFrameMove = function (frame, stageX, stageY) {
	if (!this.IsPaused) {
		...
		if (!!this.TeleportFramesLeft) this.advanceTeleportation(frame);
		this.decreaseDizziness(frame);
		this.sendAttackAlerts(frame);
		this.checkForInterupt(frame);
		this.checkGroundY();
		this.frameMove(frame, stageX, stageY);
		if (!!this.IsInAttackFrame) this.handleAttack(frame, this.CurrentFrame);
		else if (!this.checkFloater(frame)) this.checkVulnerable(frame);
		if (!!this.GrappledPlayer)
			this.handleGrapple(
				this.CurrentAnimation.FrameIndex - 1,
				frame,
				stageX,
				stageY,
			);
		...
		if (!this.HandledDead && this.isDead()) this.forceTeamLose(frame);
	} else {
...
Player.prototype.frameMoveTrail = function (frame, stageX, stageY) {
	var index = this.CurrentAnimation.Animation.getFrameIndex(
		this.CurrentFrame.ID,
	);
	this.CurrentAnimation.Animation.Trail.frameMove(
		frame,
		index,
		this.Direction,
		stageX,
...
Player.prototype.frameMove = function (frame, stageX, stageY) {
	...
	this.LastX = this.X;
	this.checkDirection();
	if (this.IsSliding) this.slide(frame);
	...
	if (!!this.CurrentAnimation && !!this.CurrentAnimation.Animation) {
		...
		if (
			!!currentFrame ||
			(!!this.CurrentFrame &&
				hasFlag(this.CurrentFrame.FlagsToSet.Player, PLAYER_FLAGS.HOLD_FRAME))
		) {
			...
			if (
				...
				hasFlag(currentFrame.FlagsToSet.Combat, COMBAT_FLAGS.TELEPORT)
			) {
				this.reverseSprite();
			} else if (
				!!this.MustChangeDirection &&
				hasFlag(
					...
					PLAYER_FLAGS.ALLOW_CHANGE_DIRECTION,
				)
			) {
			...
			if (!!currentFrame) {
				if (currentFrame.isSettingAirborneFlag()) {
					if (
						!this.isAirborne() ||
						hasFlag(
							currentFrame.FlagsToSet.Pose,
							POSE_FLAGS.FORCE_START_AIRBORNE,
						) ||
						!!currentFrame.Jump
					) {
						var direction = 1;
						if (
							hasFlag(
								currentFrame.FlagsToSet.Player,
								PLAYER_FLAGS.USE_ATTACK_DIRECTION,
							)
						)
							direction = this.CurrentAnimation.AttackDirection;

						var fx = !!currentFrame.Jump
							? currentFrame.Jump.Fx
							: this.CurrentAnimation.Vx;
						var fy = !!currentFrame.Jump
							? currentFrame.Jump.Fy
							: this.CurrentAnimation.Vy;

						this.performJump(
							direction * fx,
							fy,
							this.CurrentAnimation.Animation.getXModifier(),
							this.CurrentAnimation.Animation.getYModifier(),
							this.CurrentAnimation.Animation.NbFramesAirborneAdvance,
							this.CurrentAnimation.Animation.AirborneStartDeltaY,
							this.CurrentAnimation.Animation.UseJumpSpeed,
							this.CurrentAnimation.Animation.UseJumpT,
						);
					} else {
					...
					if (hasFlag(currentFrame.FlagsToSet.Pose, POSE_FLAGS.AIR_COMBO_1)) {
						...
						hasFlag(currentFrame.FlagsToSet.Pose, POSE_FLAGS.AIR_COMBO_2)
					) {
						this.clearAirborneFlags();
			...
			//Does the move require the key to be held?
				...
				!!currentFrame &&
				hasFlag(currentFrame.FlagsToSet.Player, PLAYER_FLAGS.MUST_HOLD_KEY)
			) {
...
Player.prototype.release = function () {
	this.pause();
	var parentElement =
		parentElement || window.document.getElementById("pnlStage");
...
Player.prototype.pause = function () {
	if (this.isDizzy()) soundManager_.pause("audio/misc/dizzy.zzz");
};

Player.prototype.resume = function () {
	if (this.isDizzy()) soundManager_.resume("audio/misc/dizzy.zzz");
};
...
Player.prototype.setButtons = function (user) {
	...
	this.ButtonStates = [];
	this.ButtonState = {};
	this.ButtonState[this.Buttons[user.Left].Bit] = {
		Value: BUTTON_STATE.NONE,
		Frame: 0,
	};
	this.ButtonState[this.Buttons[user.Right].Bit] = {
		Value: BUTTON_STATE.NONE,
		Frame: 0,
	};
	this.ButtonState[this.Buttons[user.Up].Bit] = {
		Value: BUTTON_STATE.NONE,
		Frame: 0,
	};
	this.ButtonState[this.Buttons[user.Down].Bit] = {
		Value: BUTTON_STATE.NONE,
		Frame: 0,
	};
	this.ButtonState[this.Buttons[user.P1].Bit] = {
		Value: BUTTON_STATE.NONE,
		Frame: 0,
	};
	this.ButtonState[this.Buttons[user.P2].Bit] = {
		Value: BUTTON_STATE.NONE,
		Frame: 0,
	};
	this.ButtonState[this.Buttons[user.P3].Bit] = {
		Value: BUTTON_STATE.NONE,
		Frame: 0,
	};
	this.ButtonState[this.Buttons[user.K1].Bit] = {
		Value: BUTTON_STATE.NONE,
		Frame: 0,
	};
	this.ButtonState[this.Buttons[user.K2].Bit] = {
		Value: BUTTON_STATE.NONE,
		Frame: 0,
	};
	this.ButtonState[this.Buttons[user.K3].Bit] = {
		Value: BUTTON_STATE.NONE,
		Frame: 0,
	};
	this.ButtonState[this.Buttons[user.Turn].Bit] = {
		Value: BUTTON_STATE.NONE,
		Frame: 0,
	};
