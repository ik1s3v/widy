...
Player.prototype.renderDebugInfo = function () {
    ...
    */
	...
	if (
		!!this.Flags.Combat.has(COMBAT_FLAGS.ATTACK) &&
		this.CurrentFrame.HitPoints.length > 0
	) {
...
Player.prototype.createDebugElements = function (parentElement) {
    ...
    */
	...
	for (var i = 0; i < maxDebugHits; ++i) {
		...
		(parentElement || window.document.getElementById("pnlStage")).appendChild(
			this.DebugHit[i],
		);
	}
...
Player.prototype.createKeysElement = function () {
	...
	if (this.Team == 1) {
		window.document
			.getElementById("pnlTeam1Keys")
			.appendChild(this.DebKeysElement);
	} else {
		window.document
			.getElementById("pnlTeam2Keys")
			.appendChild(this.DebKeysElement);
	}
...
Player.prototype.debugShowDirsHelper = function (bit) {
	var retVal = "";
	if (hasFlag(bit, 1) && hasFlag(bit, 4))
		retVal +=
			this.Direction == 1
				? "<img src='images/misc/buttons/nw.png' />"
				: "<img src='images/misc/buttons/ne.png' />";
	if (hasFlag(bit, 1) && hasFlag(bit, 8))
		retVal +=
			this.Direction == 1
				? "<img src='images/misc/buttons/sw.png' />"
				: "<img src='images/misc/buttons/se.png' />";
	if (hasFlag(bit, 2) && hasFlag(bit, 8))
		retVal +=
			this.Direction == 1
				? "<img src='images/misc/buttons/se.png' />"
				: "<img src='images/misc/buttons/sw.png' />";
	if (hasFlag(bit, 2) && hasFlag(bit, 4))
		retVal +=
			this.Direction == 1
				? "<img src='images/misc/buttons/ne.png' />"
				: "<img src='images/misc/buttons/nw.png' />";

	if (!hasFlag(bit, 4) && !hasFlag(bit, 8) && hasFlag(bit, 1))
		retVal +=
			this.Direction == 1
				? "<img src='images/misc/buttons/w.png' />"
				: "<img src='images/misc/buttons/e.png' />";
	if (!hasFlag(bit, 4) && !hasFlag(bit, 8) && hasFlag(bit, 2))
		retVal +=
			this.Direction == 1
				? "<img src='images/misc/buttons/e.png' />"
				: "<img src='images/misc/buttons/w.png' />";

	if (!hasFlag(bit, 1) && !hasFlag(bit, 2) && hasFlag(bit, 4))
		retVal += "<img src='images/misc/buttons/n.png' />";
	if (!hasFlag(bit, 1) && !hasFlag(bit, 2) && hasFlag(bit, 8))
		retVal += "<img src='images/misc/buttons/s.png' />";
