...
var CreateWebAudioManager = function () {
	...
	WebAudioManager.prototype.isLoading = function () {
		return nbLoading_ != maxNbLoading_;
	};
	...
	WebAudioManager.prototype.loadBase64 = function (
		path,
		nbChannels,
		defaultVolume,
		base64Data,
		...
		if (!items_[path]) {
			...
			try {
				...
				for (var i = 0; i < nbChannels; ++i) {
					this.onStartLoading();
					var byteArray = Base64Binary.decodeArrayBuffer(
						base64Data.replace("data:audio/ogg;base64,", ""),
					);
					context_.decodeAudioData(
						byteArray,
						this.setBuffer(path, this),
						this.onError(path, this),
					);
				}
			} catch (err) {
				items_[path] = null;
	...
	WebAudioManager.prototype.play = function (path) {
		...
		if (!!items_[path]) {
			...
			if (!!buffer && !items_[path].IsPlaying) {
				...
				if (items_[path].Loops) items_[path].IsPlaying = true;
			}
		}
	};
	...
	WebAudioManager.prototype.pause = function (path) {
		if (!!items_[path]) {
			...
			if (!!buffer) {
				...
				if (!!source) {
					items_[path].StartOffset +=
						context_.currentTime - items_[path].StartedAt;
					source.disconnect();
					source.stop();
				}
