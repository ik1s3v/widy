function GetDebugInstance(game) {
	...
	var setupMatch = function (t1, t2, stage, callback) {
		$$init();
		game_.end();
		...
		for (var a = 0; a < t1.length; ++a) {
			for (var b = 0; b < t2.length; ++b) {
				if (t1[a].A === t2[b].A) {
					if (!!t1[a].B) t2[b].B = false;
					if (!!t2[b].B) t1[a].B = false;
					if (!t1[a].B && !t2[b].B) t2[b].B = true;
				}
			}
		}

		/*TEAM 1*/
		if (!!t1[0]) user1_.resetChar(t1[0].A, t1[0].B, t1[0].C);
		else user1_.resetChar(CHARACTERS.KEN);
		...
		/*TEAM 2*/
		if (!!t1[1] && !!t1[1].D) {
			if (!!t2[0]) user3_.resetChar(t2[0].A, t2[0].B, t2[0].C);
			else user3_.resetChar(CHARACTERS.RYU);

			teamB.push(2);
		} else {
			if (!!t2[0]) user2_.resetChar(t2[0].A, t2[0].B, t2[0].C);
			else user2_.resetChar(CHARACTERS.RYU);
		}
		...
		console.log(MATCH_STATES.PRACTICE, teamA, teamB, stages_[stage || "guy"]);
		game_.startMatch(
			MATCH_STATES.PRACTICE,
			teamA,
			teamB,
			stages_[stage || "guy"],
			callback,
	...
	//test player battle
	Debug.prototype.startMatch = function (t1, t2, stage, callback) {
		this.setPracticeMode(false);
		__noDamage = false;
		setupMatch(null, null, stage, callback);
	};

	Debug.prototype.p1 = function () {
		if (!!game_.getMatch()) return game_.getMatch().getTeamA().getPlayer(0);
		return null;
	};

	Debug.prototype.p2 = function () {
		if (!!game_.getMatch()) return game_.getMatch().getTeamB().getPlayer(0);
		return null;
	};
	...
	Debug.prototype.p1Execute = function (input) {
		if (!!game_.getMatch()) {
			if (!this.p1().Ai.isRunning()) this.p1().enableAI();
	...
	Debug.prototype.p2Execute = function (input) {
		if (!!game_.getMatch()) {
			if (!this.p2().Ai.isRunning()) this.p2().enableAI();
	...
	Debug.prototype.setFrameData = function () {
		...
		if (!!+x || x === 0) debug_.p1().CurrentFrame.ImageOffsetX = +x;
		if (!!+y || y === 0) debug_.p1().CurrentFrame.ImageOffsetY = +y;
		if (!!s)
			debug_.p1().CurrentFrame.ShadowImageSrc =
				"images/misc/misc/shadow-" + s + ".png";
		if (!!+sx || sx === 0) debug_.p1().CurrentFrame.ShadowOffset.X = +sx;
		if (!!+sy || sy === 0) debug_.p1().CurrentFrame.ShadowOffset.Y = +sy;
	};
	...
	Debug.prototype.injectPlayer = function (playerId, team, isAlternate) {
		...
		if (!!user.getName()) {
			...
			stuffLoader_.queue(
				"script/player-" + name + ".js",
				RESOURCE_TYPES.SCRIPT,
			);
			stuffLoader_.queue(
				"script/player-" + folder + "-spritedata.js",
				RESOURCE_TYPES.SCRIPT,
			);
			...
			var onDone = (function (user) {
				return function () {
					...
					else game_.getMatch().getTeamB().addPlayer(player, true, true);
				};
			})(user);
