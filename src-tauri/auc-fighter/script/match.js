function _c2(a, b) {
	return String.prototype.concat(a, b);
}
function _c3(a, b, c) {
	return String.prototype.concat(String.prototype.concat(a, b), c);
}
function _c4(a, b, c, d) {
	return String.prototype.concat(
		String.prototype.concat(String.prototype.concat(a, b), c),
		d,
	);
}
...
var CreateMatch = function (team1, team2, stage) {
	...
	Match.prototype.checkAIMatch = function () {
		isAIMatch_ =
			(teamB_.getPlayers().length == 0 ||
				teamA_.getPlayers().every(function (player) {
					return player.Ai.isRunning();
				})) &&
			(teamB_.getPlayers().length == 0 ||
				teamB_.getPlayers().every(function (player) {
					return player.Ai.isRunning();
				}));
	};
	Match.prototype.setState = function (state) {
		state_ = state || MATCH_STATES.NONE;
	};
	Match.prototype.clearState = function () {
		state_ = 0;
	};
	Match.prototype.isInPracticeMode = function () {
		return hasFlag(state_, MATCH_STATES.PRACTICE);
	};
	Match.prototype.forceQuit = function (reason) {
		forceQuit_ = true;
		forceQuitReason_ = reason;
	};
	Match.prototype.mustQuit = function () {
		return forceQuit_;
	};
	Match.prototype.getQuitReason = function () {
		return forceQuitReason_;
	};
	Match.prototype.incAirborne = function () {
		++nbAirborne_;
	};
	Match.prototype.getNbAirborne = function () {
		return nbAirborne_;
	};
	Match.prototype.decAirborne = function () {
		nbAirborne_ = Math.max(nbAirborne_ - 1, 0);
		if (!nbAirborne_) stage_.requestScrollY(false, 0, true);
	};
	Match.prototype.getStage = function () {
		return stage_;
	};
	Match.prototype.getCDHelper = function () {
		return cdHelper_;
	};
	...
	Match.prototype.getDefeatedTeam = function () {
		return defeatedTeam_;
	};
	Match.prototype.setDefeatedTeam = function (value) {
		defeatedTeam_ = value;
	};
	Match.prototype.setRoundOver = function (value) {
		isRoundOver_ = value;
	};
	Match.prototype.getGotoNewRoundFrame = function () {
		return gotoNewRoundFrame_;
	};
	Match.prototype.setGotoNewRoundFrame = function (value) {
		gotoNewRoundFrame_ = value;
	};
	Match.prototype.getPlayerCount = function () {
		return playerCount_;
	};
	Match.prototype.setPlayerCount = function (value) {
		playerCount_ = value;
	};
	Match.prototype.getHitSystem = function () {
		return actionSystem_;
	};
	Match.prototype.isSuperMoveActive = function () {
		return isSuperMoveActive_;
	};
	Match.prototype.setSuperMoveActive = function (value) {
		isSuperMoveActive_ = value;
		if (!!value) {
			actionSystem_.pause();
		} else {
			actionSystem_.resume();
		}
	};
	Match.prototype.getDimBackgroundElement = function () {
		return dimBackground_;
	};
	Match.prototype.getRound = function () {
		return round_;
	};
	Match.prototype.setRound = function (value) {
		round_ = value;
	};
	Match.prototype.getAllowInput = function () {
		return allowInput_;
	};
	Match.prototype.setAllowInput = function (value) {
		allowInput_ = value;
	};
	...
	Match.prototype.playerCount = function () {
		if (this.getPlayerCount())
			this.setPlayerCount(
				teamA_.getPlayers().length + teamB_.getPlayers().length,
			);

		return this.getPlayerCount();
	};
	Match.prototype.getGame = function () {
		return game_;
	};
	...
	Match.prototype.changeHealth = function (team, changeAmount) {
		switch (team) {
			case CONSTANTS.TEAM1: {
				teamA_.getHealthbar().change(changeAmount);
				break;
			}
			case CONSTANTS.TEAM2: {
				teamB_.getHealthbar().change(changeAmount);
				break;
			}
		}
	};
	...
	Match.prototype.changeEnergy = function (team, changeAmount) {
		switch (team) {
			case CONSTANTS.TEAM1: {
				teamA_.getEnergybar().change(changeAmount);
				break;
			}
			case CONSTANTS.TEAM2: {
				teamB_.getEnergybar().change(changeAmount);
				break;
			}
		}
	};
	...
	Match.prototype.getHealth = function (team) {
		switch (team) {
			case CONSTANTS.TEAM1: {
				return teamA_.getHealthbar().getAmount();
			}
			case CONSTANTS.TEAM2: {
				return teamB_.getHealthbar().getAmount();
			}
		}
	};
	...
	Match.prototype.getEnergy = function (team) {
		switch (team) {
			case CONSTANTS.TEAM1: {
				return teamA_.getEnergybar().getAmount();
			}
			case CONSTANTS.TEAM2: {
				return teamB_.getEnergybar().getAmount();
			}
		}
	};
	...
	Match.prototype.onCanEndRound = function (teamThatLost) {
		...
		switch (teamThatLost) {
			case CONSTANTS.TEAM1: {
				for (var i = 0; i < teamA_.getPlayers().length; ++i)
					if (!teamA_.getPlayer(i).CanEndRound) return;
				game_.resetSpeed();
				break;
			}
			case CONSTANTS.TEAM2: {
				for (var i = 0; i < teamB_.getPlayers().length; ++i)
					if (!teamB_.getPlayer(i).CanEndRound) return;
				game_.resetSpeed();
				break;
			}
		}
	...
	Match.prototype.canEndRound = function () {
		for (var i = 0; i < teamA_.getPlayers().length; ++i)
			if (!teamA_.getPlayer(i).CanEndRound) return false;
		for (var i = 0; i < teamB_.getPlayers().length; ++i)
			if (!teamB_.getPlayer(i).CanEndRound) return false;
	...
	Match.prototype.reset = function () {
		if (this.getGotoNewRoundFrame() != CONSTANTS.NO_FRAME) {
			...
			if (!!teamA_.getPlayer(0)) teamA_.getPlayer(0).setX(STAGE.START_X);
			if (!!teamB_.getPlayer(0)) teamB_.getPlayer(0).setX(STAGE.START_X);
	...
	Match.prototype.resume = function () {
		if (!!startedTheme_) stage_.resume();
		teamA_.resume();
		teamB_.resume();
	};
	...
	Match.prototype.setupPlayer = function (player, team) {
		var moveStageX = function (thisValue, otherTeam) {
			return function (amount, dontOverrideSign) {
				for (var i = 0; i < otherTeam.getPlayers().length; ++i) {
					amount = thisValue
						.getStage()
						.scrollX(
							amount,
							this,
							otherTeam.getPlayer(i),
							thisValue,
							dontOverrideSign,
						);
				}
		...
		var fixX = function (thisValue, otherTeam) {
			return function (amount) {
				thisValue.getCDHelper().moveOtherPlayers(this);
				return 0;
			};
		};
		var moveX = function (thisValue, otherTeam) {
			return function (amount) {
				amount = thisValue.getStage().scrollX(amount, this, null, thisValue);
				thisValue.getCDHelper().moveX(amount, this, false, true);
				return 0;
			};
		};
		var moveY = function (thisValue, otherTeam) {
			return function (amount) {
				amount = thisValue.getCDHelper().moveY(amount, this);
				return 0;
			};
		};
		var moveToBack = function (thisValue, otherTeam) {
			return function () {
				for (var i = 0; i < otherTeam.getPlayers().length; ++i) {
					otherTeam.getPlayer(i).moveToBack(true);
				}
			};
		};
		var moveToFront = function (thisValue, otherTeam) {
			return function () {
				for (var i = 0; i < otherTeam.getPlayers().length; ++i) {
					otherTeam.getPlayer(i).moveToFront(true);
				}
			};
		};
		var attackPending = function (thisValue, otherTeam) {
			return function (frame, x, y, isSuperMove) {
				for (var i = 0; i < otherTeam.getPlayers().length; ++i) {
					otherTeam
						.getPlayer(i)
						.onEnemyAttackPending(frame, x, y, this, isSuperMove);
				}
			};
		};
		var projectilePending = function (thisValue, otherTeam) {
			return function (frame, x, y, isSuperMove) {
				for (var i = 0; i < otherTeam.getPlayers().length; ++i) {
					otherTeam
						.getPlayer(i)
						.onEnemyProjectilePending(frame, x, y, this, isSuperMove);
				}
			};
		};
		var projectileMoved = function (thisValue, otherTeam) {
			return function (frame, id, x, y, projectile, isSuperMove) {
				for (var i = 0; i < otherTeam.getPlayers().length; ++i) {
					otherTeam
						.getPlayer(i)
						.onEnemyProjectileMoved(frame, id, x, y, projectile, isSuperMove);
					otherTeam
						.getPlayer(i)
						.setAllowBlockFromProjectile(
							thisValue.getGame().getCurrentFrame(),
							true,
							id,
							x,
							y,
		...
		var projectileGone = function (thisValue, otherTeam) {
			return function (frame, id) {
				for (var i = 0; i < otherTeam.getPlayers().length; ++i) {
					otherTeam.getPlayer(i).onEnemyProjectileGone(frame, id);
					otherTeam
						.getPlayer(i)
						.setAllowBlockFromProjectile(
							thisValue.getGame().getCurrentFrame(),
		...
		var startAttack = function (thisValue, otherTeam) {
			return function (id, hitPoints) {
				for (var i = 0; i < otherTeam.getPlayers().length; ++i) {
					otherTeam
						.getPlayer(i)
						.allowBlock(
		...
		var endAttack = function (thisValue, otherTeam) {
			return function (id) {
				for (var i = 0; i < otherTeam.getPlayers().length; ++i) {
					this.Flags.Combat.remove(COMBAT_FLAGS.CAN_BE_BLOCKED);
					otherTeam
						.getPlayer(i)
						.removeBlock(
							id,
		...
		var startAirAttack = function (thisValue, otherTeam) {
			return function (id, hitPoints) {
				for (var i = 0; i < otherTeam.getPlayers().length; ++i) {
					otherTeam
						.getPlayer(i)
		...
		var endAirAttack = function (thisValue, otherTeam) {
			return function (id) {
				for (var i = 0; i < otherTeam.getPlayers().length; ++i) {
					this.Flags.Combat.remove(COMBAT_FLAGS.CAN_BE_AIR_BLOCKED);
					otherTeam
						.getPlayer(i)
						.removeAirBlock(
							id,
		...
		var attack = function (thisValue, otherTeam) {
			return function (
				hitStop,
				hitID,
				attackID,
		...
		var projectileAttack = function (thisValue, otherTeam) {
			return function (frame, projectile) {
				for (var i = 0; i < otherTeam.getPlayers().length; ++i) {
					thisValue
						.getCDHelper()
						.tryProjectileAttack(
		...
		var changeHealth = function (thisValue) {
			return function (amount) {
				thisValue.changeHealth(this.Team, amount);
			};
		};
		var getHealth = function (thisValue) {
			return function () {
				return thisValue.getHealth(this.Team);
			};
		};
		var changeEnergy = function (thisValue) {
			return function (amount) {
				thisValue.changeEnergy(this.Team, amount);
			};
		};
		var getEnergy = function (thisValue) {
			return function () {
				return thisValue.getEnergy(this.Team);
			};
		};
		var incCombo = function (thisValue, team) {
			return function () {
				return team.incCombo();
			};
		};
		var incComboRefCount = function (thisValue, team) {
			return function () {
				return team.incComboRefCount();
			};
		};
		var decComboRefCount = function (thisValue, team) {
			return function () {
				return team.decComboRefCount();
			};
		};
		var getCurrentComboCount = function (thisValue, team) {
			return function () {
				return team.getCurrentCombo();
			};
		};

		var onStartAttackEnemies = function (thisValue, otherTeam) {
			return function (frame) {
				for (var i = 0; i < otherTeam.getPlayers().length; ++i) {
					otherTeam.getPlayer(i).onEnemyStartAttack(frame, this);
				}
			};
		};
		var onContinueAttackEnemies = function (thisValue, otherTeam) {
			return function (frame, hitPoints) {
				for (var i = 0; i < otherTeam.getPlayers().length; ++i) {
					otherTeam.getPlayer(i).onEnemyContinueAttack(frame, this, hitPoints);
				}
			};
		};
		var onVulnerable = function (thisValue, otherTeam) {
			return function (frame, x, y) {
				for (var i = 0; i < otherTeam.getPlayers().length; ++i) {
					otherTeam.getPlayer(i).onEnemyVulerable(frame, this, x, y);
				}
			};
		};
		var onFloating = function (thisValue, otherTeam) {
			return function (frame, x, y) {
				for (var i = 0; i < otherTeam.getPlayers().length; ++i) {
					otherTeam.getPlayer(i).onEnemyFloating(frame, this, x, y);
				}
			};
		};
		var onEndAttackEnemies = function (thisValue, otherTeam) {
			return function (frame) {
				for (var i = 0; i < otherTeam.getPlayers().length; ++i) {
					otherTeam.getPlayer(i).onEnemyEndAttack(frame, this);
				}
			};
		};
		var onDizzy = function (thisValue, otherTeam) {
			return function (frame) {
				for (var i = 0; i < otherTeam.getPlayers().length; ++i) {
					otherTeam.getPlayer(i).onEnemyDizzy(frame, this);
				}
			};
		};
		var getDamageMultiplier = function (thisValue, team) {
			return function () {
				return 1;
			};
		};
		...
		if (team == 1) player.changeDirection(true);
	};
	...
	Match.prototype.start = function (ignoreMusic) {
		...
		for (var i = 0; i < teamA_.getPlayers().length; ++i) {
			...
			teamA_.getPlayer(i).getDamageMultiplier = function () {
				return teamADamageMultiplier;
			};
		}
		...
		for (var i = 0; i < teamB_.getPlayers().length; ++i) {
			...
			teamB_.getPlayer(i).getDamageMultiplier = function () {
				return teamBDamageMultiplier;
			};
		}
		...
		teamB_.init();
		this.checkAIMatch();
		if (isAIMatch_) this.showMainInsertCoin();
	};
	...
	Match.prototype.handleRound1 = function (frame) {
		...
		if (
			gotoNewRoundFrame_ != CONSTANTS.NO_FRAME &&
			frame > gotoNewRoundFrame_ + CONSTANTS.GOTO_NEW_ROUND_DELAY
		)
			this.reset();
	};
	...
	Match.prototype.render = function (frame) {
		if (!this.isSuperMoveActive()) stage_.render();
		...
		if (!isPresented_ && frame > CONSTANTS.PRESENT_DELAY) this.present();
		if (round_ == 1) faceoff_.render(frame);
		...
		if (
			...
			if (user1_.hasCredits()) {
				...
				if (!!hideInsertCoin_) pressStartElement_.style.display = "none";
				else pressStartElement_.style.display = "";
			} else {
				pressStartElement_.style.display = "none";
				insertCoinElement_.style.display = "";
				if (!!hideInsertCoin_) insertCoinElement_.style.display = "none";
				else insertCoinElement_.style.display = "";
			}
		}
	};
