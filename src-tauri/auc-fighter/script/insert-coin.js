var CreateInsertCoinScreen = function (u1, u2) {
	...
	InsertCoinScreen.prototype.isDone = function () {
		return isDone_;
	};
	...
	InsertCoinScreen.prototype.onKeyStateChanged = function (
		isDown,
		keyCode,
		frame,
	) {
		if (!!user1_) {
			if (!!isDown && keyCode == user1_.Coin) this.onAddCredit();
		}
		if (!!user2_) {
			if (!!isDown && keyCode == user2_.Coin) this.onAddCredit();
		}
	};

	InsertCoinScreen.prototype.frameMove = function (frame) {
		if (
			(!!user1_ && user1_.IsRequestingCharSelect) ||
			(!!user2_ && user2_.IsRequestingCharSelect)
		) {
	...
	InsertCoinScreen.prototype.render = function (frame) {
		if (!!mustUpdate_) {
			mustUpdate_ = false;
			if (
				(!!user1_ && user1_.hasCredits()) ||
				(!!user2_ && user2_.hasCredits())
			) {
				...
				nbCredits_ = Math.min(
					9,
					(!!user1_ ? user1_.getNbCredits() : 0) +
						(!!user2_ ? user2_.getNbCredits() : 0),
				);
