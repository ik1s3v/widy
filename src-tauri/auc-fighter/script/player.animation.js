...
Player.prototype.addProjectile = function (name, offsetX, offsetY, vx, vy) {
	var projectile = new Projectile(
		this,
		new Animation(0, name),
		new Animation(0, name + "-disintegrate"),
		offsetX,
		offsetY,
		vx,
		vy,
...
Player.prototype.addThrow = function (
	requiredState,
	name,
	duration,
	keySequence,
...
// Helper function - adds a move for the player
// TODO: remove [duration] and [keySequence], they are no longer used
Player.prototype.addAnimation = function (
	requiredState,
	name,
	duration,
	keySequence,
...
Player.prototype.addGenericAnimation = function (state, team, name, moveFlags) {
	...
	this.OtherAnimations[key] = new GenericAnimation(
		name,
		[],
		moveFlags || MOVE_FLAGS.NONE,
	);
...
Player.prototype.addDizzyAnimation = function (centeredOffset, topOffset) {
	this.OtherAnimations.Dizzy[this.OtherAnimations.Dizzy.length] = {
		...
		Animation: new GenericAnimation(
			"dizzy",
			[],
			MOVE_FLAGS.MOVE_TO_PLAYER,
			0,
			0,
			centeredOffset,
			topOffset,
	...
	return this.OtherAnimations.Dizzy[this.OtherAnimations.Dizzy.length - 1]
		.Animation;
};
...
Player.prototype.addDirtAnimation = function () {
	...
	return this.OtherAnimations.Dirt[this.OtherAnimations.Dirt.length - 1]
		.Animation;
};
...
Player.prototype.addBigDirtAnimation = function () {
	...
	return this.OtherAnimations.BigDirt[this.OtherAnimations.BigDirt.length - 1]
		.Animation;
};
...
Player.prototype.executeAnimation = function (name, forced, ignoreImmobile) {
	...
	if (!!forced || this.isMobile() || this.allowInterupt()) {
		if (!!this.ForceImmobile && !ignoreImmobile) return false;
		...
		for (var i = 0; i < this.Moves.length; ++i) {
			...
			if (this.Moves[i].BaseAnimation.Name == name) animation = this.Moves[i];
			else continue;
			...
			var pstate =
				...
				(POSE_FLAGS.ALLOW_BLOCK | POSE_FLAGS.ALLOW_AIR_BLOCK);
			var pstateNoInterupts =
				(pstate |
				...
				(POSE_FLAGS.ALLOW_INTERUPT_1 |
					...
					POSE_FLAGS.ALLOW_INTERUPT_6);
			var pstateInterupts =
				(move.RequiredFlags & POSE_FLAGS.ALLOW_INTERUPT_1) |
			...
			if (!pstate || !!this.Flags.Pose.has(pstate)) {
				...
				if (
					!!mustAllowAirBlock &&
					!this.Flags.Pose.has(POSE_FLAGS.ALLOW_AIR_BLOCK)
				)
					return false;
				...
				if (!!this.allowInterupt() && !this.Flags.Pose.has(pstateInterupts))
					return false;
				if (!!this.allowInterupt() && !pstateInterupts) return false;

				if (!!this.isProjectileInUse(move)) return false;
			}
		}
		...
		if (!!animation) {
			if (!this.setInteruptAnimation(animation)) {
				this.setCurrentAnimation({
					Animation: animation,
					StartFrame: this.getMatch().getCurrentFrame(),
					Direction: this.Direction,
				});
			}
...
Player.prototype.testAnimation = function (frame, currentEnergy, input, move) {
	...
	if (!!move.IsImplicit) return null;

	if (!!move.EnergyToSubtract && currentEnergy < move.EnergyToSubtract)
		return null;

	var pstate =
		(move.RequiredFlags | POSE_FLAGS.ALLOW_BLOCK | POSE_FLAGS.ALLOW_AIR_BLOCK) ^
		(POSE_FLAGS.ALLOW_BLOCK | POSE_FLAGS.ALLOW_AIR_BLOCK);
	var pstateNoInterupts =
		(pstate |
			POSE_FLAGS.ALLOW_INTERUPT_1 |
	...
	if (!pstate || !!this.Flags.Pose.has(pstate)) {
		...
		if (!!this.allowInterupt() && !this.Flags.Pose.has(pstateInterupts))
			return null;
		if (!!this.allowInterupt() && !pstateInterupts) return null;

		if (!!this.isProjectileInUse(move)) return null;
		...
		if (!!isMatch) {
			if (!!move.GrappleDistance) {
				if (!!this.RegisteredHit.HitID) return null;
				if (!this.tryStartGrapple(move, frame)) return null;
			}
			//you can only chain to a different block if you are currently blocking
			if (this.isBlocking()) return null;
			return move;
		} else {
			if (!!move.GrappleDistance) return null;
		}
	}
...
Player.prototype.goToStance = function (frame) {
	if (this.isDead()) {
		...
		this.setCurrentAnimation(
			{ Animation: move, StartFrame: frame, Direction: this.Direction },
			true,
		);
...
Player.prototype.spawnRedFire = function (frame) {
	...
	instance.Animation.InitialY =
		this.getConstOffsetTop() -
		this.HeadOffsetY -
		game_.getMatch().getStage().getGroundY();
...
Player.prototype.spawnSmallDirt = function (frame, offsetX) {
	...
	if (index > -1) {
		...
		if (!!offsetX)
			this.setLastHit(
				instance.Animation,
				CONSTANTS.USE_PLAYER_XY,
				offsetX,
				CONSTANTS.SMALLDIRT_OFFSETY,
			);
		else this.setLastHit(instance.Animation, CONSTANTS.USE_PLAYER_BOTTOM);
...
Player.prototype.spawnHitReportAnimations = function (
	frame,
	flags,
	hitState,
	attackDirection,
	...
	if (!!this.Flags.Player.has(PLAYER_FLAGS.BLOCKING)) {
		frontKey = _c3("_", ATTACK_FLAGS.FRONT | ATTACK_FLAGS.BLOCK, "_0");
	} else if (hasFlag(flags, ATTACK_FLAGS.LIGHT)) {
		frontKey = _c4(
			"_",
			ATTACK_FLAGS.FRONT | ATTACK_FLAGS.LIGHT,
			"_",
			this.Team,
		);
	} else if (hasFlag(flags, ATTACK_FLAGS.MEDIUM)) {
		frontKey = _c4(
			"_",
			ATTACK_FLAGS.FRONT | ATTACK_FLAGS.MEDIUM,
			"_",
			this.Team,
		);
	} else if (hasFlag(flags, ATTACK_FLAGS.HARD)) {
		frontKey = _c4("_", ATTACK_FLAGS.FRONT | ATTACK_FLAGS.HARD, "_", this.Team);
	}
	...
	if (!!animation) {
		...
		this.FrontHitReport[this.FrontHitReport.length] = {
			Animation: animation,
			StartFrame: frame,
			Direction: attackDirection,
			Element: this.getNextFrontHitReportImage(),
		};
	}

	if (
		!this.Flags.Player.has(PLAYER_FLAGS.BLOCKING) &&
		hasFlag(flags, ATTACK_FLAGS.REAR)
	) {
		var rearKey = "";
		if (
			hasFlag(hitState, HIT_FLAGS.HIGH) &&
			hasFlag(flags, ATTACK_FLAGS.SPIT1)
		) {
			rearKey = _c3("_", ATTACK_FLAGS.REAR | ATTACK_FLAGS.SPIT1, "_0");
		} else if (
			hasFlag(hitState, HIT_FLAGS.HIGH) &&
			hasFlag(flags, ATTACK_FLAGS.SPIT2)
		) {
			rearKey = _c3("_", ATTACK_FLAGS.REAR | ATTACK_FLAGS.SPIT2, "_0");
		} else if (hasFlag(flags, ATTACK_FLAGS.SPECIAL)) {
			if (hasFlag(flags, ATTACK_FLAGS.LIGHT)) {
				rearKey = _c3("_", ATTACK_FLAGS.REAR | ATTACK_FLAGS.SPECIAL1, "_0");
			} else if (hasFlag(flags, ATTACK_FLAGS.MEDIUM)) {
				rearKey = _c3("_", ATTACK_FLAGS.REAR | ATTACK_FLAGS.SPECIAL2, "_0");
			} else if (hasFlag(flags, ATTACK_FLAGS.HARD)) {
				rearKey = _c3("_", ATTACK_FLAGS.REAR | ATTACK_FLAGS.SPECIAL3, "_0");
			}
		} else if (hasFlag(flags, ATTACK_FLAGS.SPECIAL1)) {
			rearKey = _c3("_", ATTACK_FLAGS.REAR | ATTACK_FLAGS.SPECIAL1, "_0");
		} else if (hasFlag(flags, ATTACK_FLAGS.SPECIAL2)) {
			rearKey = _c3("_", ATTACK_FLAGS.REAR | ATTACK_FLAGS.SPECIAL2, "_0");
		} else if (hasFlag(flags, ATTACK_FLAGS.SPECIAL3)) {
			rearKey = _c3("_", ATTACK_FLAGS.REAR | ATTACK_FLAGS.SPECIAL3, "_0");
		} else if (hasFlag(flags, ATTACK_FLAGS.LIGHT)) {
			rearKey = _c4(
				"_",
				ATTACK_FLAGS.REAR | ATTACK_FLAGS.LIGHT,
				"_",
				this.Team,
			);
		} else if (hasFlag(flags, ATTACK_FLAGS.MEDIUM)) {
			rearKey = _c4(
				"_",
				ATTACK_FLAGS.REAR | ATTACK_FLAGS.MEDIUM,
				"_",
				this.Team,
			);
		} else if (hasFlag(flags, ATTACK_FLAGS.HARD)) {
			rearKey = _c4("_", ATTACK_FLAGS.REAR | ATTACK_FLAGS.HARD, "_", this.Team);
		}

		var rearAnimation = this.OtherAnimations[rearKey];
		if (!!rearAnimation) {
			this.setLastHit(rearAnimation, CONSTANTS.USE_PLAYER_TOP);
			this.RearHitReport[this.RearHitReport.length] = {
				Animation: rearAnimation,
				StartFrame: frame,
				Direction: attackDirection,
				Element: this.getNextRearHitReportImage(),
			};
		}
	}
};
...
Player.prototype.tryChainOnHit = function (frame) {
	if (
		...
		!!this.CurrentAnimation.Animation.ChainOnHitAnimation
	) {
...
Player.prototype.tryChainAnimation = function (frame, stageX, stageY) {
	if (
		...
		!!this.CurrentAnimation.Animation &&
		hasFlag(
			this.CurrentAnimation.Animation.Flags.Player,
			PLAYER_FLAGS.LOOP_IF_KEYDOWN,
		)
	) {
		...
		var state =
			this.CurrentAnimation.Animation.ButtonSequence[
				this.CurrentAnimation.Animation.ButtonSequence.length - 1
			];
	...
	if (
		!!this.CurrentAnimation &&
		!!this.MustChangeDirection &&
		!this.isDead() &&
		(!this.CurrentAnimation.Animation ||
			(!!this.CurrentAnimation.Animation &&
				!this.CurrentAnimation.Animation.ChainAnimation))
	) {
		if (this.getMustChangeDirection(true)) {
			if (
				!!this.CurrentAnimation.Animation &&
				!!this.CurrentAnimation.Animation.Flags.Pose &&
				hasFlag(
					this.CurrentAnimation.Animation.Flags.Pose,
					POSE_FLAGS.QUICK_CHANGE_DIRECTION,
				)
			)
...
Player.prototype.chainToAnimation = function (
	frame,
	move,
	frameOffset,
	stageX,
	...
	var newFrame =
		move.BaseAnimation.Frames[
			frameOffset || this.CurrentAnimation.Animation.ChainAnimationFrame
		];
	var attackDirection = this.CurrentAnimation.AttackDirection;

	var tmp = {
		ID: this.Id + move.BaseAnimation.Name + frame.toString(),
		Animation: move,
		StartFrame: frame - newFrame.FrameOffset,
		Direction: this.Direction,
...
Player.prototype.setCurrentAnimation = function (newAnimation, isChaining) {
	...
	if (!!this.CurrentAnimation && this.CurrentAnimation.Animation) {
		...
		if (!!newAnimation && !!this.CurrentAnimation.Animation.ChainAnimation) {
			if (
				hasFlag(
					this.CurrentAnimation.Animation.ChainAnimation.Flags.Player,
					PLAYER_FLAGS.USE_CURRENT_VX,
				)
			)
				newAnimation.Vx =
					this.CurrentAnimation.Animation.ChainAnimation.chainVxFn(
						this.CurrentAnimation.Vx,
					);
			if (
				hasFlag(
					this.CurrentAnimation.Animation.ChainAnimation.Flags.Player,
					PLAYER_FLAGS.USE_CURRENT_VY,
				)
			)
				newAnimation.Vy =
					this.CurrentAnimation.Animation.ChainAnimation.chainVyFn(
						this.CurrentAnimation.Vy,
					);
		}
	...
	if (!!newAnimation && !!newAnimation.Animation) {
		...
		this.CurrentAnimation.ID = _c3(
			this.Id,
			this.CurrentAnimation.Animation.BaseAnimation.Name,
			game_.getCurrentFrame(),
		);
		...
		if (!!this.CurrentAnimation.Animation.Flags.Player) {
			...
			if (
				hasFlag(
					this.CurrentAnimation.Animation.Flags.Player,
					PLAYER_FLAGS.MOVE_TO_ENEMY,
				)
			)
				this.CurrentAnimation.Animation.Vx =
					this.getCDHelper().getInitialCloseGapVelocityX(
						this,
						this.getTarget(),
						this.CurrentAnimation.Animation.Vy,
					);
			//turn around if required
			if (
				hasFlag(
					this.CurrentAnimation.Animation.Flags.Player,
					PLAYER_FLAGS.FACE_ENEMY,
				)
			) {
				if (!!this.MustChangeDirection) this.changeDirection(true);
			}
			//clear fire if required
			ignoreClearFire = !hasFlag(
				this.CurrentAnimation.Animation.Flags.Combat,
				COMBAT_FLAGS.IGNORE_CLEAR_FIRE,
			);
		}

		if (!!ignoreClearFire) this.clearFire();
		...
		if (!newAnimation.Animation.IsThrow) this.abortThrow();
		...
		if (this.isAirborne()) this.CanHoldAirborne = false;
		else this.CanHoldAirborne = true;

		if (
			!this.CurrentAnimation.Animation.KeepCurrentAirborneFunctions ||
			!!this.CurrentAnimation.Animation.UseNewAirborneFunctions
		) {
		...
		if (!!newAnimation.Animation.Trail)
			this.CurrentAnimation.Animation.Trail.enable(
				newAnimation.StartFrame,
				this.Element,
				this.Direction,
			);

		if (!isChaining) this.showFirstAnimationFrame();

		//if this move is an attack, then raise the event
		if (
			!!this.CurrentAnimation.Animation.BaseAnimation.IsAttack &&
			!this.CurrentAnimation.Animation.ProjectileId
		) {
...
Player.prototype.doAnimationAlerts = function () {
	if (
		!!this.CurrentAnimation &&
		!!this.CurrentAnimation.Animation &&
		hasFlag(this.CurrentAnimation.Animation.Flags.Alert, ALERT_FLAGS.DIZZY)
	) {
		this.onDizzyFn();
	}
};
...
Player.prototype.AirborneFlags =
...
Player.prototype.PoseFlagsToIgnore =
	POSE_FLAGS.AIR_COMBO_1 |
	POSE_FLAGS.AIR_COMBO_2 |
	POSE_FLAGS.AIRBORNE |
	POSE_FLAGS.AIRBORNE_FB |
...
Player.prototype.CombatFlagsToIgnore =
...
Player.prototype.PlayerFlagsToIgnore =
	PLAYER_FLAGS.MOBILE | PLAYER_FLAGS.BULLDOZE;
...
Player.prototype.setCurrentFrame = function (
	newFrame,
	frame,
	stageX,
	stageY,
	...
	if (!!this.CurrentFrame) {
		if (!!newFrame && newFrame.ID == this.CurrentFrame.ID) return;
		...
		this.Flags.Player.remove(
			(this.CurrentFrame.FlagsToSet.Player |
				Player.prototype.PlayerFlagsToIgnore) ^
				Player.prototype.PlayerFlagsToIgnore,
		);
		this.Flags.Pose.remove(
			(this.CurrentFrame.FlagsToSet.Pose | Player.prototype.PoseFlagsToIgnore) ^
				Player.prototype.PoseFlagsToIgnore,
		);
		this.Flags.Combat.remove(
			(this.CurrentFrame.FlagsToSet.Combat |
				Player.prototype.CombatFlagsToIgnore) ^
				Player.CombatFlagsToIgnore,
		);
	...
	if (!!this.IsFirstFrame) this.IsNewFrame = true;
	else if (!!newFrame && !this.CurrentFrame) this.IsNewFrame = true;
	else if (
		!!newFrame &&
		!!this.CurrentFrame &&
		...
		spriteLookup_.getLeft(newFrame.RightSrc) !=
			spriteLookup_.getLeft(this.CurrentFrame.RightSrc)
	)
		this.IsNewFrame = true;

	var isNewSound =
		!!newFrame &&
		((!!newFrame.SoundFilename &&
			(!this.CurrentFrame ||
				(!!this.CurrentFrame &&
					this.CurrentFrame.SoundFilename != newFrame.SoundFilename))) ||
			!!newFrame.FlagsToSet.SwingSound);

	this.CurrentFrame = newFrame;
	if (!!newFrame) {
		this.IsFirstFrame = false;
		if (this.CurrentAnimation.Animation.IsSuperMove) this.stopSlide(true);
		...
		if (!!this.IsNewFrame) {
			...
			if (!!this.CurrentFrame.SlideForce)
				this.startSlide(
					frame,
					this.CurrentFrame.SlideForce,
					this.Direction,
					this.CurrentFrame.SlideFactor,
					this.CurrentFrame.HideSlideDirt,
					true,
				);
		}

		this.IgnoreHoldFrame = hasFlag(
			newFrame.FlagsToSet.Player,
			PLAYER_FLAGS.IGNORE_HOLD_FRAME,
		);
		...
		if (
			!this.Flags.Combat.has(COMBAT_FLAGS.PROJECTILE_ACTIVE) &&
			hasFlag(newFrame.FlagsToSet.Combat, COMBAT_FLAGS.SPAWN_PROJECTILE)
		) {
		...
		if (
			!!this.IsSliding &&
			hasFlag(newFrame.FlagsToSet.Combat, COMBAT_FLAGS.STOP_SLIDE_BACK)
		)
			this.stopSlide();
		...
		if (
			hasFlag(newFrame.FlagsToSet.Player, PLAYER_FLAGS.MOVE_TO_BACK) ||
			hasFlag(newFrame.FlagsToSet.Player, PLAYER_FLAGS.BLOCKING)
		)
			this.moveToBack();
		...
		ignoredFlags =
			Player.prototype
				.AirborneFlags; /*flags to be set in the OnFrame function*/
		this.Flags.Pose.add(
			(newFrame.FlagsToSet.Pose | ignoredFlags) ^ ignoredFlags,
		);
        ...
        */
		...
		this.ClipMoveFront =
			newFrame.ClipMoveFront ||
			(this.isStanding() ? this.StandingClip.Front : 0) ||
			0;
		this.ClipMoveBack =
			newFrame.ClipMoveBack ||
			(this.isStanding() ? this.StandingClip.Back : 0) ||
			0;
		this.ClipMoveBottom =
			newFrame.ClipMoveBottom ||
			(this.isStanding() ? this.StandingClip.Bottom : 0) ||
			0;
		//some players will have a default clipTop, to make it easier to be jumped over.
		this.ClipMoveTop =
			newFrame.ClipMoveTop ||
			(this.isStanding() ? this.StandingClip.Top : 0) ||
			0;

		if (!ignoreTranslation) {
			if (!!newFrame.X) this.moveX(newFrame.X);
			if (!!newFrame.Y) this.moveY(newFrame.Y);
		}

		if (
			(!!newFrame.SoundFilename || !!newFrame.FlagsToSet.SwingSound) &&
			!!isNewSound
		) {
...
Player.prototype.isStanding = function () {
	return !this.isCrouching() && !this.isAirborne() && !this.isDead();
};
...
Player.prototype.initSprite = function () {
	...
	for (var i = 0; i < this.Projectiles.length; ++i)
		this.Projectiles[i].Element.style.backgroundImage =
			"url(images/misc/" + this.Folder.toLowerCase() + "/projectiles.png)";
};
...
Player.prototype.setSprite = function (frame) {
	...
	if (!this.IsSpriteReversed) {
		if (!!this.CurrentFrame.IsFlipped) {
			if (this.Direction == -1 && !!this.IsFlipped) this.flip(false);
			else if (this.Direction == 1 && !this.IsFlipped) this.flip(true);
		} else {
			if (this.Direction == -1 && !this.IsFlipped) this.flip(true);
			else if (this.Direction == 1 && !!this.IsFlipped) this.flip(false);
		}
	}
	if (this.Direction == -1)
		// && (this.CurrentFrame.ImageOffsetX != undefined))
	...
	if (!!this.CurrentAnimation && !!this.CurrentAnimation.Animation)
		this.CurrentAnimation.Animation.renderChildren(
			frame,
			this.CurrentAnimation.StartFrame,
			this.CurrentAnimation.Direction,
			parseInt(this.Element.style.zIndex) + 1,
			this.X,
			this.Y,
		);
};
...
Player.prototype.render = function (frame, stageDiffX, stageDiffY) {
	...
	if (!this.IsPaused) {
		if (!!this.CurrentFrame) {
			...
			if (
				!!this.CurrentFrame.ShadowImageSrc &&
		...
		if (this.Direction > 0) this.setRight(this.X);
		else this.setLeft(this.X);
		this.Element.style.bottom =
			Math.max(this.Y, game_.getMatch().getStage().getGroundY()) + "px";
		this.setImgRect();

		this.renderShadow();
		this.renderTrail(frame, stageDiffX, stageDiffY);

		if (!!__debugMode) this.renderDebugInfo();
	}
	this.IsNewFrame = false;
};
...
Player.prototype.renderTrail = function (frame, stageDiffX, stageDiffY) {
	if (
		!!this.CurrentAnimation &&
		!!this.CurrentAnimation.Animation &&
		!!this.CurrentAnimation.Animation.Trail
	)
		this.CurrentAnimation.Animation.Trail.render(
			frame,
			-this.Direction * stageDiffX,
			stageDiffY,
		);
};
...
Player.prototype.renderShadow = function () {
	if (this.Direction > 0) {
		this.ShadowX =
			this.X +
			((!!this.CurrentFrame ? this.CurrentFrame.ShadowOffset.X : 0) ||
				this.DefaultShadowOffset) +
			"px";
		if (this.ShadowX != this.LastShadowX || !!this.MustForceRenderShadow) {
			...
			if (!this.Shadow.style.right) this.Shadow.style.right = "0px";
			if (
				!!this.getAdjustShadowPosition() &&
				!this.ForceNoAdjustShadowPosition
			) {
		...
		if (this.ShadowX != this.LastShadowX || !!this.MustForceRenderShadow) {
			...
			if (!this.Shadow.style.left) this.Shadow.style.left = "0px";
			if (
				!!this.getAdjustShadowPosition() &&
				!this.ForceNoAdjustShadowPosition
			) {
	...
	this.ShadowY =
		game_.getMatch().getStage().getOffsetY(true) +
		(!!this.CurrentFrame ? this.CurrentFrame.ShadowOffset.Y : 0) +
		"px";
...
Player.prototype.forceWinAnimation = function (frame) {
	var name =
		this.WinAnimationNames[
			Math.ceil(Math.random() * this.WinAnimationNames.length) - 1
		];
