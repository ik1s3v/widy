...
User.prototype.enableStoryMode = function () {
	if (!this.IsAI) this.IsInStoryMode = true;
	this.StoryModeLevel = this.StoryMode.getLevel();
};

User.prototype.advanceStoryMode = function () {
	if (!this.IsAI) {
		if (!!this.IsInStoryMode) this.StoryMode.incLevel();
		else this.IsInStoryMode = true;
	}
...
User.prototype.useCredit = function () {
	if (!this.NbCredits) return false;
...
User.prototype.charSelectElementsVisible = function (isVisible) {
	if (!!this.SelectIcon.Element) {
		...
		if (state == "none") this.RandomCharFace.Element.style.display = "none";
	}
};
...
User.prototype.init = function (isUser1) {
	...
	if (!!isUser1) {
		if (this.Selected === null) this.Selected = CHARACTERS.RYU;

		this.Animations["select_icon"] = new BasicAnimation(
			"select_icon",
			[],
			true,
		);
		this.Animations["select_icon"].addFrame(
			this,
			"|images/misc/misc/p1-select-0.png",
			1,
		);
		this.Animations["select_icon"].addFrame(
			this,
			"|images/misc/misc/p1-select-1.png",
			1,
		);
		...
		if (this.Selected === null) this.Selected = CHARACTERS.KEN;
		...
		this.Animations["select_icon"].addFrame(
			this,
			"|images/misc/misc/p2-select-0.png",
			1,
		);
		this.Animations["select_icon"].addFrame(
			this,
			"|images/misc/misc/p2-select-1.png",
			1,
		);
...
User.prototype.onKeyStateChanged = function (isDown, keyCode, frame) {
	if (!!isDown) {
		if (keyCode == this.Coin) {
			...
			if (!!this.Player && !this.Player.Ai.isRunning()) return;
		...
		if (!this.IsCharSelected) {
			...
			if (keyCode == this.Down) direction = CONSTANTS.DOWN;
				...
				if (
					...
					this.IsAlternate =
						keyCode == this.K1 || keyCode == this.K2 || keyCode == this.K3;
			...
			if (!!direction) {
				...
				if (
					!(
						sel.Row == CONSTANTS.ROW3 &&
						(sel.Col == CONSTANTS.COL1 || sel.Col == CONSTANTS.COL4)
					)
				) {
					this.RandomSelect = 0;
...
User.prototype.determineIsAternate = function () {
	...
	if (isCharOnOtherTeam && !isAltCharOnOtherTeam) {
		...
		if (!isCharOnOtherTeam) this.IsAlternate = this.IsAlternate || false;
...
User.prototype.showCharacter = function () {
	...
	spriteLookup_.set(
		...
		"images/misc/misc/name-" + this.CurrentStance + ".png",
	);
	spriteLookup_.set(
		this.PortriatElement.Element,
		"images/misc/misc/p2-select-" + this.CurrentStance + ".png",
	);
	spriteLookup_.set(
		this.ShadowElement.Element,
		"images/misc/misc/" + this.CurrentStance + "-shadow.png",
	);

	if (!this.IsCharSelected && !!this.RandomSelect) {
		spriteLookup_.set(
			this.RandomCharFace.Element,
			"images/misc/misc/char-" + this.CurrentStance + "-r.png",
		);
	}
};
...
User.prototype.setPositionValues = function (
	shadowX,
	shadowY,
	nameX,
	nameY,
...
User.prototype.render = function (frame) {
	if (!this.IsCharSelected && !!this.RandomSelect && frame % 5 === 0) {
		...
		this.showCharacter();
		if (++this.RandomSelect > CHAR_NAMES.length) this.RandomSelect = 1;
	}
	...
	if (!!this.Animations[this.CurrentStance]) {
		this.setPositions();
		this.Animations[this.CurrentStance].tryRender(
			frame,
			this.SelectedCharStance,
			this.Direction,
		);
...
User.prototype.addStanceAnimations = function () {
	if (!this.IsInitialized) {
		...
		this.Animations["ryu"].addFrame(
			this,
			"images/misc/misc/ryu-r-stance-0.png",
			5,
		);
		this.Animations["ryu"].addFrame(
			this,
			"images/misc/misc/ryu-r-stance-1.png",
			5,
		);
		this.Animations["ryu"].addFrame(
			this,
			"images/misc/misc/ryu-r-stance-2.png",
			5,
		);
		this.Animations["ryu"].addFrame(
			this,
			"images/misc/misc/ryu-r-stance-3.png",
			5,
		);
		this.Animations["ryu"].addFrame(
			this,
		...
		this.Animations["ryu_selected"] = new BasicAnimation(
			"ryu_selected",
			[],
			true,
		);
		this.Animations["ryu_selected"].addFrame(
			this,
			"images/misc/misc/ryu-r-win-2-0.png",
			5,
		);
		this.Animations["ryu_selected"].addFrame(
			this,
			"images/misc/misc/ryu-r-win-2-1.png",
			5,
		);
		this.Animations["ryu_selected"].addFrame(
			this,
			"images/misc/misc/ryu-r-win-2-2.png",
			CONSTANTS.MAX_FRAME,
		);

		this.Animations["ken"] = new BasicAnimation("ken_stance", [], true);
		this.Animations["ken"].addFrame(
			this,
			"images/misc/misc/ken-r-stance-0.png",
			5,
		);
		this.Animations["ken"].addFrame(
			this,
			"images/misc/misc/ken-r-stance-1.png",
			5,
		);
		this.Animations["ken"].addFrame(
			this,
			"images/misc/misc/ken-r-stance-2.png",
			5,
		);
		this.Animations["ken"].addFrame(
			this,
			"images/misc/misc/ken-r-stance-3.png",
			5,
		);
		this.Animations["ken"].addFrame(
			this,
		...
		this.Animations["ken_selected"] = new BasicAnimation(
			"ken_selected",
			[],
			true,
		);
		this.Animations["ken_selected"].addFrame(
			this,
			"images/misc/misc/ken-r-win-2-0.png",
			5,
		);
		this.Animations["ken_selected"].addFrame(
			this,
			"images/misc/misc/ken-r-win-2-1.png",
			5,
		);
		this.Animations["ken_selected"].addFrame(
			this,
			"images/misc/misc/ken-r-win-2-2.png",
			CONSTANTS.MAX_FRAME,
		);

		this.Animations["akuma"] = new BasicAnimation("akuma_stance", [], true);
		this.Animations["akuma"].addFrame(
			this,
			"images/misc/misc/akuma-r-stance-0.png",
			5,
		);
		this.Animations["akuma"].addFrame(
			this,
			"images/misc/misc/akuma-r-stance-1.png",
			5,
		);
		this.Animations["akuma"].addFrame(
			this,
			"images/misc/misc/akuma-r-stance-2.png",
			5,
		);
		this.Animations["akuma"].addFrame(
			this,
			"images/misc/misc/akuma-r-stance-3.png",
			5,
		);
		this.Animations["akuma"].addFrame(
			this,
		...
		this.Animations["akuma_selected"] = new BasicAnimation(
			"akuma_selected",
			[],
			true,
		);
		this.Animations["akuma_selected"]
			.addFrame(this, "images/misc/misc/akuma-selected-0.png", 8)
			.flip();
		this.Animations["akuma_selected"]
			.addFrame(this, "images/misc/misc/akuma-selected-1.png", 8)
			.flip();
		this.Animations["akuma_selected"].addFrame(
			this,
			"images/misc/misc/akuma-selected-2.png",
			6,
		);
		this.Animations["akuma_selected"].addFrame(
			this,
			"images/misc/misc/akuma-selected-3.png",
			4,
		);
		this.Animations["akuma_selected"].addFrame(
			this,
			"images/misc/misc/akuma-selected-4.png",
			4,
		);
		this.Animations["akuma_selected"].addFrame(
			this,
			"images/misc/misc/akuma-selected-5.png",
			4,
		);
		this.Animations["akuma_selected"].addFrame(
			this,
			"images/misc/misc/akuma-selected-6.png",
			4,
		);
		this.Animations["akuma_selected"].addFrame(
			this,
			"images/misc/misc/akuma-selected-7.png",
			4,
		);
		this.Animations["akuma_selected"].addFrame(
			this,
			"images/misc/misc/akuma-selected-8.png",
			4,
		);
		this.Animations["akuma_selected"].addFrame(
			this,
			"images/misc/misc/akuma-selected-9.png",
			CONSTANTS.MAX_FRAME,
		);

		this.Animations["mbison"] = new BasicAnimation("ken_stance", [], true);
		this.Animations["mbison"].addFrame(
			this,
			"images/misc/misc/mbison-r-stance-0.png",
			5,
		);
		this.Animations["mbison"].addFrame(
			this,
			"images/misc/misc/mbison-r-stance-1.png",
			5,
		);
		this.Animations["mbison"].addFrame(
			this,
			"images/misc/misc/mbison-r-stance-2.png",
			5,
		);
		this.Animations["mbison"].addFrame(
			this,
			"images/misc/misc/mbison-r-stance-3.png",
			5,
		);
		this.Animations["mbison"].addFrame(
			this,
		...
		this.Animations["mbison_selected"] = new BasicAnimation(
			"mbison_selected",
			[],
			true,
		);
		this.Animations["mbison_selected"].addFrame(
			this,
			"images/misc/misc/mbison-r-win-0-0.png",
			5,
		);
		this.Animations["mbison_selected"].addFrame(
			this,
			"images/misc/misc/mbison-r-win-0-1.png",
			5,
		);
		this.Animations["mbison_selected"].addFrame(
			this,
			"images/misc/misc/mbison-r-win-0-2.png",
			5,
		);
		this.Animations["mbison_selected"].addFrame(
			this,
			"images/misc/misc/mbison-r-win-0-3.png",
			5,
		);
		this.Animations["mbison_selected"].addFrame(
			this,
			"images/misc/misc/mbison-r-win-0-4.png",
			5,
		);
		this.Animations["mbison_selected"].addFrame(
			this,
			"images/misc/misc/mbison-r-win-0-5.png",
			5,
		);
		this.Animations["mbison_selected"].addFrame(
			this,
			"images/misc/misc/mbison-r-win-0-6.png",
			CONSTANTS.MAX_FRAME,
		);

		this.Animations["sagat"] = new BasicAnimation("sagat_stance", [], true);
		this.Animations["sagat"].addFrame(
			this,
			"images/misc/misc/sagat-r-stance-0.png",
			5,
		);
		this.Animations["sagat"].addFrame(
			this,
			"images/misc/misc/sagat-r-stance-3.png",
			5,
		);
		this.Animations["sagat"].addFrame(
			this,
			"images/misc/misc/sagat-r-stance-2.png",
			5,
		);
		this.Animations["sagat"].addFrame(
			this,
			"images/misc/misc/sagat-r-stance-1.png",
			5,
		);
		this.Animations["sagat"].addFrame(
			this,
		...
		this.Animations["sagat_selected"] = new BasicAnimation(
			"sagat_selected",
			[],
			true,
		);
		this.Animations["sagat_selected"].addFrame(
			this,
			"images/misc/misc/sagat-selected-0.png",
			5,
		);
		this.Animations["sagat_selected"].addFrame(
			this,
			"images/misc/misc/sagat-selected-1.png",
			5,
		);
		this.Animations["sagat_selected"].addFrame(
			this,
			"images/misc/misc/sagat-selected-2.png",
			5,
		);
		this.Animations["sagat_selected"].addFrame(
			this,
			"images/misc/misc/sagat-selected-3.png",
			5,
		);
		this.Animations["sagat_selected"].addFrame(
			this,
			"images/misc/misc/sagat-selected-4.png",
			5,
		);
		this.Animations["sagat_selected"].addFrame(
			this,
			"images/misc/misc/sagat-selected-5.png",
			5,
		);
		this.Animations["sagat_selected"].addFrame(
			this,
			"images/misc/misc/sagat-selected-6.png",
			5,
		);
		this.Animations["sagat_selected"].addFrame(
			this,
			"images/misc/misc/sagat-selected-7.png",
			5,
		);
		this.Animations["sagat_selected"].addFrame(
			this,
			"images/misc/misc/sagat-selected-8.png",
			5,
		);
		this.Animations["sagat_selected"].addFrame(
			this,
			"images/misc/misc/sagat-selected-9.png",
			5,
		);
		this.Animations["sagat_selected"].addFrame(
			this,
			"images/misc/misc/sagat-selected-10.png",
			5,
		);
		this.Animations["sagat_selected"].addFrame(
			this,
			"images/misc/misc/sagat-selected-11.png",
			5,
		);
		this.Animations["sagat_selected"].addFrame(
			this,
			"images/misc/misc/sagat-selected-12.png",
			5,
		);
		this.Animations["sagat_selected"].addFrame(
			this,
			"images/misc/misc/sagat-selected-13.png",
			5,
		);
		this.Animations["sagat_selected"].addFrame(
			this,
			"images/misc/misc/sagat-selected-14.png",
			5,
		);
		this.Animations["sagat_selected"].addFrame(
			this,
			"images/misc/misc/sagat-selected-15.png",
			5,
		);
		this.Animations["sagat_selected"].addFrame(
			this,
			"images/misc/misc/sagat-selected-16.png",
			5,
		);
		this.Animations["sagat_selected"].addFrame(
			this,
			"images/misc/misc/sagat-selected-17.png",
			CONSTANTS.MAX_FRAME,
		);

		this.Animations["guy"] = new BasicAnimation("guy_stance", [], true);
		this.Animations["guy"].addFrame(
			this,
			"images/misc/misc/guy-r-stance-0.png",
			26,
		);
		this.Animations["guy"].addFrame(
			this,
			"images/misc/misc/guy-r-stance-3.png",
			15,
		);
		this.Animations["guy"].addFrame(
			this,
			"images/misc/misc/guy-r-stance-2.png",
			15,
		);
		this.Animations["guy"].addFrame(
			this,
			"images/misc/misc/guy-r-stance-1.png",
			26,
		);
		this.Animations["guy"].addFrame(
			this,
		...
		this.Animations["birdie"].addFrame(
			this,
			"images/misc/misc/birdie-r-stance-0.png",
			15,
		);
		this.Animations["birdie"].addFrame(
			this,
			"images/misc/misc/birdie-r-stance-1.png",
			15,
		);
		this.Animations["birdie"].addFrame(
			this,
			"images/misc/misc/birdie-r-stance-2.png",
			15,
		);
		this.Animations["birdie"].addFrame(
			this,
			"images/misc/misc/birdie-r-stance-3.png",
			15,
		);

		this.Animations["chunli"] = new BasicAnimation("chunli_stance", [], true);
		this.Animations["chunli"].addFrame(
			this,
			"images/misc/misc/chunli-r-stance-0.png",
			10,
		);
		this.Animations["chunli"].addFrame(
			this,
			"images/misc/misc/chunli-r-stance-1.png",
			10,
		);
		this.Animations["chunli"].addFrame(
			this,
			"images/misc/misc/chunli-r-stance-2.png",
			10,
		);
		this.Animations["chunli"].addFrame(
			this,
			"images/misc/misc/chunli-r-stance-3.png",
			10,
		);

		this.Animations["charlie"] = new BasicAnimation("charlie_stance", [], true);
		this.Animations["charlie"].addFrame(
			this,
			"images/misc/misc/charlie-r-stance-0.png",
			5,
		);
		this.Animations["charlie"].addFrame(
			this,
			"images/misc/misc/charlie-r-stance-1.png",
			5,
		);
		this.Animations["charlie"].addFrame(
			this,
			"images/misc/misc/charlie-r-stance-2.png",
			5,
		);
		this.Animations["charlie"].addFrame(
			this,
			"images/misc/misc/charlie-r-stance-3.png",
			5,
		);
		this.Animations["charlie"].addFrame(
			this,
		...
		this.Animations["sodom"].addFrame(
			this,
			"images/misc/misc/sodom-r-stance-0.png",
			5,
		);
		this.Animations["sodom"].addFrame(
			this,
			"images/misc/misc/sodom-r-stance-1.png",
			5,
		);
		this.Animations["sodom"].addFrame(
			this,
			"images/misc/misc/sodom-r-stance-2.png",
			5,
		);
		this.Animations["sodom"].addFrame(
			this,
			"images/misc/misc/sodom-r-stance-3.png",
			5,
		);
		this.Animations["sodom"].addFrame(
			this,
			"images/misc/misc/sodom-r-stance-4.png",
			5,
		);
		this.Animations["sodom"].addFrame(
			this,
			"images/misc/misc/sodom-r-stance-5.png",
			5,
		);

		this.Animations["adon"] = new BasicAnimation("adon_stance", [], true);
		this.Animations["adon"].addFrame(
			this,
			"images/misc/misc/adon-r-stance-0.png",
			5,
		);
		this.Animations["adon"].addFrame(
			this,
			"images/misc/misc/adon-r-stance-1.png",
			5,
		);
		this.Animations["adon"].addFrame(
			this,
			"images/misc/misc/adon-r-stance-2.png",
			5,
		);
		this.Animations["adon"].addFrame(
			this,
			"images/misc/misc/adon-r-stance-3.png",
			5,
		);
		this.Animations["adon"].addFrame(
			this,
			"images/misc/misc/adon-r-stance-4.png",
			5,
		);

		this.Animations["rose"] = new BasicAnimation("rose_stance", [], true);
		this.Animations["rose"].addFrame(
			this,
			"images/misc/misc/rose-r-c-stance-0.png",
			5,
		);
		this.Animations["rose"].addFrame(
			this,
			"images/misc/misc/rose-r-c-stance-1.png",
			5,
		);
		this.Animations["rose"].addFrame(
			this,
			"images/misc/misc/rose-r-c-stance-2.png",
			5,
		);
		this.Animations["rose"].addFrame(
			this,
			"images/misc/misc/rose-r-c-stance-3.png",
			5,
		);
		this.Animations["rose"].addFrame(
			this,
			"images/misc/misc/rose-r-c-stance-4.png",
			5,
		);
		this.Animations["rose"].addFrame(
			this,
			"images/misc/misc/rose-r-c-stance-5.png",
			5,
		);

		this.Animations["random"] = new BasicAnimation("random", [], true);
		this.Animations["random"].addFrame(
			this,
			"images/misc/misc/rose-r-c-stance-0.png",
			5,
		);
