...
MoveOverrideFlags.prototype.hasAllowOverrideFlag = function (flag) {
	return !!flag && hasFlag(this.AllowOverrideFlags, flag);
};
MoveOverrideFlags.prototype.hasOverrideFlag = function (flag) {
	return !!flag && hasFlag(this.OverrideFlags, flag);
};
...
ActionDetails.prototype.getKey = function (playerID, otherID) {
	...
	if (playerID.charAt(1) == "1") retVal = playerID + otherID;
	else retVal = otherID + playerID;
...
var HitSystem = function (actionFrameDelay) {
	...
	this.ActionFrameDelay =
		actionFrameDelay || CONSTANTS.DEFAULT_ACTION_FRAME_DELAY;
};
...
HitSystem.prototype.canHit = function (key, index) {
	...
	if (
		!!victimA.OtherPlayer &&
		!!victimA.Player.CurrentAnimation.Animation &&
		!victimA.IsProjectile
	) {
		if (victimA.Player.isUnhittable()) return false;
		//grapples can not be overriden
		if (victimA.Player.isGrappling()) return false;
		if (victimA.IsGrapple || (!!victimB && victimB.IsGrapple)) return true;
		var flags = victimA.Player.CurrentAnimation.Animation.OverrideFlags;
		var otherFlags =
			victimA.OtherPlayer.CurrentAnimation.Animation.OverrideFlags;

		var imDoingUppercut =
			victimA.Player.CurrentAnimation.Animation.OverrideFlags.hasAllowOverrideFlag(
				OVERRIDE_FLAGS.SHORYUKEN,
			);
		var youreDoingUppercut =
			victimA.OtherPlayer.CurrentAnimation.Animation.OverrideFlags.hasAllowOverrideFlag(
				OVERRIDE_FLAGS.SHORYUKEN,
			);

		var canIOverrideYou =
			(!flags.hasOverrideFlag(OVERRIDE_FLAGS.NONE) &&
				flags.hasOverrideFlag(
					victimA.OtherPlayer.CurrentAnimation.Animation.OverrideFlags
						.AllowOverrideFlags,
				) /*overrides yours*/ &&
				!otherFlags.hasOverrideFlag(
					victimA.Player.CurrentAnimation.Animation.OverrideFlags
						.AllowOverrideFlags,
				)) /*overrides mine (if both moves override each other, then both should hit)*/ ||
			imDoingUppercut /*overrides everything*/;

		var canYouOverrideMe =
			(!otherFlags.hasOverrideFlag(OVERRIDE_FLAGS.NONE) &&
				otherFlags.hasOverrideFlag(
					victimA.Player.CurrentAnimation.Animation.OverrideFlags
						.AllowOverrideFlags,
				) &&
		...
		var iDidFirstJumpInAttack =
			victimA.Player.CurrentAnimation.Animation.BaseAnimation.IsAttack &&
			victimA.OtherPlayer.CurrentAnimation.Animation.BaseAnimation.IsAttack &&
			victimA.Player.isAirborne() &&
			!victimA.OtherPlayer.isAirborne() &&
			victimA.Player.CurrentAnimation.StartFrame <
				victimA.OtherPlayer.CurrentAnimation.StartFrame;

		var youDidFirstJumpInAttack =
			victimA.Player.CurrentAnimation.Animation.BaseAnimation.IsAttack &&
			victimA.OtherPlayer.CurrentAnimation.Animation.BaseAnimation.IsAttack &&
			victimA.OtherPlayer.isAirborne() &&
			!victimA.Player.isAirborne() &&
		...
		var canYourHProjectileHit =
			otherFlags.hasAllowOverrideFlag(OVERRIDE_FLAGS.HPROJECTILE) && !victimB;

		//if we aren't facing our victimA, then we can't override
		if (!victimA.Player.isFacingPlayer(victimA.OtherPlayer, true)) return true;

		//after all attack frames in a move are finished - IgnoreOverrides will be set to true
		if (!!victimA.Player.IgnoreOverrides) return true;

		//ken and ryu's uppercut can not be overriden
		if (!!imDoingUppercut && !youreDoingUppercut) return false;
		else if (!!youreDoingUppercut) return true;

		if (!!canYourHProjectileHit) return true;

		//special overrides
		if (!!canYouOverrideMe) return true;
		else if (!!canIOverrideYou) return false;

		//the first attack on a jump in should hit
		if (!!iDidFirstJumpInAttack) return false;
		else if (!!youDidFirstJumpInAttack) return true;
	}
	return retVal;
};
...
HitSystem.prototype.frameMove = function (frame) {
	for (var i in this.Actions) {
		...
		if (!!this.HoldFrame) {
			if (!!item && !!item.ActionFrame) ++item.ActionFrame;
			continue;
		}
...
HitSystem.prototype.register = function (details) {
	if (details.IsProjectile) details.Key += "_" + details.Frame.toString();
	if (!this.Actions[details.Key])
		this.Actions[details.Key] = {
			ActionFrame:
				details.Frame + (!!details.NoFrameDelay ? 0 : this.ActionFrameDelay),
		};
	if (
		!this.Actions[details.Key][0] ||
		this.Actions[details.Key][0].PlayerID == details.PlayerID
	)
		this.Actions[details.Key][0] = details;
	else this.Actions[details.Key][1] = details;
