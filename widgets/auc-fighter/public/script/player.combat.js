Player.prototype.canBlock = function () {
	return this.Flags.Pose.has(POSE_FLAGS.ALLOW_BLOCK);
};
Player.prototype.canAirBlock = function () {
	return this.Flags.Pose.has(POSE_FLAGS.ALLOW_AIR_BLOCK);
};
...
Player.prototype.isCurrentMoveAttack = function () {
	return (
		!!this.CurrentAnimation &&
		!!this.CurrentAnimation.Animation &&
		!!this.CurrentAnimation.Animation.BaseAnimation.IsAttack
	);
};
Player.prototype.isCurrentMoveProjectile = function () {
	return (
		!!this.CurrentAnimation &&
		!!this.CurrentAnimation.Animation &&
		!!this.CurrentAnimation.Animation.IsProjectile
	);
};
Player.prototype.isProjectilePending = function () {
	return (
		!!this.CurrentAnimation &&
		!!this.CurrentAnimation.Animation &&
		!!this.CurrentAnimation.Animation.IsProjectilePending
	);
};
Player.prototype.allowJuggle = function () {
	return (
		this.Flags.Juggle.has(JUGGLE_FLAGS.ALLOW) &&
		!this.Flags.Juggle.has(JUGGLE_FLAGS.IGNORE)
	);
};
Player.prototype.hasRegisteredHit = function () {
	return !!this.RegisteredHit.HitID;
};
Player.prototype.hasBlueFire = function () {
	return this.Flags.Player.has(PLAYER_FLAGS.BLUE_FIRE);
};
Player.prototype.hasRedFire = function () {
	return this.Flags.Player.has(PLAYER_FLAGS.RED_FIRE);
};
Player.prototype.clearFire = function () {
	if (this.hasBlueFire()) {
		this.Flags.Player.remove(PLAYER_FLAGS.BLUE_FIRE);
		this.OtherAnimations.BlueFire.Animation.stop(
			this.OtherAnimations.BlueFire.Element,
		);
	}
	if (this.hasRedFire()) {
		this.Flags.Player.remove(PLAYER_FLAGS.RED_FIRE);
		this.OtherAnimations.RedFire.Animation.stop(
			this.OtherAnimations.RedFire.Element,
		);
	}
};
...
Player.prototype.canJuggle = function (other, attackId, juggleGroup) {
	if (other.allowJuggle()) {
		...
		if (
			!!attackId &&
			!!juggleGroup &&
			juggleGroup === other.LastHitJuggleGroup[this.Id] &&
			!other.isInCombo(attackId)
		) {
			return false;
		}
...
Player.prototype.canHit = function (otherFlags) {
	if (hasFlag(otherFlags.Player, PLAYER_FLAGS.IGNORE_COLLISIONS)) return false;
	return true;
};
...
Player.prototype.onProjectileMoved = function (
	frame,
	id,
	x,
	y,
...
Player.prototype.sendAttackAlerts = function (frame) {
	if (
		...
		this.onProjectilePending(
			frame,
			this.getFrontX(),
			this.getMidY(),
			!!this.CurrentAnimation.Animation.IsSuperMove,
		);
	if (
		!!this.CurrentFrame &&
		!!hasFlag(this.CurrentFrame.FlagsToSet.AI, AI_FLAGS.ATTACK_PENDING)
	)
		this.onAttackPending(
			frame,
			this.getFrontX(),
			this.getMidY(),
			!!this.CurrentAnimation.Animation.IsSuperMove,
...
Player.prototype.setAllowBlockFromProjectile = function (
	frame,
	isAllowed,
	projectileId,
	x,
...
Player.prototype.removeBlock = function (
	attackId,
	frame,
	isAllowed,
	x,
...
Player.prototype.allowBlock = function (
	attackId,
	frame,
	isAllowed,
	x,
...
Player.prototype.removeAirBlock = function (
	attackId,
	frame,
	isAllowed,
	x,
...
Player.prototype.allowAirBlock = function (
	attackId,
	frame,
	isAllowed,
	x,
...
Player.prototype.setAllowBlock = function (
	attackId,
	frame,
	isAllowed,
	x,
	...
	if (!!isAllowed) {
		...
		if (!!hitPoints) {
			...
			if (!canBlock) {
				if (!this.isBlocking())
					...
					dist > (this.BlockDistanceSq2 || CONSTANTS.MAX_BLOCK_DISTANCE_SQ2)
				)
					return this.setAllowBlock(attackId, frame, false, x1, y1);
			}
		} else {
		...
		if (this.removeBlockableAttack(attackId)) {
			...
			if (this.BlockedAttacks.length == 0) {
				...
				if (
					this.Flags.Player.has(PLAYER_FLAGS.BLOCKING) &&
					!this.isBlockingInAir()
				) {
...
Player.prototype.setAllowAirBlock = function (
	attackId,
	frame,
	isAllowed,
	x,
	...
	if (!!isAllowed) {
		...
		if (!this.addBlockableAirAttack(attackId))
			this.BlockedAirAttacks[this.BlockedAirAttacks.length] = {
				AttackId: attackId,
			};
		/*allow block*/
...
Player.prototype.tryStartGrapple = function (move, frame) {
	if (!!this.InteruptAnimation) return false;
	if (this.hasRegisteredHit()) return false;
	if (
		this.getFrame() - this.LandedOnFrame <
		CONSTANTS.GROUND_FRAMES_FOR_GRAPPLE
	)
		return false;
	...
	var grappledPlayer = this.getCDHelper().getGrappledPlayer(
		this.Team,
		this.getImgFrontX(true),
		this.Y,
		distance,
		airborneFlags,
		this.isAirborne(),
	...
	if (!!grappledPlayer) {
		...
		this.setGiveHit(
			firstFrame.AttackFlags,
			firstFrame.HitStop,
			firstFrame.EnergyToAdd,
			move.BehaviorFlags,
			grappledPlayer,
		);
		grappledPlayer.takeHit(
			firstFrame.AttackFlags,
			HIT_FLAGS.LOW,
			firstFrame.FlagsToSend,
			frame,
			frame,
			firstFrame.BaseDamage,
			firstFrame.EnergyToAdd,
			false,
			0,
			0,
			...
			this,
			move.BehaviorFlags,
			move.InvokedAnimationName,
			firstFrame.FlagsToSet.HitSound,
			firstFrame.FlagsToSet.BlockSound,
		);
...
Player.prototype.setInteruptAnimation = function (move) {
	if (this.allowInterupt()) {
		this.InteruptAnimation = {
			Delay: CONSTANTS.INTERUPT_DELAY,
			Animation: move,
			StartFrame: game_.getCurrentFrame(),
...
Player.prototype.isInThrowableState = function () {
	if (this.isUnhittable()) return false;
	...
	if (this.getHasPendingGrapple() || this.isBeingGrappled()) return false;

	return retVal;
};

//returns true if this player can be grappled given the params
Player.prototype.canBeGrappled = function (
	x,
	y,
	distance,
	airborneFlags,
	...
	if (airborneFlags == AIRBORNE_FLAGS.YES && !this.isAirborne()) return false;
	else if (airborneFlags == AIRBORNE_FLAGS.NO && this.isAirborne())
		return false;
	else if (
		airborneFlags == AIRBORNE_FLAGS.EQUAL &&
	...
	if (
		this.isImgWithinDistanceX(x, distance, grappleDirection) &&
		Math.abs(y - this.Y) < distance &&
		!this.hasInvulnerableFlag() &&
		!this.GrappledPlayer &&
		!this.CurrentAnimation.Animation.OverrideFlags.hasOverrideFlag(
...
Player.prototype.handleGrapple = function (
	forcedFrameIndex,
	frame,
	stageX,
	stageY,
	...
	if (
		!!this.GrappledPlayer &&
		!!this.GrappledPlayer.CurrentAnimation.Animation &&
		!!this.GrappledPlayer.isBeingGrappled()
	) {
		var forcedFrame =
			this.GrappledPlayer.CurrentAnimation.Animation.BaseAnimation.Frames[
				forcedFrameIndex
			];
		if (!!forcedFrame) {
			...
			this.GrappledPlayer.setCurrentFrame(
				forcedFrame,
				frame,
				stageX,
				stageY,
...
Player.prototype.handleProjectiles = function (frame, stageX, stageY) {
	...
	for (var i = 0; i < this.Projectiles.length; ++i) {
		if (this.Projectiles[i].getIsActive()) {
			...
			this.projectileAttackFn(
				frame,
				this.Projectiles[i].advance(frame, stageX, stageY),
			);
...
Player.prototype.handleAttack = function (frame, moveFrame) {
	...
	if (moveFrame.HitPoints.length > 0) {
		...
		for (var i = 0; i < moveFrame.HitPoints.length; ++i) {
			if (this.Direction < 0)
				hitPoints.push({
					x: rect.Left + moveFrame.HitPoints[i].x,
					y: rect.Bottom + moveFrame.HitPoints[i].y,
				});
			else
				hitPoints.push({
					x: rect.Right - moveFrame.HitPoints[i].x,
					y: rect.Bottom + moveFrame.HitPoints[i].y,
				});
		}
	}
	if (!!moveFrame.IsPendingAttack) {
		...
		hitPoints = [];
		if (this.Direction < 0) hitPoints.push({ x: rect.Right, y: rect.Bottom });
		else hitPoints.push({ x: rect.Left, y: rect.Bottom });
	}
	...
	var otherParams = {
		...
		HitReact:
			this.CurrentAnimation.Animation.Flags.HitReact ||
			moveFrame.FlagsToSet.HitReact,
	...
	this.attackFn(
		...
		moveFrame.HitID,
		this.CurrentAnimation.ID,
		this.CurrentAnimation.Animation.MaxNbHits,
		frame,
		moveFrame.HitPoints,
		moveFrame.FlagsToSend,
		moveFrame.AttackFlags,
		baseDamage,
		this.CurrentAnimation.Animation.OverrideFlags,
		moveFrame.EnergyToAdd,
		this.CurrentAnimation.Animation.BehaviorFlags,
		this.CurrentAnimation.Animation.InvokedAnimationName,
		moveFrame.FlagsToSet.HitSound,
		moveFrame.FlagsToSet.BlockSound,
		moveFrame.HitStop,
		otherParams,
	);
};
...
Player.prototype.setRegisteredHit = function (
	attackFlags,
	hitState,
	flags,
	frame,
	...
	if (!!isGrapple) this.setPendingGrapple(true);
	...
	if (
		...
		!!this.CurrentAnimation.Animation &&
		hasFlag(
			this.CurrentAnimation.Animation.Flags.Combat,
			COMBAT_FLAGS.IGNORE_PROJECTILES,
		)
	)
		return false;
...
Player.prototype.checkPendingHit = function () {
	if (!!this.CurrentAnimation.Animation.BaseAnimation.IsAttack) {
		this.getMatch()
			...
			.checkPendingHits(
				this.Id,
				this.CurrentAnimation.Animation.BaseAnimation.OverrideFlags,
			);
	}
};
...
Player.prototype.isBlockingLow = function () {
	return (
		this.Flags.Player.has(PLAYER_FLAGS.BLOCKING) &&
		this.Flags.Pose.has(POSE_FLAGS.CROUCHING)
	);
};
...
Player.prototype.isHitFrameOk = function (playerId, hitFrameId) {
	return (
		!!this.LastHitFrame[playerId] && this.LastHitFrame[playerId] == hitFrameId
	);
};
...
Player.prototype.clearDizzy = function () {
	...
	if (!this.isDead()) this.goToStance();
	this.stopDizzyAudio();
};
...
Player.prototype.increaseDizziness = function (
	frame,
	attackFlags,
	damage,
	factor,
	...
	if (hasFlag(attackFlags, ATTACK_FLAGS.LIGHT)) {
		value = CONSTANTS.LIGHT_INCREASE_DIZZY * factor;
	}
	if (hasFlag(attackFlags, ATTACK_FLAGS.MEDIUM)) {
		value = CONSTANTS.MEDIUM_INCREASE_DIZZY * factor;
	}
	if (hasFlag(attackFlags, ATTACK_FLAGS.HARD)) {
		value = CONSTANTS.HARD_INCREASE_DIZZY * factor;
	}
...
Player.prototype.decreaseDizziness = function (frame) {
	if (this.isDizzy() && !this.hasInvulnerableFlag()) {
		...
		if (!this.changeDizzy(value)) this.clearDizzy();
	}
};
...
Player.prototype.takeHit = function (
	...
	if (this.isDizzy()) this.clearDizzy();
	...
	this.LastHitJuggleGroup[who] = !!otherParams
		? otherParams.HitJuggleGroup
		: null;
	this.LastHitFrame[who] = hitID;
	...
	if (hasFlag(attackFlags, ATTACK_FLAGS.THROW_START)) {
		...
		(hasFlag(attackFlags, ATTACK_FLAGS.HITS_LOW) && this.isBlockingLow()) ||
		(hasFlag(attackFlags, ATTACK_FLAGS.HITS_HIGH) && this.isBlockingHigh()) ||
		(!hasFlag(attackFlags, ATTACK_FLAGS.HITS_LOW) &&
			!hasFlag(attackFlags, ATTACK_FLAGS.HITS_HIGH) &&
			this.isBlocking())
	) {
		wasBlocked = true;
		this.stopGettingDizzy();
		...
		if (hasFlag(attackFlags, ATTACK_FLAGS.LIGHT)) {
			slideAmount = CONSTANTS.DEFAULT_CROUCH_LIGHT_HRSLIDE / 2;
		}
		if (hasFlag(attackFlags, ATTACK_FLAGS.MEDIUM)) {
			slideAmount = CONSTANTS.DEFAULT_CROUCH_MEDIUM_HRSLIDE / 2;
		}
		if (hasFlag(attackFlags, ATTACK_FLAGS.HARD)) {
			slideAmount = CONSTANTS.DEFAULT_CROUCH_HARD_HRSLIDE / 2;
		}
		...
		//notify AI
		if (!!otherPlayer) otherPlayer.onAttackStateChanged(this, ATTACK_STATE.HIT);
		...
		if (this.isBlocking()) {
			...
			if (!!otherParams.Combo && !!this.Hits[attackID]) {
				if (
					hasFlag(otherParams.Combo, COMBO_FLAGS.RED_FIRE_SOUND_ON_MAX_HIT) &&
					!!maxHits &&
					this.Hits[attackID].Nb >= maxHits
				) {
					...
					hasFlag(otherParams.Combo, COMBO_FLAGS.RED_FIRE_ON_MAX_HIT) &&
					!!maxHits &&
					this.Hits[attackID].Nb >= maxHits
				) {
					attackFlags |= ATTACK_FLAGS.RED_FIRE;
					...
					hasFlag(otherParams.Combo, COMBO_FLAGS.BLUE_FIRE_ON_FIRST_HIT) &&
					this.Hits[attackID].Nb == 1
				) {
					attackFlags |= ATTACK_FLAGS.BLUE_FIRE;
				} else if (
					hasFlag(otherParams.Combo, COMBO_FLAGS.KNOCKDOWN_ON_MAX_HIT) &&
					!!maxHits &&
					this.Hits[attackID].Nb >= maxHits
				) {
		...
		if (this.Flags.Pose.has(POSE_FLAGS.CROUCHING)) {
			if (hasFlag(attackFlags, ATTACK_FLAGS.TRIP)) {
				move = this.Moves[this.MoveNdx.Tripped];
			}
			if (hasFlag(attackFlags, ATTACK_FLAGS.LIGHT)) {
				slideAmount = CONSTANTS.DEFAULT_CROUCH_LIGHT_HRSLIDE;
				move = this.Moves[this.MoveNdx.CLH];
			}
			if (hasFlag(attackFlags, ATTACK_FLAGS.MEDIUM)) {
				slideAmount = CONSTANTS.DEFAULT_CROUCH_MEDIUM_HRSLIDE;
				move = this.Moves[this.MoveNdx.CMH];
			}
			if (hasFlag(attackFlags, ATTACK_FLAGS.HARD)) {
				slideAmount = CONSTANTS.DEFAULT_CROUCH_HARD_HRSLIDE;
				move = this.Moves[this.MoveNdx.CHH];
			}
		} else {
			if (
				hasFlag(attackFlags, ATTACK_FLAGS.TRIP) &&
				hasFlag(hitState, HIT_FLAGS.LOW)
			) {
				move = this.Moves[this.MoveNdx.Tripped];
			} else if (hasFlag(attackFlags, ATTACK_FLAGS.KNOCKDOWN)) {
				move = this.Moves[this.MoveNdx.KnockDown];
			} else if (
				hasFlag(attackFlags, ATTACK_FLAGS.LIGHT) &&
				hasFlag(hitState, HIT_FLAGS.LOW)
			) {
				slideAmount = CONSTANTS.DEFAULT_LIGHT_HRSLIDE;
				move = this.Moves[this.MoveNdx.SLL];
			} else if (
				hasFlag(attackFlags, ATTACK_FLAGS.LIGHT) &&
				hasFlag(hitState, HIT_FLAGS.HIGH)
			) {
				slideAmount = CONSTANTS.DEFAULT_LIGHT_HRSLIDE;
				move = this.Moves[this.MoveNdx.SLH];
			} else if (
				hasFlag(attackFlags, ATTACK_FLAGS.MEDIUM) &&
				hasFlag(hitState, HIT_FLAGS.LOW)
			) {
				slideAmount = CONSTANTS.DEFAULT_MEDIUM_HRSLIDE;
				move = this.Moves[this.MoveNdx.SML];
			} else if (
				hasFlag(attackFlags, ATTACK_FLAGS.MEDIUM) &&
				hasFlag(hitState, HIT_FLAGS.HIGH)
			) {
				slideAmount = CONSTANTS.DEFAULT_MEDIUM_HRSLIDE;
				move = this.Moves[this.MoveNdx.SMH];
			} else if (
				hasFlag(attackFlags, ATTACK_FLAGS.HARD) &&
				hasFlag(hitState, HIT_FLAGS.LOW)
			) {
				slideAmount = CONSTANTS.DEFAULT_HARD_HRSLIDE;
				move = this.Moves[this.MoveNdx.SHL];
			} else if (
				hasFlag(attackFlags, ATTACK_FLAGS.HARD) &&
				hasFlag(hitState, HIT_FLAGS.HIGH)
			) {
				slideAmount = CONSTANTS.DEFAULT_HARD_HRSLIDE;
				move = this.Moves[this.MoveNdx.SHH];
			}
		}
	}
	...
	else relAttackDirection = this.getAttackDirection(attackDirection);
	...
	if (this.isDead() && !this.IsLosing) {
		...
		if (!!this.isBeingGrappled()) {
			...
			if (this.Flags.Player.has(PLAYER_FLAGS.BLUE_FIRE)) {
				...
				hasFlag(attackFlags, ATTACK_FLAGS.RED_FIRE_NO_SOUND)
			) {
				enemyHitStop = CONSTANTS.DEFAULT_FIRE_HITSTOP;
				ignoreDeadAnimation = true;
				attackDirection = this.getRelativeDirection(attackDirection);
		...
		if (
			hasFlag(flags, ATTACK_FLAGS.SUPER) ||
			hasFlag(attackFlags, ATTACK_FLAGS.SUPER)
		)
			announcer_.runSuperComboFinish();
		else this.queueHitSound(hitSound);
	...
	if (this.getDizzyValue() >= this.MaxDizzyValue && !this.isDizzy()) {
		...
		if (!!isProjectile) this.queueBlockProjectileSound();
		else this.queueBlockSound();
		...
		!!otherParams.HitReact &&
		((hasFlag(otherParams.HitReact, HIT_REACT.AIRBORNE_EJECT) &&
			this.isAirborne()) ||
		...
		hasFlag(attackFlags, ATTACK_FLAGS.FLOOR_AIRBORNE_HARD) &&
		!!this.isAirborne()
	) {
		attackDirection = this.getRelativeDirection(attackDirection);
		...
		(hasFlag(attackFlags, ATTACK_FLAGS.FLOOR_AIRBORNE) && !!this.isAirborne())
	) {
		attackDirection = this.getRelativeDirection(attackDirection);
		...
		if (
			otherPlayer.isAirborne() &&
			!this.isAirborne() &&
			!otherParams.ForceHitFx
		)
			fx *= 0.2;
	...
	if (
		...
		(hasFlag(attackFlags, ATTACK_FLAGS.TRIP) ||
			...
			(hasFlag(attackFlags, ATTACK_FLAGS.FLOOR_AIRBORNE_HARD) &&
				!!this.isAirborne()) ||
			hasFlag(attackFlags, ATTACK_FLAGS.KNOCKDOWN) ||
			(hasFlag(attackFlags, ATTACK_FLAGS.FLOOR_AIRBORNE) &&
				!!this.isAirborne()))
	) {
...
Player.prototype.forceLose = function (attackDirection, ignoreAnimation) {
	ignoreAnimation =
		ignoreAnimation || this.DeadContext.Reason === DEAD_REASON.BIG_DROP;
	if (!this.IsLosing && !this.HandledDead) {
		...
		if (this.Ai.isRunning()) this.Ai.reset();
...
Player.prototype.getDizzy = function (
	attackFlags,
	hitState,
	flags,
	frame,
...
/*Player gets tripped*/
Player.prototype.takeTrip = function (
	attackFlags,
	hitState,
	flags,
	frame,
...
Player.prototype.eject = function (
	attackFlags,
	hitState,
	flags,
	frame,
	...
	if (!!move) {
		...
		if (
			!!otherParams &&
			!hasFlag(otherParams.RCombatFlags, RCOMBAT_FLAGS.IGNORE_CLEAR_DIZZY)
		)
			this.stopGettingDizzy();
		var direction = this.getAttackDirection(attackDirection);
		if (
			!(
				!!otherParams &&
				!!otherParams.UseCurrentJump &&
				this.CurrentAnimation.Animation.BaseAnimation.Name ==
					move.BaseAnimation.Name
			)
		)
			this.setCurrentAnimation({
				Animation: move,
...
Player.prototype.blueKnockDown = function (
	attackFlags,
	hitState,
	flags,
	frame,
	...
	if (!!move) {
		...
		if (!ignoreSound) this.queueLightFireSound();
	}
};
/*Player turns red and gets knocked down*/
Player.prototype.redKnockDown = function (
	attackFlags,
	hitState,
	flags,
	frame,
	...
	if (!!move) {
		...
		if (!ignoreSound) {
			if (!!otherParams && !!otherParams.UseHardFire) this.queueHardFireSound();
			else this.queueLightFireSound();
		}
	}
};
/*Player takes a hit while in the air*/
Player.prototype.takeAirborneHit = function (
	attackFlags,
	hitState,
	flags,
	frame,
...
Player.prototype.slideBack = function (
	...
	if (x < CONSTANTS.SLIDE_BACK_RANGE_FAR) {
		...
		if (hasFlag(attackFlags, ATTACK_FLAGS.LIGHT)) {
			slideAmount *= CONSTANTS.LIGHT_SLIDE_BACK_RATE;
		} else if (hasFlag(attackFlags, ATTACK_FLAGS.MEDIUM)) {
			slideAmount *= CONSTANTS.MEDIUM_SLIDE_BACK_RATE;
		} else if (hasFlag(attackFlags, ATTACK_FLAGS.HARD)) {
			slideAmount *= CONSTANTS.HARD_SLIDE_BACK_RATE;
		}
		...
		return this.startSlide(
			...
			slideAmount,
			-this.Direction,
			this.CurrentFrame.SlideFactor || 1,
			true,
		);
	}
	return 0;
...
Player.prototype.setGiveHit = function (
	attackFlags,
	hitStop,
	energyToAdd,
	behaviorFlags,
...
Player.prototype.giveHit = function (
	frame,
	attackFlags,
	hitStop,
	energyToAdd,
	...
	if (!!this.CurrentAnimation.Animation.ChainOnHitAnimation) {
		...
		if (!!this.Flags.Player.has(PLAYER_FLAGS.BULLDOZE)) this.setHoldFrame(0);
		else this.setHoldFrame(hitStop);
	}
...
Player.prototype.isGrappling = function (id) {
	if (!!id) return this.GrappledPlayerId == id;
	else return !!this.GrappledPlayerId;
};
...
Player.prototype.setLastHit = function (animation, type, offsetX, offsetY) {
	...
	switch (type) {
		...
		case CONSTANTS.USE_PLAYER_BOTTOM: {
			animation.InitialX =
				this.getX() + this.Width / Math.ceil(Math.random() * 8);
...
Player.prototype.checkFallingDamage = function (frame) {
	...
	var damage =
		this.peakY > CONSTANTS.FALL_DAMAGE_HEIGHT
