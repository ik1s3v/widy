var CreateMbisonAI = function (player) {
	...
	MBisonAI.prototype.initCombos = function () {
		...
		this.AI.VeryCloseCombos = [
			[
				{ A: 0, B: "get_close", C: CONSTANTS.GRAPPLE_DISTANCE + 30, D: -999 },
				{ A: 0, B: "t1" },
			],
			[
				{ A: 0, B: "get_close", C: -100 },
				{ A: 0, B: "k1" },
				{ A: 10, B: "k3" },
				{ A: 30, B: "k2" },
				{ A: 25, B: "lk3" },
		...
		this.AI.CounterAttackCombos = [[{ A: 0, B: "tb" }], [{ A: 0, B: "tf" }]];
	...
	MBisonAI.prototype.getForwardKey = function () {
		this.AI.Player.MustChangeDirection ? BUTTONS.BACK : BUTTONS.FORWARD;
	};
	MBisonAI.prototype.wanderForward = function (nbFrames) {
	...
	MBisonAI.prototype.wanderBackward = function (nbFrames) {
    ...
    */
	...
	MBisonAI.prototype.onEnemyAttackPending = function (
		frame,
		x,
		y,
		player,
	...
	MBisonAI.prototype.onEnemyProjectilePending = function (
		frame,
		x,
		y,
		player,
	...
	MBisonAI.prototype.onEnemyProjectileMoved = function (
		frame,
		id,
		x,
		y,
	...
	MBisonAI.prototype.onEnemyContinueAttack = function (
		frame,
		attacker,
		hitPoints,
	) {
	...
	MBisonAI.prototype.onEndAnimation = function (name) {
		...
		if (this.AI.isTurnBusy()) this.AI.clearTurnBusy();
	};

	MBisonAI.prototype.reactAirborne = function (
		frame,
		attacker,
		isEnemyVulernerable,
		x,
		...
		if (!attacker.isAirborne()) return false;

		if (!this.AI.AllowOverrideBlock) return true;

		if (!this.AI.Player.isBlocking()) {
			if (!!this.AI.isAirborneReactBusy()) return true;
	...
	MBisonAI.prototype.reactNotAirborne = function (
		frame,
		attacker,
		isEnemyVulernerable,
		x,
	...
	MBisonAI.prototype.doMove = function (key) {
		this.execute(this.AI.SingleMoves[key]);
	};
	MBisonAI.prototype.doCloseCombo = function (key) {
		this.execute(
			this.AI.CloseCombos[
	...
	MBisonAI.prototype.doVeryCloseCombo = function (key) {
		this.execute(
			this.AI.VeryCloseCombos[
	...
	MBisonAI.prototype.doJumpInCombo = function (key) {
		this.execute(this.AI.JumpInCombos[key]);
	};
	MBisonAI.prototype.doCloseJumpInCounterCombo = function (key) {
		this.execute(this.AI.CounterCloseJumpInCombos[key]);
	};
	MBisonAI.prototype.doCounterProjectileCombo = function (key) {
		this.execute(
			this.AI.CounterProjectileCombos[
	...
	MBisonAI.prototype.doCounterProjectileMovedCombo = function (key) {
		this.execute(
			this.AI.CounterProjectileMovedCombos[
				getRand(this.AI.CounterProjectileMovedCombos.length - 1)
			],
		);
	};
	MBisonAI.prototype.doCounterCloseAttackCombo = function (key) {
		this.execute(
			this.AI.CounterAttackCombos[
				getRand(this.AI.CounterAttackCombos.length - 1)
			],
		);
	};
	MBisonAI.prototype.doCounterAirborneCombo = function (key) {
		this.execute(
			this.AI.ReactAirborneCombos[
	...
	MBisonAI.prototype.doCounterNotAirborneCombo = function (key) {
		this.execute(
			this.AI.ReactNotAirborneCombos[
	...
	MBisonAI.prototype.doEvadeAttack = function (key) {
		this.execute(this.AI.EvadeAttack[getRand(this.AI.EvadeAttack.length - 1)]);
	};
	...
	MBisonAI.prototype.execute = function (sequence) {
		...
		for (var i = 0; i < sequence.length; ++i) {
			...
			autoCombo = sequence[i].AC === undefined ? autoCombo : sequence[i].AC;

			mustHit = sequence[i].H === undefined ? mustHit : sequence[i].H;
			...
			switch (sequence[i].B) {
				...
				case "pc1": {
					move = "psycho crusher p1";
					flags = FLAGS.EXECUTE_MOVE;
					break;
				}
				case "pc2": {
					move = "psycho crusher p2";
					flags = FLAGS.EXECUTE_MOVE;
					break;
				}
				case "pc3": {
					move = "psycho crusher p3";
					flags = FLAGS.EXECUTE_MOVE;
					break;
				}
				case "hs": {
				...
				case "tf": {
					input = teleportFarInput_;
					break;
				}
				case "tm": {
					input = teleportMiddleInput_;
					break;
				}
				case "ti": {
					input = teleportFrontInput_;
					break;
				}
				case "tb": {
					input = teleportBehindInput_;
					break;
				}
				case "t1": {
	...
	MBisonAI.prototype.react = function (
		frame,
		attacker,
		isEnemyVulernerable,
		x,
	...
	MBisonAI.prototype.proact = function (frame) {
		...
		if (
			!this.AI.Player.isFacingPlayer(item.Player, true) &&
			!this.AI.isTurnBusy()
		) {
			this.AI.setTurnBusy();
		...
		if (!!this.AI.isBusy() || this.AI.Actions.length > 0) {
			...
			this.AI.reset();
			if (rnd > 50) this.wanderBackward(25);
			else {
				this.AI.reset();
				this.doCloseCombo();
				this.AI.setBusy();
