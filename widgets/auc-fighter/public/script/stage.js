...
var CreateStage = function (bg0XOffset) {
	...
	var Stage = function () {
		this.BgImg0 = {
			xOffset: 0,
			element: window.document.getElementById("bg0"),
		};
	...
	Stage.prototype.getMatch = function () {
		return game_.getMatch();
	};
	Stage.prototype.getCDHelper = function () {
	...
	Stage.prototype.loadAssets = function () {
		...
		stuffLoader_.queue(
			this.Params.Name.toLowerCase() + "-theme.js",
	...
	Stage.prototype.getDeltaY = function () {
		return this.getGroundY() - this.LastGroundY;
	};
	Stage.prototype.getDeltaX = function () {
	...
	Stage.prototype.render = function (frame) {
		...
		this.BgImg0.element.style.top =
			this.BgImg0.yOffset - this.BgYScrollRate * this.getOffsetY() + "px";
		this.BgImg1.element.style.top =
			this.BgImg1.yOffset - this.getOffsetY() + "px";
	...
	Stage.prototype.requestScrollY = function (isUp, y, forceDown) {
		if (
			this.IsIncrementingY === undefined ||
			!this.IsIncrementingY ||
			!!forceDown
		) {
			y = y || 0;
			if (!!isUp && y > STAGE.VERT_SCROLL_Y) this.IsIncrementingY = true;
			else if (!isUp) this.IsIncrementingY = false;
		}
	};
	...
	Stage.prototype.handleYIncrement = function () {
		if (this.IsIncrementingY !== undefined) {
			...
			if (!!this.IsIncrementingY) {
				...
				if (!this.YCount) this.IsIncrementingY = undefined;
			}
	...
	Stage.prototype.scrollX = function (amount, p1, p2, match, dontOverrideSign) {
        ...
        */
		...
		var areBothPlayersInRightThreshold =
			isP1InRightThreshold && isP2InRightThreshold;
		var areBothPlayersInLeftThreshold =
			isP1InLeftThreshold && isP2InLeftThreshold;

		var isP1InThreshold =
			p1NewMidX >= CONSTANTS.MOVEMENT_THRESHOLD_LEFT &&
			p1NewMidX <= CONSTANTS.MOVEMENT_THRESHOLD_RIGHT;
		var isP2InThreshold =
			p2NewMidX >= CONSTANTS.MOVEMENT_THRESHOLD_LEFT &&
			p2NewMidX <= CONSTANTS.MOVEMENT_THRESHOLD_RIGHT;
		...
		var rightCornerGap = !isLeftPlayer
			? STAGE.MAX_STAGEX - p1RightX
			: STAGE.MAX_STAGEX - p2RightX;
		...
		var isP1InAnyThreshold =
			isP1InThreshold ||
			(isStageLeftCornered && isP1InLeftThreshold) ||
			(isStageRightCornered && isP1InRightThreshold);
		var isMovingBackwards = !(
			(p1.Direction == -1 && amount > 0) ||
			(p1.Direction == 1 && amount < 0)
		);
		var canIncreaseDeltaX =
			p1.jumpedOverAPlayer() &&
			this.getCDHelper().isWithinDistanceX(p1, p2, CONSTANTS.SO_CLOSE);
		...
		/*if the stage is left cornered, and the cornered player has moved far enough, and one of the players is beyond one of the right threshold, then the stage can move*/
			isStageLeftCornered &&
			hasLargerLeftGap &&
			!areBothPlayersInRightThreshold
		) {
			moveXHelper(
				this,
				amount,
		...
		/*if the stage is right cornered, and the cornered player has moved far enough, and one of the players is beyond one of the left threshold, then the stage can move*/
			isStageRightCornered &&
			hasLargerRightGap &&
			!areBothPlayersInLeftThreshold
		) {
			moveXHelper(
				this,
				amount,
	...
	Stage.prototype.init = function () {
		...
		this.MaxRight =
			img1Left - (this.BgImg1.element.width - STAGE.MAX_STAGEX) - img1Left;
        ...
        */
		...
		var bgImg0RightGap =
			parseInt(this.BgImg0.element.width) +
			parseInt(this.BgImg0.element.style.left) -
			w;
		var bgImg1RightGap =
			parseInt(this.BgImg1.element.width) +
			parseInt(this.BgImg1.element.style.left) -
			w;
	...
	Stage.prototype._MoveX = function (amount, dontAlignPlayers, px) {
		...
		if (this.X1 < this.MaxRight) {
			...
			this.X1 = this.MaxRight;
			!dontAlignPlayers ? (this.DeltaX = 0) : (this.DeltaX = 0 - this.X);
			this.X = 0;
		}
		if (!dontAlignPlayers) this.alignPlayersX();
		return this.DeltaX;
	};
