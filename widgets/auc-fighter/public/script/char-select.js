var CreateCharSelect = function (users) {
	...
	function determineTeams() {
		...
		if (
			users[0].isRequestingCharSelect() ||
			users[0].hasSelectedPlayer() ||
			users[0].isInCharSelect()
		) {
			teamA_.push(0);
			users[0].ShowSelectIcon = true;
			if (!!users[0].IsRequestingCharSelect) {
				for (var i = 2; i < users.length; ++i)
					if (users[i].Team == 1) users[i].reset();
			}
		}
		if (
			users[1].isRequestingCharSelect() ||
			users[1].hasSelectedPlayer() ||
			users[1].isInCharSelect()
		) {
			teamB_.push(1);
			users[1].ShowSelectIcon = true;
			if (users[1].isRequestingCharSelect())
				for (var i = 2; i < users.length; ++i)
					if (users[i].Team == 2) users[i].reset();
		}

		for (var i = 2; i < users.length; ++i) {
			if (!!users[i].IsAI) users[i].reset();
			if (
				users[i].Team == 1 &&
				!users[i].IsAI &&
				!users[0].isRequestingCharSelect()
	...
	var isU1Human = function () {
		return !!u1_ && !u1_.isAI();
	};
	var isU2Human = function () {
		return !!u2_ && !u2_.isAI();
	};
	...
	CharSelect.prototype.getDelayAfterSelect = function () {
		return delayAfterSelect_;
	};
	CharSelect.prototype.getUser1 = function () {
		return !!u1_ && (!!u1_.IsInCharSelect || u1_.IsInStoryMode) ? u1_ : null;
	};
	CharSelect.prototype.getUser2 = function () {
		return !!u2_ && (!!u2_.IsInCharSelect || u2_.IsInStoryMode) ? u2_ : null;
	};
	CharSelect.prototype.getPlayers = function (users) {
		var retVal = [];

		for (var i = 0; i < users.length; ++i) retVal.push(users[i].getPlayer());
	...
	CharSelect.prototype.setupAnimations = function () {
		animations_ = {};
		animations_["show_next_foe"] = new BasicAnimation(
			"show_next_foe",
			[],
			false,
		);
		animations_["show_next_foe"].addFrame(
			this,
			"images/misc/misc/question-0.png",
			8,
		);
		animations_["show_next_foe"]
			.addFrame(this, "images/misc/misc/question-1.png", 8)
			.set({ X: 10 });
		animations_["show_next_foe"]
			.addFrame(this, "images/misc/misc/question-2.png", 8)
			.set({ X: 25 });
		animations_["show_next_foe"]
			.addFrame(this, "images/misc/misc/question-3.png", 8)
			.set({ X: 23 });
		animations_["show_next_foe"]
			.addFrame(this, "images/misc/misc/question-4.png", 8)
			.set({ X: 10 });
	...
	CharSelect.prototype.getRow = function (user) {
		...
		if (
			user.Selected <= this.CharsRow1.Max &&
			user.Selected >= this.CharsRow1.Min
		)
			return CONSTANTS.ROW1;
		else if (
			user.Selected <= this.CharsRow2.Max &&
			user.Selected >= this.CharsRow2.Min
		)
			return CONSTANTS.ROW2;
		// if((user.Selected <= this.CharsRow3.Max) && (user.Selected >= this.CharsRow3.Min))
		else return CONSTANTS.ROW3;
	};
	...
	CharSelect.prototype.getColumn = function (user) {
		...
		if (-1 < this.CharsCol1.indexOf(user.Selected)) return CONSTANTS.COL1;
		else if (-1 < this.CharsCol2.indexOf(user.Selected)) return CONSTANTS.COL2;
		else if (-1 < this.CharsCol3.indexOf(user.Selected)) return CONSTANTS.COL3;
		else if (-1 < this.CharsCol4.indexOf(user.Selected)) return CONSTANTS.COL4;
		else if (user.PlayerNdx == 1) return CONSTANTS.COL1;
		else if (user.PlayerNdx == 2) return CONSTANTS.COL4;
	};
	...
	CharSelect.prototype.tryChangeCharacter = function (who, direction) {
		...
		if (direction == CONSTANTS.DOWN) {
			...
			if (
				!(
					row == CONSTANTS.ROW2 &&
					((isUser1 && col == CONSTANTS.COL4) ||
						(isUser2 && col == CONSTANTS.COL1))
				)
			)
			...
			switch (row) {
				...
				case CONSTANTS.ROW3: {
					//ensure that the player can only go to its own random select
					if (direction == CONSTANTS.RIGHT) {
						if (isUser1 && col == CONSTANTS.COL3) break;

						col = Math.min(col + 1, CONSTANTS.COL4);
					} else if (direction == CONSTANTS.LEFT) {
						if (isUser2 && col == CONSTANTS.COL2) break;
		...
		if (tmpSelected == 8 && who.Selected >= 11) {
			...
			if (tmpRow != row || tmpCol != col) {
				if (isUser1) this.queueUser1MoveSound();
				else this.queueUser2MoveSound();
			}
	...
	CharSelect.prototype.init = function () {
		if (!initialized_) {
			...
			spriteLookup_.set(
				this.CharSelectElement,
				"images/misc/misc/char-select.png",
				true,
			);
			...
			spriteLookup_.set(
				this.PlayerSelectImg,
				"images/misc/misc/player-select.png",
				true,
			);
			...
			spriteLookup_.set(
				parentElement,
				"images/misc/misc/player-select-back-bg.png",
				true,
			);
	...
	CharSelect.prototype.checkFor1PMode = function () {
		...
		if (hasP1 && hasP2) {
			...
			u1_.enableStoryMode();
			if (!!u2_) u2_.disableStoryMode();
		} else if (hasP2) {
			u2_.enableStoryMode();
			if (!!u1_) u1_.disableStoryMode();
		}
	};
	...
	CharSelect.prototype.enableUser = function (user, slot, otherTeam) {
		...
		if (user.isRequestingCharSelect()) user.useCredit();

		var changeCharacterFn = function (thisValue) {
			return function (direction) {
				return thisValue.tryChangeCharacter(this, direction);
			};
		};
		var getOtherCharacterFn = function (thisValue) {
			return function (direction) {
				return !!thisValue
					? thisValue.IsCharSelected
						? thisValue.getName()
						: ""
					: "";
			};
		...
		var isCharOnOtherTeamFn = function (thisValue, otherTeam) {
			return function (direction) {
				return otherTeam.some(function (a) {
					return (
						users[a].isCharSelected() &&
						users[a].getName() == thisValue.getName()
					);
				});
			};
		};
	...
	CharSelect.prototype.getTeamA = function () {
		if (teamA_.length == 0) teamA_ = this.getAiTeam(teamB_);
		return teamA_;
	};
	CharSelect.prototype.getTeamB = function () {
		if (teamB_.length == 0) teamB_ = this.getAiTeam(teamA_);
		return teamB_;
	};
	...
	CharSelect.prototype.addAiUser = function (team, otherTeam, ch) {
		...
		var isAlternate =
			otherTeam.some(function (a) {
				return users[a].Selected == ch && !users[a].isAlternateChar();
			}) ||
			team.some(function (a) {
				return users[a].Selected == ch && !users[a].isAlternateChar();
			});
		...
		if (users[team[0]].TeamMates.length > 0) this.addAiUser(team, otherTeam);
	};
	...
	CharSelect.prototype.getAiTeam = function (otherTeam) {
		...
		for (var i = 0; i < opponents.length; ++i) {
			...
			var isAlternate = otherTeam.some(function (a) {
				return users[a].Selected == ch && !users[a].isAlternateChar();
			});
	...
	CharSelect.prototype.check = function (frame) {
		if (
			(!this.getUser1() || u1_.IsCharSelected) &&
			(!this.getUser2() || u2_.IsCharSelected)
		) {
			this.IsDone = true;
	...
	CharSelect.prototype.queueUser1MoveSound = function (value) {
		soundManager_.queueSound("audio/misc/p-select-move-0.zzz");
	};
	CharSelect.prototype.queueUser1ChooseSound = function (value) {
		soundManager_.queueSound("audio/misc/p-select-choose-0.zzz");
	};
	CharSelect.prototype.queueUser2MoveSound = function (value) {
		soundManager_.queueSound("audio/misc/p-select-move-1.zzz");
	};
	CharSelect.prototype.queueUser2ChooseSound = function (value) {
		soundManager_.queueSound("audio/misc/p-select-choose-0.zzz");
	};
	...
	CharSelect.prototype.showNextFoe = function (who, boxIndex) {
		boxIndex = boxIndex || 0;
		if (!!this.getUser1()) u1_.charSelectElementsVisible(false);
		if (!!this.getUser2()) u2_.charSelectElementsVisible(false);
		showNextFoe_ = true;
		...
		var checkDefeatedOpponents = function (u) {
			for (var i = 0; i < CONSTANTS.MAX_STORY_MODE_LEVEL; ++i) {
				...
				if (i < 4)
					spriteLookup_.set(
						this.NextFoeLine1Element.children[i].firstChild,
						src[0],
					);
				else
					spriteLookup_.set(
						this.NextFoeLine2Element.children[i - 4].firstChild,
						src[0],
					);
			}
		...
		switch (who) {
			case CHARACTERS.KEN: {
				animations_["show_next_foe"].BaseAnimation.Frames[5].RightSrc =
					"images/misc/misc/char-ken-r.png";
				break;
			}
			case CHARACTERS.RYU: {
				animations_["show_next_foe"].BaseAnimation.Frames[5].RightSrc =
					"images/misc/misc/char-ryu-r.png";
				break;
			}
			case CHARACTERS.CHUNLI: {
				animations_["show_next_foe"].BaseAnimation.Frames[5].RightSrc =
					"images/misc/misc/char-chunli-r.png";
				break;
			}
			case CHARACTERS.CHARLIE: {
				animations_["show_next_foe"].BaseAnimation.Frames[5].RightSrc =
					"images/misc/misc/char-charlie-r.png";
				break;
			}
			case CHARACTERS.GUY: {
				animations_["show_next_foe"].BaseAnimation.Frames[5].RightSrc =
					"images/misc/misc/char-guy-r.png";
				break;
			}
			case CHARACTERS.BIRDIE: {
				animations_["show_next_foe"].BaseAnimation.Frames[5].RightSrc =
					"images/misc/misc/char-birdie-r.png";
				break;
			}
			case CHARACTERS.SODOM: {
				animations_["show_next_foe"].BaseAnimation.Frames[5].RightSrc =
					"images/misc/misc/char-sodom-r.png";
				break;
			}
			case CHARACTERS.ADON: {
				animations_["show_next_foe"].BaseAnimation.Frames[5].RightSrc =
					"images/misc/misc/char-adon-r.png";
				break;
			}
			case CHARACTERS.ROSE: {
				animations_["show_next_foe"].BaseAnimation.Frames[5].RightSrc =
					"images/misc/misc/char-rose-r.png";
				break;
			}
			case CHARACTERS.SAGAT: {
				animations_["show_next_foe"].BaseAnimation.Frames[5].RightSrc =
					"images/misc/misc/char-sagat-r.png";
				break;
			}
			case CHARACTERS.MBISON: {
				animations_["show_next_foe"].BaseAnimation.Frames[5].RightSrc =
					"images/misc/misc/char-mbison-r.png";
				break;
			}
			case CHARACTERS.AKUMA: {
				animations_["show_next_foe"].BaseAnimation.Frames[5].RightSrc =
					"images/misc/misc/char-akuma-r.png";
				break;
			}
			case CHARACTERS.DAN: {
				animations_["show_next_foe"].BaseAnimation.Frames[5].RightSrc =
					"images/misc/misc/char-dan-r.png";
				break;
			}
			default: {
				animations_["show_next_foe"].BaseAnimation.Frames[5].RightSrc =
	...
	CharSelect.prototype.frameMove = function (frame) {
		...
		if (!!this.getUser1()) u1_.frameMove(frame);
		if (!!this.getUser2()) u2_.frameMove(frame);
	};
	...
	CharSelect.prototype.render = function (frame) {
		...
		if (!!this.getUser1()) u1_.render(frame);
		if (!!this.getUser2()) u2_.render(frame);
	};
