...
var CreateGame = function () {
	...
	var pnlLoadingProgress_ =
		window.document.getElementById("pnlLoadingProgress");
	...
	Game.prototype.isTeamMode = function () {
		return enableTeamMode_;
	};

	Game.prototype.setTeamMode = function (value) {
		enableTeamMode_ = value;
	};

	Game.prototype.onMatchEnd = function () {};
	...
	Game.prototype.gameLoopState = function () {
		return gameLoopState_;
	};

	Game.prototype.getMaxWinsPerMatch = function () {
		return maxWinsPerMatch_;
	};
	...
	Game.prototype.getCharSelect = function () {
		return charSelect_;
	};

	Game.prototype.getGlobalDamageMultiplier = function () {
		if (!!enableTeamMode_) return CONSTANTS.TEAMMODE_DAMAGE_MULTIPLIER;
		return 1;
	};
	...
	Game.prototype.getCurrentTime = function () {
		...
		else return new Date() - new Date(0);
	};
	...
	Game.prototype.onStageImagesLoaded = function () {
		if (!!match_) match_.getStage().init();
	};
	...
	Game.prototype.resetKeys = function () {
		keyboardState_ = {};
		if (!!managed_) managed_.resetKeys();
	};
		...
		if (!isInitialized_) {
			...
			if (!!window.document.attachEvent) {
				window.document.attachEvent(
					"onkeydown",
					getKeyPressHandler(this, true),
					true,
				);
				window.document.attachEvent(
					"onkeyup",
					getKeyPressHandler(this, false),
					true,
				);
	...
	Game.prototype.startMatch = function (
		...
		var teamA = teamAIndexes.map(function (i) {
			return users_[i];
		});
		var teamB = teamBIndexes.map(function (i) {
			return users_[i];
		});
	...
	Game.prototype.getSecondUserForCharSelect = function () {
		for (var i = 1; i < users_.length; ++i)
			if (users_[i].isRequestingCharSelect()) return users_[i];
		return users_[1];
	};
	...
	Game.prototype.startLoading = function (loopFn, callback) {
		...
		var createClosure = function (thisRef, fn1, fn2) {
			return function () {
				thisRef.onDoneLoading(fn1, fn2);
			};
		};

		stuffLoader_.start(
			this.onProgress,
			createClosure(this, loopFn, callback),
			this,
		);
	};
	...
	Game.prototype.onProgress = function (nbRemaining) {
		if (!nbRemaining) pnlLoading_.innerHTML = "done";
		else pnlLoading_.innerHTML = nbRemaining;
	};
	...
	Game.prototype.resetGameData = function () {
		...
		announcer_.release();
		this.resetSpeed();
		if (!!charSelect_) charSelect_.release();
		if (!!match_) match_.release();
		if (!!insertCoinScreen_) insertCoinScreen_.release();
	};
	...
	Game.prototype.isPaused = function () {
		return this.hasState(GAME_STATES.PAUSED);
	};
	...
	Game.prototype.quickPause = function () {
		...
		if (!!this.hasState(GAME_STATES.AUDIO_LOADING)) {
			window.document.getElementById("spnState").innerHTML =
				'State: <span class="audio-loading">Loading...</span>';
			pnlStage_.style.opacity = 0;
		} else {
			window.document.getElementById("spnState").innerHTML =
				"State: <span>Frame by frame</span>";
		}
	...
	Game.prototype.handleGamePadButtonPresses = function () {
		if (!!this.UsingGamepads) {
			for (var i = 0, length = users_.length; i < length; ++i) {
				...
				if (!!pad) {
					this.handleGamePadHelper(
						pad.buttons[users_[i].jm.CROUCH].pressed,
						users_[i].jm.CROUCH,
					);
					this.handleGamePadHelper(
						pad.buttons[users_[i].jm.LEFT].pressed,
						users_[i].jm.LEFT,
					);
					this.handleGamePadHelper(
						pad.buttons[users_[i].jm.RIGHT].pressed,
						users_[i].jm.RIGHT,
					);
					this.handleGamePadHelper(
						pad.buttons[users_[i].jm.JUMP].pressed,
						users_[i].jm.JUMP,
					);
					this.handleGamePadHelper(
						pad.buttons[users_[i].jm.HK].pressed,
						users_[i].jm.HK,
					);
					this.handleGamePadHelper(
						pad.buttons[users_[i].jm.MK].pressed,
						users_[i].jm.MK,
					);
					this.handleGamePadHelper(
						pad.buttons[users_[i].jm.MP].pressed,
						users_[i].jm.MP,
					);
					this.handleGamePadHelper(
						pad.buttons[users_[i].jm.HP].pressed,
						users_[i].jm.HP,
					);
					this.handleGamePadHelper(
						pad.buttons[users_[i].jm.LP].pressed,
						users_[i].jm.LP,
					);
					this.handleGamePadHelper(
						pad.buttons[users_[i].jm.LK].pressed,
						users_[i].jm.LK,
					);
					this.handleGamePadHelper(
						pad.buttons[users_[i].jm.START].pressed,
						users_[i].jm.START,
					);
					this.handleGamePadHelper(
						pad.buttons[users_[i].jm.SELECT].pressed,
						users_[i].jm.SELECT,
					);
	...
	Game.prototype.handleKeyPress = function (e, isDown) {
		...
		if (this.wasKeyPressed(KEYS.EIGHT, keyCode, isDown)) this.speedUp();
		if (this.wasKeyPressed(KEYS.NINE, keyCode, isDown)) this.slowDown();
		...
		if (isDown) {
			...
			else buttonState_["_" + keyCode] = BUTTON_STATE.STILL_PRESSED;
		} else {
			buttonState_["_" + keyCode] = BUTTON_STATE.JUST_RELEASED;
		}

		if (!!this.hasState(GAME_STATES.AUDIO_LOADING)) return;
	...
	Game.prototype.addUser1 = function (
		right,
		up,
		left,
		down,
	...
	Game.prototype.addUser2 = function (
		right,
		up,
		left,
		down,
	...
	Game.prototype.addGamepadUser = function (
		right,
		up,
		left,
		down,
	...
	Game.prototype.clearUsers = function () {
		...
		for (var i = 0; i < length; ++i) delete users_[i];
		users_ = [];
	};
	...
	Game.prototype.runGameLoop = function (start) {
		this.handleInput();
		if (
			!this.hasState(GAME_STATES.PAUSED) ||
			this.hasState(GAME_STATES.STEP_FRAME)
		) {
			...
			if (!match_.mustQuit()) {
				...
				if (!match_.isSuperMoveActive()) fontSystem_.frameMove(frame_);
				...
				if (!match_.isSuperMoveActive()) fontSystem_.render(frame_);
		...
		if (match_.mustQuit()) {
			...
			if (gameLoopState_ != GAME_STATES.MATCH) return;
	...
	Game.prototype.runInsertCoinScreenLoop = function () {
		...
		if (!insertCoinScreen_.isDone(frame_)) {
			if (gameLoopState_ != GAME_STATES.INSERT_COIN) return;
	...
	Game.prototype.runCharSelectLoop = function () {
		if (!!charSelect_) {
			...
			if (
				...
				if (
					!!charSelect_.IsDone &&
					frame_ >= charSelect_.getDelayAfterSelect()
				) {
					managed_ = null;
					this.startMatch(
						false,
						charSelect_.getTeamA(),
						charSelect_.getTeamB(),
						charSelect_.getStage(),
					);
				}
			...
			if (!this.isGameOver()) {
				if (gameLoopState_ != GAME_STATES.CHAR_SELECT) return;
