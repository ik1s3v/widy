var CreateGenericAI = function (player) {
	...
	var GenericAI = function () {
		...
		this.CLEARED_INPUT = [
			...
			{ IsDown: false, Button: BUTTONS.JUMP },
			{ IsDown: false, Button: BUTTONS.CROUCH },
			{ IsDown: false, Button: BUTTONS.FORWARD },
			{ IsDown: false, Button: BUTTONS.LIGHT_PUNCH },
			{ IsDown: false, Button: BUTTONS.MEDIUM_PUNCH },
			{ IsDown: false, Button: BUTTONS.HARD_PUNCH },
			{ IsDown: false, Button: BUTTONS.LIGHT_KICK },
			{ IsDown: false, Button: BUTTONS.MEDIUM_KICK },
			{ IsDown: false, Button: BUTTONS.HARD_KICK },
		];
	...
	GenericAI.prototype.release = function (
		frame,
		flags,
		input,
		name,
	...
	GenericAI.prototype.createAction = function (
		frame,
		flags,
		input,
		name,
	...
	GenericAI.prototype.createActionWithParams = function (params) {
		return {
			...
			MoveName: params.name || 0,
			Frame: params.frame || (params.autoCombo ? 1 : 0),
			Input: params.input || [],
			Distance: params.dist,
			IsActive: false,
			SetForceBusy: params.forceBusy,
			MustHit: params.forceBusy || params.mustHit,
			Move: params.move,
			AutoCombo: params.autoCombo,
			RequiredState: params.requiredState,
		};
	};

	GenericAI.prototype.getForwardKey = function () {
		this.Player.MustChangeDirection ? BUTTONS.BACK : BUTTONS.FORWARD;
	};
	GenericAI.prototype.getOtherInputCleared = function (button) {
		return this.CLEARED_INPUT.filter(function (a) {
			return a.Button != button;
		});
	};
	GenericAI.prototype.setAttackReactBusy = function () {
		this.AttackReactBusyFrame = game_.getCurrentFrame();
	};
	GenericAI.prototype.isAttackReactBusy = function (nbFrames) {
		return (
			this.isForceBusy() ||
			game_.getCurrentFrame() - this.AttackReactBusyFrame < (nbFrames || 20)
		);
	};
	GenericAI.prototype.setAirborneReactBusy = function () {
		this.AirborneReactBusyFrame = game_.getCurrentFrame();
	};
	GenericAI.prototype.isAirborneReactBusy = function (nbFrames) {
		return (
			this.isForceBusy() ||
			game_.getCurrentFrame() - this.AirborneReactBusyFrame < (nbFrames || 50)
		);
	};
	GenericAI.prototype.setWanderBusy = function () {
		this.WanderBusyFrame = game_.getCurrentFrame();
	};
	GenericAI.prototype.isWanderBusy = function (nbFrames) {
		return (
			this.isForceBusy() ||
			game_.getCurrentFrame() - this.WanderBusyFrame < (nbFrames || 50)
		);
	};
	GenericAI.prototype.setProactBusy = function () {
		this.ProactBusyFrame = game_.getCurrentFrame();
	};
	GenericAI.prototype.isProactBusy = function (nbFrames) {
		return (
			this.isForceBusy() ||
			game_.getCurrentFrame() - this.ProactBusyFrame < (nbFrames || 50)
		);
	};
	GenericAI.prototype.setProjectileReactBusy = function () {
		this.IgnoreProjectileGone = true;
		this.ProjectileReactBusyFrame = game_.getCurrentFrame();
	};
	GenericAI.prototype.isProjectileReactBusy = function (nbFrames) {
		return (
			this.isForceBusy() ||
			game_.getCurrentFrame() - this.ProjectileReactBusyFrame < (nbFrames || 50)
		);
	};
	GenericAI.prototype.setBusy = function () {
		this.BusyFrame = game_.getCurrentFrame();
	};
	GenericAI.prototype.isBusy = function (nbFrames) {
		return (
			this.isForceBusy() ||
			game_.getCurrentFrame() - this.ForceBusyFrame < (nbFrames || 50)
		);
	};
	GenericAI.prototype.setForceBusy = function () {
		this.ForceBusyFrame = game_.getCurrentFrame();
	};
	GenericAI.prototype.isForceBusy = function (nbFrames) {
		return game_.getCurrentFrame() - this.ForceBusyFrame < (nbFrames || 50);
	};
	GenericAI.prototype.clearTurnBusy = function () {
		this.TurnBusyFrame = 0;
	};
	GenericAI.prototype.setTurnBusy = function () {
		this.TurnBusyFrame = game_.getCurrentFrame();
	};
	GenericAI.prototype.isTurnBusy = function (nbFrames) {
		return game_.getCurrentFrame() - this.TurnBusyFrame < (nbFrames || 50);
	};
	...
	GenericAI.prototype.reset = function () {
		if (!!this.Actions && !!this.Actions.length) this.Actions = [];
		if (!!this.InputToSend && !!this.InputToSend.length) this.InputToSend = [];
	...
	GenericAI.prototype.getOtherTeam = function () {
		...
		else return player.getMatch().getTeamA().getPlayers();
	};

	GenericAI.prototype.getClosestEnemy = function (
		isAirborne,
		isVulernable,
		isMobile,
	) {
		...
		for (var i = 0; i < otherPlayers.length; ++i) {
			var temp = this.Player.getCDHelper().getImgDistanceX(
				this.Player,
				otherPlayers[i],
			);
			if (temp < retVal.X) {
				if (
					(isAirborne === undefined ||
						isAirborne == otherPlayers[i].isAirborne()) &&
					(isVulernable === undefined ||
						isVulernable == !otherPlayers[i].hasInvulnerableFlag()) &&
	...
	GenericAI.prototype.jumpInToEnemy = function (flags, dist, frame) {
		frame = frame || 0;
		this.Actions.push(
			this.createAction(frame, FLAGS.JUMP_IN | flags, [
				{ IsDown: true, Button: BUTTONS.FORWARD },
				{ IsDown: false, Button: BUTTONS.BACK },
			]),
		);
	...
	GenericAI.prototype.jumpTowardEnemy = function (flags, dist, frame) {
		frame = frame || 0;
		this.Actions.push(
			this.createAction(frame, FLAGS.CLEAR_INPUT, [
				{ IsDown: true, Button: BUTTONS.FORWARD },
				{ IsDown: false, Button: BUTTONS.BACK },
			]),
		);
	...
	GenericAI.prototype.parseAndSendInput = function () {
		...
		if (!!this.Actions[0].AutoCombo) {
			if (
				!!this.AutoCombo.isWaiting() ||
				(!this.Player.isMobile() && !this.Player.allowInterupt()) ||
				(!!this.Actions[0].RequiredState &&
					!this.Player.Flags.Pose.has(this.Actions[0].RequiredState))
			) {
				return null;
		...
		if (hasFlag(this.Actions[0].Flags, FLAGS.MOVE_TO_ENEMY)) {
			if (
				this.getClosestEnemy().X < (this.Actions[0].Distance || this.REAL_CLOSE)
			) {
				this.clearInput();
			} else {
				canClear = false;
			...
			if (this.Actions[0].Distance === undefined) {
				...
				if (
					this.getClosestEnemy().X <
					(this.Actions[0].Distance || this.REAL_CLOSE)
				) {
					this.clearInput();
			...
			if (
				this.getClosestEnemy().X <
				(this.Actions[0].Distance || this.JUMP_IN_DIST)
			) {
				this.clearInput();
			} else {
				canClear = false;
			...
			hasFlag(this.Actions[0].Flags, FLAGS.EXECUTE_MOVE) &&
			!!this.Actions[0].MoveName
		) {
		...
		if (!!canSendInput) this.Player.sendInput(this.Actions[0].Input);
		if (!!canClear) this.Actions.splice(0, 1);
		if (this.Actions.length == 0) this.IgnoreProjectileGone = false;

		return retVal;
	};

	GenericAI.prototype.sendInput = function (
		flags,
		frame,
		input,
		mustHit,
	...
	GenericAI.prototype.pressBlock = function () {
		var blockKey = this.Player.MustChangeDirection
			? BUTTONS.FORWARD
			: BUTTONS.BACK;
		this.pressButton(FLAGS.CLEAR_OTHER_INPUT, blockKey);
	};

	GenericAI.prototype.pressButton = function (
		flags,
		button,
		pressFrame,
		releaseFrame,
		...
		this.Actions.push(
			this.createAction(pressFrame, flags, [{ IsDown: true, Button: button }]),
		);
	...
	GenericAI.prototype.releaseButton = function (button, releaseFrame) {
		if (!!releaseFrame) {
			this.Actions.push(
				this.createAction(releaseFrame, null, [
					{ IsDown: false, Button: button },
				]),
			);
		}
	};
	...
	GenericAI.prototype.onProjectilePending = function (
		frame,
		x,
		y,
		player,
	...
	GenericAI.prototype.onEnemyProjectileGone = function (frame, id) {
		if (!this.IgnoreProjectileGone) this.reset();
	};

	//fired every frame an enemy projectile is active
	GenericAI.prototype.onEnemyProjectileMoved = function (
		frame,
		id,
		x,
		y,
		...
		if (!!this.isProjectileReactBusy()) return;
	...
	GenericAI.prototype.onEnemyContinueAttack = function (
		frame,
		attacker,
		hitPoints,
	) {
		if (!!this.isAttackReactBusy()) return;
		if (!!this.isAirborneReactBusy()) return;
		var blockKey = this.Player.MustChangeDirection
			? BUTTONS.FORWARD
			: BUTTONS.BACK;
		if (this.Player.canBlock()) {
			if (
				...
				(!this.Player.isKeyDown(BUTTONS.CROUCH) ||
					!this.Player.isKeyDown(blockKey))
			) {
				this.reset();
				this.AllowOverrideBlock = false;
				this.pressButton(FLAGS.NONE, BUTTONS.CROUCH);
	...
	GenericAI.prototype.process = function (frame) {
		...
		if (!!this.AllowOverrideBlock) {
			if (!player.isMobile()) {
				if (!!this.JustAttacked) this.nbMobileDelay = 3;
				return;
			}
