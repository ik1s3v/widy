...
BaseAnimation.prototype.addFrameWithSound = function (
	player,
	volume,
	soundFilename,
	shadowOffset,
...
/*Adds a frame to the move*/
BaseAnimation.prototype.addOffsetFrame = function (
	player,
	image,
	nbFrames,
	x,
...
/*Adds a frame to the move*/
BaseAnimation.prototype.addFrame = function (
	player,
	shadowOffset,
	shadowImage,
	image,
	...
	if (shadowImage == "" && !!player) shadowImage = player.DefaultShadowImageSrc;
	++player.NbFrames;
	this.Frames[this.Frames.length] = new Frame(
		this.Frames.length,
		player.getNextFrameID(),
		shadowOffset,
		shadowImage,
		image,
		nbFrames,
	...
	if (!!this.IsThrow) {
		currentFrame.FlagsToSet.Player =
			(currentFrame.FlagsToSet.Player || MISC_FLAGS.NONE) |
			PLAYER_FLAGS.INVULNERABLE;
	}
	...
	if (!!this.IsAttack) {
		...
		if (!!this.AllowAirBlock) flags = COMBAT_FLAGS.CAN_BE_AIR_BLOCKED;
		else flags = COMBAT_FLAGS.CAN_BE_BLOCKED;

		if (!this.SkipFrameBlockCheck) {
			currentFrame.FlagsToSet.Combat =
				(currentFrame.FlagsToSet.Combat || MISC_FLAGS.NONE) | flags;
			...
			currentFrame.FlagsToClear.Combat =
				(currentFrame.FlagsToClear.Combat || MISC_FLAGS.NONE) | flags;
		}
	}
};

/*Adds a frame multiple times, and adds the sound effect on the first frame only.*/
BaseAnimation.prototype.addRepeatingFrameWithSound = function (
	player,
	volume,
	soundFilename,
	shadowOffset,
...
BaseAnimation.prototype.addRepeatingFrame = function (
	player,
	shadowOffset,
	shadowImage,
	image,
...
BaseAnimation.prototype.getFrame = function (frameDelta) {
	var count = 0;
	for (var i = 0; i < this.Frames.length; ++i)
		if ((count += this.Frames[i].Frames) >= frameDelta) return this.Frames[i];
	return null;
};
...
BaseAnimation.prototype.getNextFrameOffset = function (id) {
	...
	for (var i = 0; i < this.Frames.length; ++i) {
		...
		if (this.Frames[i].ID == id) return count;
	}
	return 0;
};
...
/*Adds a frame to the move*/
BasicBaseAnimation.prototype.addFrame = function (
	owner,
	image,
	nbFrames,
	x,
