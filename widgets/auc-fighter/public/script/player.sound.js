...
Player.prototype.removeQueuedDizzyAudio = function () {
	soundManager_.stopAny("audio/misc/dizzy.zzz");
};
...
Player.prototype.queueSwingSound = function (value) {
	...
	else if (hasFlag(value, SWINGSOUND.SLIDE0))
		this.queueSound("audio/misc/slide-0.zzz");
};

Player.prototype.queueHitSound = function (value) {
	if (!!value.length) {
		for (var i = 0; i < value.length; ++i) this.queueHitSoundHelper(value[i]);
	} else {
...
Player.prototype.queueHitSoundHelper = function (value) {
	...
	else if (hasFlag(value, HITSOUND.MP))
		this.queueSound("audio/misc/hit-mp.zzz");
	else if (hasFlag(value, HITSOUND.LP))
		this.queueSound("audio/misc/hit-lp.zzz");
	else if (hasFlag(value, HITSOUND.HP3))
		this.queueSound("audio/misc/hit-hp-3.zzz");
	else if (hasFlag(value, HITSOUND.HK))
		this.queueSound("audio/misc/hit-hk.zzz");
	else if (hasFlag(value, HITSOUND.MK))
		this.queueSound("audio/misc/hit-mk.zzz");
	else if (hasFlag(value, HITSOUND.LK))
		this.queueSound("audio/misc/hit-lk.zzz");
	else if (hasFlag(value, HITSOUND.FIRE0))
		this.queueSound("audio/misc/fire-0.zzz");
	else if (hasFlag(value, HITSOUND.FIRE1))
		this.queueSound("audio/misc/fire-1.zzz");
};
...
Player.prototype.queueLightFireSound = function () {
	this.queueSound("audio/misc/fire-0.zzz");
};
Player.prototype.queueHardFireSound = function () {
	this.queueSound("audio/misc/fire-1.zzz");
};
