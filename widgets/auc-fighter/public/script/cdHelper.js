var CreateCDHelper = function () {
	...
	var getStage = function () {
		return getMatch().getStage();
	};

	var HasIntersection_ = function (a, b) {
		if (
			a.Flags.Player.has(PLAYER_FLAGS.IGNORE_COLLISIONS) ||
			b.Flags.Player.has(PLAYER_FLAGS.IGNORE_COLLISIONS) ||
			a.IgnoreCollisionsWith == b.Id ||
			b.IgnoreCollisionsWith == a.Id
		)
			return false;
		...
		var hasP1RightIntersection =
			rect.Right > otherRect.Left && rect.Right < otherRect.Right;
		var hasP1LeftIntersection =
			rect.Left > otherRect.Left && rect.Left < otherRect.Right;
		var hasP1TopIntersection =
			rect.Top > otherRect.Bottom + padding &&
			rect.Top < otherRect.Top - padding;
		var hasP1BottomIntersection =
			rect.Bottom > otherRect.Bottom + padding &&
			rect.Bottom < otherRect.Top - padding;
        ...
        */
		...
		retVal =
			(hasP1TopIntersection || hasP1BottomIntersection) &&
			(hasP1RightIntersection || hasP1LeftIntersection);
	...
	var GetPlayersAtPosition_ = function (rect, who, type) {
		...
		for (var i = 0, length = players.length; i < length; ++i) {
			if (
				who.Flags.Player.has(PLAYER_FLAGS.IGNORE_COLLISIONS) ||
				players[i].Flags.Player.has(PLAYER_FLAGS.IGNORE_COLLISIONS) ||
				who.IgnoreCollisionsWith == players[i].Id ||
				players[i].IgnoreCollisionsWith == who.Id
			)
				continue;
			...
			hasP1RightIntersection =
				rect.Right >= otherRect.Left && rect.Right <= otherRect.Right;
			hasP1ExistsingRightIntersection =
				!!rect.OldRight &&
				rect.OldRight >= otherRect.Left &&
				rect.OldRight <= otherRect.Right;
			hasP1LeftIntersection =
				rect.Left >= otherRect.Left && rect.Left <= otherRect.Right;
			hasP1ExistsingLeftIntersection =
				!!rect.OldLeft &&
				rect.OldLeft >= otherRect.Left &&
				rect.OldLeft <= otherRect.Right;
			hasP1TopIntersection =
				rect.Top >= otherRect.Bottom && rect.Top <= otherRect.Top;
			hasP1BottomIntersection =
				rect.Bottom >= otherRect.Bottom && rect.Bottom <= otherRect.Top;

			hasP2RightIntersection =
				otherRect.Right >= rect.Left && otherRect.Right <= rect.Right;
			hasP2LeftIntersection =
				otherRect.Left >= rect.Left && otherRect.Left <= rect.Right;
			hasP2TopIntersection =
				otherRect.Top >= rect.Bottom && otherRect.Top <= rect.Top;
			hasP2BottomIntersection =
				otherRect.Bottom >= rect.Bottom && otherRect.Bottom <= rect.Top;

			switch (type) {
				case CONSTANTS.RIGHT /*moving right*/: {
					if (hasP1RightIntersection && hasP1BottomIntersection)
						distance = rect.Right - otherRect.Left;
					else if (hasP1ExistsingRightIntersection && hasP1BottomIntersection)
						distance = -(rect.OldRight - otherRect.Left);
					break;
				}
				case CONSTANTS.LEFT /*moving left*/: {
					if (hasP1LeftIntersection && hasP1BottomIntersection)
						distance = rect.Left - otherRect.Right;
					else if (hasP1ExistsingLeftIntersection && hasP1BottomIntersection)
						distance = -(rect.OldLeft - otherRect.Right);
					break;
				}
				case CONSTANTS.LEFT_AND_CHECK_RIGHT /*moving left, but checks for right side collision as well*/: {
					if (
						(hasP1LeftIntersection || hasP1RightIntersection) &&
						hasP1BottomIntersection
					)
						distance = rect.Left - otherRect.Right;
					break;
				...
				case CONSTANTS.UP /*moving up*/: {
					if (
						(hasP1TopIntersection || hasP2TopIntersection) &&
						(hasP1RightIntersection || hasP2RightIntersection)
					)
						distance = rect.Top - otherRect.Bottom;
					break;
				}
				case CONSTANTS.DOWN /*moving down*/: {
					if (
						(hasP1BottomIntersection || hasP2BottomIntersection) &&
						(hasP1RightIntersection || hasP2RightIntersection)
					)
						distance = rect.Bottom - otherRect.Top;
					break;
				}
				default: /*check for any intersection*/ {
					if (
						hasP1LeftIntersection ||
						(hasP1RightIntersection &&
							hasP1BottomIntersection) /*right or left*/ ||
						hasP2LeftIntersection ||
						(hasP2RightIntersection &&
							hasP1BottomIntersection) /*right or left*/
					) {
		...
		while (temp.length > 0) {
			...
			if (index > -1) retVal[retVal.length] = temp.splice(index, 1)[0].Player;
		}
	...
	CDHelper.prototype.tryAttack = function (
		hitStop,
		hitID,
		attackID,
		maxNbHits,
		...
		if (!p2) return;
		if (p2.Flags.Player.has(PLAYER_FLAGS.IGNORE_ATTACKS)) return;
		/*some moves can only hit a certain number of times - e.g. MBison psycho crusher*/
		if (!!p2.Hits[attackID] && p2.Hits[attackID].Nb >= maxNbHits) return;
		...
		if (!!isEjecting && !isP1GrapplingP2) return;
		/*Is p2 being grappled by p1?*/
		if (isP2BeingGrappled && !isP1GrapplingP2) return;
		/*Can only start a grapple once*/
		if (isP2BeingGrappled && isP1GrapplingP2 && !isEjecting) return;
		if (!p2.isVisible() && !p2.isTeleporting()) return;
		if (p1.canJuggle(p2, attackID, otherParams.HitJuggleGroup)) {
		} else if (
		/*need to reform the "invulernable" flags - there are too many*/
			p2.Flags.Player.has(PLAYER_FLAGS.SUPER_INVULNERABLE) &&
			!hasFlag(behaviorFlags, BEHAVIOR_FLAGS.THROW)
		)
			return;
		/*frame can not hit more than once*/
		if (!!p2.isHitFrameOk(p1.Id, p1.getHitFrameID(hitID))) return;
		/*if the attack is a throw, it can not grab more than one player*/
		if (isP1Grappling && !isP1GrapplingP2) return;
		if (isP2Grappling) return;
		if (p1.canJuggle(p2)) {
		} else if (
			p2.isAirborne() &&
			hasFlag(attackFlags, ATTACK_FLAGS.CAN_AIR_JUGGLE)
		) {
			//return;
		...
		if (p1.Direction < 0) {
			for (var i = 0; i < points.length; ++i) {
				...
				if (
					(points[i].x == -1 && !!p2.isBeingGrappled()) ||
					(x >= p2Left && x < p2Right && y >= p2Bottom && y < p2Top)
				) {
			...
			for (var i = 0; i < points.length; ++i) {
				...
				if (
					(points[i].x == -1 && !!p2.isBeingGrappled()) ||
					(x <= p2Right && x > p2Left && y >= p2Bottom && y < p2Top)
				) {
					p1.setGiveHit(attackFlags, hitStop, energyToAdd, behaviorFlags, p2);
					p2.setRegisteredHit(
						attackFlags,
	...
	CDHelper.prototype.tryProjectileAttack = function (
		frame,
		projectile,
		p1,
		p2,
	) {
		if (!!projectile && !!projectile.IsDisintegrating) return;
		if (!p2) return;
		if (p2.isGrappling()) return;
		if (!p2.isVisible() && !p2.isTeleporting()) return;
		if (p2.Flags.Player.has(PLAYER_FLAGS.IGNORE_ATTACKS)) return;
		if (p2.Flags.Player.has(PLAYER_FLAGS.SUPER_INVULNERABLE)) return;
		if (p2.Flags.Player.has(PLAYER_FLAGS.IGNORE_PROJECTILES)) return;
		...
		if (projectile.Direction < 0) {
			...
			if (
				(((x0 >= p2Left && x0 < p2Right) || (x1 >= p2Left && x1 < p2Right)) &&
					((y0 >= p2Bottom && y0 < p2Top) || (y1 >= p2Bottom && y1 < p2Top))) ||
				(projectile.isFirstFrame() &&
					x1 >= p2Right &&
					p2Left > p1Left &&
					((y0 >= p2Bottom && y0 < p2Top) || (y1 >= p2Bottom && y1 < p2Top)))
			) {
				...
				var hitX = (x1 - x0) / 2 + x0;
				if (p2.Direction > 0) hitX = STAGE.MAX_STAGEX - hitX;
				var hitY = (y1 - y0) / 2 + y0;
				if (
					p2.setRegisteredHit(
						projectile.getAttackState(p2.Hits[projectile.Id]),
						projectile.getHitState(p2.Hits[projectile.Id]),
						projectile.FlagsToSend,
						frame,
						projectile.BaseDamage,
						projectile.EnergyToAdd,
						false,
						true,
						hitX,
						hitY,
						...
						p1.Id,
						projectile.getHitId(frame),
						projectile.Id,
						projectile.OverrideFlags,
						projectile.Owner,
						projectile.Fx,
						projectile.Fy,
						0,
						0,
						projectile.HitSound,
						projectile.BlockSound,
						projectile.getHitStop(),
						projectile.MaxHits,
						projectile.Params,
					)
				) {
				...
				for (var i = 0; i < p2.Projectiles.length; ++i)
					this.tryProjectileHitProjectile(
						frame,
						x0,
						x1,
						y0,
			...
			x1 = projectile.getFrontX();
			if (
				(((x1 >= p2Left && x1 < p2Right) || (x0 >= p2Left && x0 < p2Right)) &&
					((y0 >= p2Bottom && y0 < p2Top) || (y1 >= p2Bottom && y1 < p2Top))) ||
				(projectile.isFirstFrame() &&
					x1 < p2Left &&
					p2Right < p1Right &&
	...
	CDHelper.prototype.tryProjectileHitProjectile = function (
		frame,
		x0,
		x1,
		y0,
		...
		if (!!projectile.IsDisintegrating || !!otherProjectile.IsDisintegrating)
			return;
		if (!otherProjectile.IsActive) return;
		var otherProjectileBottom = parseInt(otherProjectile.Element.style.bottom);
		var otherProjectileTop =
			parseInt(otherProjectile.Element.style.bottom) +
			parseInt(otherProjectile.Element.style.height);
		var otherProjectileLeft = 0;
		var otherProjectileRight = 0;

		if (projectile.Direction < 0) {
			otherProjectileRight =
				STAGE.MAX_STAGEX - parseInt(otherProjectile.Element.style.right);
			otherProjectileLeft =
				STAGE.MAX_STAGEX -
				parseInt(otherProjectile.Element.style.right) -
				parseInt(otherProjectile.Element.style.width);

			if (
				((x0 >= otherProjectileLeft && x0 < otherProjectileRight) ||
					(x1 >= otherProjectileLeft && x1 < otherProjectileRight)) &&
				((y0 >= otherProjectileBottom && y0 < otherProjectileTop) ||
					(y1 >= otherProjectileBottom && y1 < otherProjectileTop))
			) {
			...
			otherProjectileRight =
				parseInt(otherProjectile.Element.style.width) +
				parseInt(otherProjectile.Element.style.left);
	...
	CDHelper.prototype.moveOtherPlayers = function (player) {
		...
		for (var i = 0, length = collisions.length; i < length; ++i) {
			...
			if (otherPlayer.getMidX() >= player.getMidX()) {
				/*must move to the right. If there isn't enough space, then move to the left*/
				var deltaX = 0;
			...
			} /*must move to the left. If there isn't enough space, then move to the right*/ else {
				var deltaX = 0;
				if (otherPlayer.Direction == 1) {
					deltaX = otherRect.Right - myRect.Left;
	...
	CDHelper.prototype.fixX = function (
		amount,
		player,
		dontOverrideSign,
		canFixStageX,
		...
		if (!!amount) {
			...
			if (amount > 0) {
				...
				var collisions = GetPlayersAtPosition_(
					myRect,
					player,
					CONSTANTS.RIGHT_AND_CHECK_LEFT,
				).reverse();
				for (var i = 0, length = collisions.length; i < length; ++i) {
					...
					if (!!impededAmount) {
						if (myRect.Right <= otherRect.Right) {
							...
							impededAmount =
								Math.abs(otherRect.Right - myRect.Left) * direction;
							this.moveX(impededAmount, player, true, false, true);
						}
					}
				}
				...
				var collisions = GetPlayersAtPosition_(
					myRect,
					player,
					CONSTANTS.LEFT_AND_CHECK_RIGHT,
				).reverse();
				for (var i = 0, length = collisions.length; i < length; ++i) {
					collisions[i].moveCircleToTop();
					var otherRect = collisions[i].getRect();
					...
					if (!!impededAmount) {
						if (myRect.Left >= otherRect.Left) {
							...
							impededAmount =
								Math.abs(myRect.Right - otherRect.Left) * direction;
							this.moveX(impededAmount, player, true, false, true);
						}
					}
				}
	...
	CDHelper.prototype.moveX = function (
		amount,
		player,
		dontOverrideSign,
		canFixStageX,
		...
		if (!!amount) {
			...
			if (amount > 0) {
				...
				for (var i = 0, length = collisions.length; i < length; ++i) {
					if (player.Flags.Pose.has(POSE_FLAGS.ALLOW_OVERLAP)) continue;
					if (!!ignoredPlayer && ignoredPlayer == collisions[i].Id) continue;
					...
					if (!!impededAmount) {
						...
						if (
							(player.isOnGround() && collisions[i].isOnGround()) ||
							(!player.isOnGround() && !collisions[i].isOnGround()) ||
							collisions[i].isRightCornered()
						) {
							amountPushed = this.moveX(
								!!doubleImpededAmount ? impededAmount : impededAmount / 2,
								collisions[i],
								true,
								false,
								true,
			...
			if (!!canFixStageX && !!stageFixX) getStage().fixX(stageFixX);
		}
	...
	CDHelper.prototype.moveY = function (amount, player) {
		if (!!amount) {
			...
			if (amount > 0) {
				...
				for (var i = 0, length = collisions.length; i < length; ++i) {
					...
					var amountRejected = player.Flags.Pose.has(POSE_FLAGS.ALLOW_OVERLAP)
						? 0
						: player.Circle.rejectX(otherPlayer.Circle);

					if (!!amountRejected) {
						if (myMidX > otherMidX) {
							/*on right side of the other player*/
							...
							else this.moveX(amountRejected, player, true);
						} /*on left side of other player*/ else {
							amountRejected = -Math.abs(amountRejected);
							/*must move the current player away from the play that was contacted*/
							if (player.isLeftCornered())
								this.moveX(-amountRejected, otherPlayer, true);
				...
				for (var i = 0, length = collisions.length; i < length; ++i) {
					...
					/*special case: when the airborne player is jumping and cornered*/
						HasIntersection_(player, otherPlayer) &&
						(player.isLeftCornered() || player.isRightCornered())
					) {
						if (myMidX > otherMidX) {
							/*on right side of the other player*/
		...
		if (!!amount) getStage().requestScrollY(amount > 0, player.Y);
		return amount;
	};
	...
	CDHelper.prototype.isAnyPlayerAirborne = function () {
		var match = getMatch();
		for (
			var i = 0, length = match.getTeamB().getPlayers().length;
			i < length;
			++i
		)
			if (match.getTeamB().getPlayer(i).isAirborne()) return true;
		for (
			var i = 0, length = match.getTeamA().getPlayers().length;
			i < length;
			++i
		)
			if (match.getTeamA().getPlayer(i).isAirborne()) return true;
	...
	CDHelper.prototype.getLeftMostPlayer = function () {
		...
		if (!tALength || !tBLength) return null;
	...
	CDHelper.prototype.getGrappledPlayer = function (
		team,
		x,
		y,
		distance,
	...
	CDHelper.prototype.isWithinDistanceX = function (p1, p2, distance, useFront) {
		if (!useFront) {
			var x = p1.getMidX();
			return Math.abs(x - p2.getMidX()) < distance;
		} else {
			var p1Rect = p1.getImgRect();
			var p2Rect = p2.getImgRect();
		...
		switch (team) {
			case CONSTANTS.TEAM1: {
				...
				for (var i = 0; i < nbPlayers; ++i)
					if (
						!!match.getTeamB().getPlayer(i).isVisible() &&
						match.getTeamB().getPlayer(i).getMidX() < x
					)
						return true;
				break;
			}
			case CONSTANTS.TEAM2: {
				...
				for (var i = 0; i < nbPlayers; ++i)
					if (
						!!match.getTeamA().getPlayer(i).isVisible() &&
						match.getTeamA().getPlayer(i).getMidX() < x
					)
						return true;
				break;
			}
	...
	CDHelper.prototype.getRearEnemy = function (
		myTeam,
		myDirection,
		myRect,
		myTarget,
