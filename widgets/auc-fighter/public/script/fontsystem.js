...
ManagedText.prototype.animateIncNumber = function (startValue, endValue) {
	this.animate(
		startValue,
		function (value) {
...
ManagedText.prototype.animateDecNumber = function (startValue, endValue) {
	this.animate(
		startValue,
		function (value) {
			return value > endValue;
...
ManagedText.prototype.change = function (
	newText,
	x,
	hideFrame,
	showFrame,
	...
	for (var i = 0, length = this.Text.length; i < length; ++i) {
		this.Width +=
			(!!+this.Text.charAt(i) ? 2 * this.CharWidth : this.CharWidth) +
			this.FontSpacing;
...
ManagedText.prototype.hideNow = function () {
	this.setVisible(false);
};
ManagedText.prototype.hide = function () {
	this.setTarget(-this.Width, this.Y);
};
ManagedText.prototype.hideFadeNow = function () {
	this.X = -this.Width;
};

ManagedText.prototype.clearTarget = function () {
	this.dX = 0;
	this.dY = 0;
	this.HideFrame = -1;
};
ManagedText.prototype.showNow = function (x, hideFrame) {
	this.X = (x != 0 ? x : null) || 0;
	this.MustUpdate = true;
	if (!!hideFrame) {
...
ManagedText.prototype.show = function (hideFrame) {
	this.setTarget(this.Indent, this.Y);
	if (!!hideFrame) {
		this.HideFrame = hideFrame;
	}
};
...
ManagedText.prototype.frameMove = function (frame) {
    ...
    */
	if (frame > this.ShowFrame) {
		if (!!this.dX) {
			this.moveX(this.dX);
			if (this.X >= this.TargetX) this.dX = 0;
		}
		if (!!this.dY) {
			this.moveY(this.dY);
			if (this.Y >= this.TargetY) this.dY = 0;
		}
	}
};
...
var CreateFontSystem = function () {
	...
	FontSystem.prototype.addText = function (
		elementID,
		text,
		x,
		y,
