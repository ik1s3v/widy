var CreateSoundManager = function () {
	...
	SoundManager.prototype.getExtension = function () {
		return extension_;
	};
	...
	SoundManager.prototype.loadBase64 = function (
		path,
		nbChannels,
		defaultVolume,
		base64Data,
	...
	SoundManager.prototype.queueSound = function (value, volume, delay) {
		if (!isEnabled_) return;
		sounds_[_c3(value, "_", delay)] = {
			Value: value,
			Volume: volume || 1,
			Frame: game_.getCurrentFrame() + (delay || 0),
		};
	};
	...
	SoundManager.prototype.stopAny = function (value) {
		if (!isEnabled_) return;
		for (var i in sounds_) if (sounds_[i].Value == value) delete sounds_[i];
	};
