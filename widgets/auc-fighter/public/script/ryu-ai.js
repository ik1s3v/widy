var CreateRyuAI = function (player) {
	...
	RyuAI.prototype.getForwardKey = function () {
		this.AI.Player.MustChangeDirection ? BUTTONS.BACK : BUTTONS.FORWARD;
	};
	RyuAI.prototype.wanderForward = function (nbFrames) {
	...
	RyuAI.prototype.wanderBackward = function (nbFrames) {
	...
	RyuAI.prototype.onEnemyAttackPending = function (
		frame,
		x,
		y,
		player,
	...
	RyuAI.prototype.onEnemyProjectilePending = function (
		frame,
		x,
		y,
		player,
	...
	RyuAI.prototype.onEnemyProjectileMoved = function (
		frame,
		id,
		x,
		y,
	...
	RyuAI.prototype.onEnemyContinueAttack = function (
		frame,
		attacker,
		hitPoints,
	) {
	...
	RyuAI.prototype.doMove = function (key) {
		this.execute(this.AI.SingleMoves[key]);
	};
	RyuAI.prototype.doCloseCombo = function (key) {
		this.execute(
			this.AI.CloseCombos[key || getRand(this.AI.CloseCombos.length - 1)],
		);
	};
	RyuAI.prototype.doVeryCloseCombo = function (key) {
		this.execute(
			this.AI.VeryCloseCombos[
				key || getRand(this.AI.VeryCloseCombos.length - 1)
			],
		);
	};
	RyuAI.prototype.doJumpInCombo = function (key) {
		this.execute(
			this.AI.JumpInCombos[key || getRand(this.AI.JumpInCombos.length - 1)],
		);
	};
	RyuAI.prototype.doCloseJumpInCounterCombo = function (key) {
		this.execute(
			this.AI.CounterCloseJumpInCombos[
				key || getRand(this.AI.CounterCloseJumpInCombos.length - 1)
			],
		);
	};
	RyuAI.prototype.doCounterProjectileCombo = function (key) {
		this.execute(
			this.AI.CounterProjectileCombos[
	...
	RyuAI.prototype.doCounterCloseProjectileCombo = function (key) {
		this.execute(
			this.AI.CounterCloseProjectileCombos[
	...
	RyuAI.prototype.doCounterVeryCloseProjectileCombo = function (key) {
		this.execute(
			this.AI.CounterVeryCloseProjectileCombos[
	...
	RyuAI.prototype.react = function (frame, attacker, isVulnerable) {
		...
		else this.reactNotAirborne(frame, attacker, isVulnerable);
	};
	...
	RyuAI.prototype.proact = function (frame) {
		...
		if (item.X < CONSTANTS.GRAPPLE_DISTANCE) {
			...
			if (item.Player.isInThrowableState() && this.AI.JustBecameMobile > 0) {
				if (rnd > 50) this.doVeryCloseCombo(0);
				else this.doVeryCloseCombo(1);
			} else {
			...
			this.AI.reset();
			if (rnd > 70) this.doCloseCombo(getRand(this.AI.CloseCombos.length - 1));
			...
			this.AI.reset();
			if (rnd > 80) this.executeFireball(getRand());
			else if (rnd > 50)
				this.doCloseCombo(getRand(this.AI.CloseCombos.length - 1));
			else {
